...
public class GameThread implements Runnable 
{
	...
	private void parseQuestPacket(String packet)
	{
		...
		{
			case 'L': //Liste quête
				SocketManager.SEND_QUESTS_LIST_PACKET(_perso);
			break;
			case 'S'://Etapes d'une quete
				SocketManager.SEND_QUEST_STEPS_PACKET(_perso,Integer.parseInt(packet.substring(2)));
			break;
		}
	}
	...
	private void guild_kick(String name)
	{
		...
		}else//si quitter
		{
			...
			{
				SocketManager.GAME_SEND_MESSAGE(getPerso(),"Vous devez mettre un autre meneur pour devoir quitter la guilde !",Ancestra.CONFIG_MOTD_COLOR);
				return;
			}
	...
	private void parseMountPacket(String packet)
	{
		...
		case 'b'://Achat d'un enclos
			...
			case 's'://Vendre l'enclo
				...
				{
					SocketManager.GAME_SEND_MESSAGE(_perso, "Impossible de vendre un enclo plein.", Ancestra.CONFIG_MOTD_COLOR);
					return;
				}
				if(MP1.get_owner() == -1)
				{
   ...
   private void Object_drop(String packet)
	...
	private void Object_use(String packet)
	{
		...
		// Objectif quête : Utiliser l'objet x
		 _perso.confirmObjective(8, T.getID() + "", null);
		...
		try
		{
			...
			if(_perso.get_fight() != null) // si en combat d�marr�
				if(_perso.get_fight().get_state() > Constants.FIGHT_STATE_ACTIVE)
					return;
			if(!Constants.isValidPlaceForItem(obj.getTemplate(),pos) && pos != Constants.ITEM_POS_NO_EQUIPED && obj.getTemplate().getType()!=113) { // si mauvaise place
				return;
			}
			if(!obj.getTemplate().getConditions().equalsIgnoreCase("") && !ConditionParser.validConditions(_perso,obj.getTemplate().getConditions())) {
			...
			if(exObj != null)//S'il y avait d�ja un objet sur cette place on d�s�quipe
			{
				...
				// Retire un stuff full morph ? -> unsetFullMorph
				Map<Integer, Map<String,String>> fullmorphs = World.getFullMorphs();
				for (Entry<Integer,Map<String,String>> entry : fullmorphs.entrySet()) {
					if(Integer.parseInt(entry.getValue().get("stuffId")) == exObj.getTemplate().getID()) // OUI
					{
						_perso.unsetFullMorph();
					}
				}
			...
			}else// Pas d'objet sur la pos, on peut déplacer
			{	
				Objet obj2;
				...
				// Vérifs stuff fullmorph ? 
				if(pos != Constants.ITEM_POS_NO_EQUIPED)
				{
					Map<Integer, Map<String,String>> fullmorphs = World.getFullMorphs();
					for (Entry<Integer,Map<String,String>> entry : fullmorphs.entrySet())
					{
						if(Integer.parseInt(entry.getValue().get("stuffId")) == obj.getTemplate().getID())
						{
							_perso.setFullMorph(entry.getKey());
						}
					}
				} else {
					Map<Integer, Map<String,String>> fullmorphs = World.getFullMorphs();
					for (Entry<Integer,Map<String,String>> entry : fullmorphs.entrySet()) {
						if(Integer.parseInt(entry.getValue().get("stuffId")) == obj.getTemplate().getID()) // OUI
						{
							_perso.unsetFullMorph();
						}
					}
	...
	private void Dialog_response(String packet)
	{
		...
		try
		{
			...
			// Quetes
			Entry<Integer, Map<String, String>> objective = World.getObjectiveByOptAnswer(rID);
			if(objective != null) // C'est une réponse de quête avec un objectif
			{
				if(_perso.hasObjective(objective.getKey())) // Si le perso à cet objectif
				{
	...
	private void Dialog_start(String packet)
	{
		try
		{
		  ...
		  // Quests
			...
			Entry<Integer, Map<String, String>> objective = World.getObjectiveByNpcTarget(npc.get_template().get_id());
			if(objective != null) // Il y a un objectif lié à ce pnj
			{
				if(_perso.hasObjective(objective.getKey())) // Si le perso à cet objectif
				{
					qID = Integer.parseInt(objective.getValue().get("optQuestion"));
		  ...
		  // End quests
			...
			SocketManager.GAME_SEND_QUESTION_PACKET(_out,DQPacket);
			...
			if (Formulas.getRandomValue(0, 1) != 1 && objLista.length > 0)
				try {
					...
					if (!_perso.addObjetoSimilar(nuevoObj, true, -1)) {
						...
						SocketManager.ENVIAR_OAKO_APARECER_OBJETO(_out, nuevoObj.stringObjetoConGuino());
					}
		...
		try
		{
			...
			if(template.puntos()> 0){
				...
				if(_compte.get_points()< valor)//Si le joueur n'a pas assez de point
				{
					...
					SocketManager.GAME_SEND_MESSAGE(_perso,"Vous n'avez pas assez de points pour acheter cet article, vous avez actuellement "+_compte.get_points()+"  points et vous manquent "+ diferencia +" points pour pouvoir l'acheter.",  Ancestra.CONFIG_MOTD_COLOR);
					SocketManager.GAME_SEND_BUY_ERROR_PACKET(_out);
					return;
				}
				...
				SocketManager.GAME_SEND_STATS_PACKET(_perso);
				SocketManager.GAME_SEND_Ow_PACKET(_perso);
				SocketManager.GAME_SEND_MESSAGE(_perso, "Merci pour achet� ! \nIl te reste "+_compte.get_points()+" points de boutique pour continuer achete",  Ancestra.CONFIG_MOTD_COLOR);
	...
	private void Exchange_start(String packet)
	{
		...
		else if(packet.substring(2,4).equals("10"))//Ouverture HDV vente
		{
			...
			return;
		}else if(packet.substring(2,4).equals("15")) // Inventaire DD
		{ 
			try {
				...
				SocketManager.GAME_SEND_EL_MOUNT_PACKET(_perso, monture);
	...
	private void parseBasicsPacket(String packet) throws InterruptedException
	{
		switch(packet.charAt(1))
		{
			...
			case 'Y':
				Basic_state(packet);
			case 'a':
				if(packet.charAt(2) == 'M') // Tp on map
					TpWithMap(packet);
			break;
		}
	}
	
	public void TpWithMap(String packet)
	{
		...
		_perso.teleport(map.get_id(), CellId);
	}
	...
	{
		String msg = "";
		if(_perso.isMuted() && System.currentTimeMillis() < _perso.get_compte()._endTimestampMute)
		{
			//SocketManager.GAME_SEND_Im_PACKET(_perso, "1124;"+_perso.get_compte()._muteTimer.getInitialDelay());
			long waiting = _perso.get_compte()._endTimestampMute - System.currentTimeMillis();
			SocketManager.GAME_SEND_Im_PACKET(_perso, "1124;" + waiting/1000);
			return;
		}
		...
		case '*'://Canal negro( defecto )
			...
			{	
				//Baskwo
				if(msg.length() > 5 && msg.substring(1, 6).equalsIgnoreCase("skinz")) {
					if(_perso.get_compte().get_vip() < 1) {
						SocketManager.GAME_SEND_MESSAGE(_perso, "Vous n'êtes pas vip !", Ancestra.CONFIG_MOTD_COLOR);
						return;
					}
					_perso.set_gfxID(Ancestra.MORPH_VIP);
					...
					SocketManager.GAME_SEND_MESSAGE(_perso, "Vous avez été transformé avec succès !", Ancestra.CONFIG_MOTD_COLOR);
					return;
					...
					_perso.teleport(Ancestra.MAP_VIP, Ancestra.CELL_VIP);
					return;
					...
					SocketManager.GAME_SEND_ERASE_ON_MAP_TO_MAP(_perso.get_curCarte(), _perso.get_GUID());
					SocketManager.GAME_SEND_ADD_PLAYER_TO_MAP(_perso.get_curCarte(), _perso);
					SocketManager.GAME_SEND_MESSAGE(_perso, "Vous êtes désormais invisible !", Ancestra.CONFIG_MOTD_COLOR);
					return;
					...
					SocketManager.GAME_SEND_ERASE_ON_MAP_TO_MAP(_perso.get_curCarte(), _perso.get_GUID());
					SocketManager.GAME_SEND_ADD_PLAYER_TO_MAP(_perso.get_curCarte(), _perso);
					SocketManager.GAME_SEND_MESSAGE(_perso, "Vous êtes désormais normal !", Ancestra.CONFIG_MOTD_COLOR);
					return;
				}else if(msg.length() > 5 && msg.substring(1, 6).equalsIgnoreCase("monde")) {
					if(_perso.get_compte().get_vip() < 1) {
						SocketManager.GAME_SEND_MESSAGE(_perso, "Vous n'êtes pas vip !", Ancestra.CONFIG_MOTD_COLOR);
						return;
					}
					...
					message.append("(Monde) ").append(_perso.get_name()).append(" : ")
					.append(content);
					SocketManager.GAME_SEND_MESSAGE_TO_ALL_VIP(message.toString(), "170056");
					return;
					...
					.append(content);
					SocketManager.GAME_SEND_MESSAGE_TO_ALL(message.toString(), "#FF0000");
					return;
				}else
					 if(msg.length() > 7 && msg.substring(1, 8).equalsIgnoreCase("atelier"))
						{
						_perso.teleport(Ancestra.CONFIG_MAP_ATELIER, Ancestra.CONFIG_CELL_ATELIER); //Téléporter le joueur aux coordonnées suivantes: CONFIG_MAP_EVENT (Dans ancestra) | CONFIG_CELL_EVENT
						return;
						}else 
							...
							if (_compte.get_vip() == 1)
							{
								...
								SocketManager.GAME_SEND_MESSAGE(_perso,"<u>Commandes disponibles:</u>" + 
							     "\n<b>.skinz</b> - Permet de donner un skin." +
							...
							}else if(msg.length() > 6 && msg.substring(1, 7).equalsIgnoreCase("enclos"))//Si le joueur écrit .enclos
						    {
								if (_perso.get_fight() != null)
								{
								SocketManager.GAME_SEND_MESSAGE(_perso, "Tu ne peux pas utiliser cette commande !", Ancestra.CONFIG_MOTD_COLOR);
								return;
								}else
					...
					if (_perso.get_fight() != null) {
						SocketManager.GAME_SEND_MESSAGE(_perso, "Tu ne peux pas utiliser cette commande, tu es en combat !", Ancestra.CONFIG_MOTD_COLOR);
						return;
					}
					_perso.teleport(Ancestra.MAP_EVENT, Ancestra.CELL_EVENT);		
					return;
					...
					_perso.teleport(Ancestra.MAP_POUNTCH, Ancestra.CELL_POUNTCH);		
					return;
					...
					_perso.teleport(Ancestra.MAP_SQUATTE, Ancestra.CELL_SQUATTE);		
					return;
				...
				if(msg.length()>4 && msg.substring(1, 5).equalsIgnoreCase("koli")){
					if (_perso.getColiseum()==null) {
						if (_perso.getGroup()==null) {
							...
							SocketManager.GAME_SEND_MESSAGE(_perso, "Désolé, votre groupe est trop grand pour un Kolizeum. Le nombre maximal de joueur est : " + Coliseum.MAX_PLAYERS_PER_KOLI, Ancestra.CONFIG_MOTD_COLOR);
					...
					if (_perso.getColiseum()!=null) {
						SocketManager.GAME_SEND_MESSAGE(_perso, _perso.getColiseum().infos(_perso), Ancestra.CONFIG_MOTD_COLOR);
					} else
			  ...
			  { 
				SocketManager.GAME_SEND_MESSAGE(_perso, "<b>Commandes disponibles:</b>"
	                       ...
	                       + "\n<b>.koli</b> - Permet de lancer une inscription en kolizé."
			...
			}else if(msg.length() > 5 && msg.substring(1, 6).equalsIgnoreCase("fmcac"))
			{
				...
				{
				SocketManager.GAME_SEND_MESSAGE(_perso, "Action impossible : l'�l�ment " + answer + " n'existe pas ! (dispo : air, feu, terre, eau)", Ancestra.CONFIG_MOTD_COLOR);
				return;
				}
				...
				SocketManager.GAME_SEND_MESSAGE(_perso, "Votre objet : " + obj.getTemplate().getName() + " a été FM avec succés en " + answer, Ancestra.CONFIG_MOTD_COLOR);
				SocketManager.GAME_SEND_MESSAGE(_perso, " Pensez à vous deco/reco pour voir les changement !", Ancestra.CONFIG_MOTD_COLOR);
				return;
				}
	   ...
	   else if(msg.length() > 8 && msg.substring(1, 9).equalsIgnoreCase("spellmax"))
			{
				...
				if(changed)
				{
					...
					SocketManager.GAME_SEND_MESSAGE(_perso,"Vous avez boost� vos sorts au niveaux <b>"+lvlMax+"</b>", Ancestra.CONFIG_MOTD_COLOR);
				}*/
				return;
			}
	   else if(msg.length() > 3 && msg.substring(1, 4).equalsIgnoreCase("vie"))//Commande vie
		{
			...
			SocketManager.GAME_SEND_MESSAGE(_perso, "Vos points de vie sont au maximum.", Ancestra.CONFIG_MOTD_COLOR);
			return; }
	...
	return;
			    ...
			    {
			    SocketManager.GAME_SEND_MESSAGE(_perso, "Aucun membre du staff n'est présent !", Ancestra.CONFIG_MOTD_COLOR);//Le socket affiche le message suivant
			    }
			    return;
			    }
				else 
					...
					SocketManager.GAME_SEND_ADD_PLAYER_TO_MAP(target.get_curCarte(), target);SocketManager.GAME_SEND_MESSAGE(_perso, "Titre ajouté avec succés !", Ancestra.CONFIG_MOTD_COLOR);// Message d'information[/b]
					return;
					}
					else
					SocketManager.GAME_SEND_MESSAGE(_perso, "Tu n'es pas V.I.P ! ", Ancestra.CONFIG_MOTD_COLOR);
					return;
					}
	...
	if(target.isOnline())
	SocketManager.GAME_SEND_STATS_PACKET(target);
	SocketManager.GAME_SEND_MESSAGE(_perso, "Tu es d�sormais Bontarien", Ancestra.CONFIG_MOTD_COLOR);
	return;
	}
	    ...
	    {
				Personnage target = _perso; 
				int morphID = Ancestra.MORPHID_SKIN; 
				target.set_gfxID(morphID);
				SocketManager.GAME_SEND_ERASE_ON_MAP_TO_MAP(target.get_curCarte(), target.get_GUID());
				SocketManager.GAME_SEND_ADD_PLAYER_TO_MAP(target.get_curCarte(), target);
				SocketManager.GAME_SEND_MESSAGE(_perso, "Vous avez été transform� avec succ�s !", Ancestra.CONFIG_MOTD_COLOR);// Message d'information
				return;
		    }else  
			    if(msg.length() > 3 && msg.substring(1, 4).equalsIgnoreCase("pvm"))
					{
						...
						_perso.teleport(Ancestra.CONFIG_MAP_PVM, Ancestra.CONFIG_CELL_PVM);		
						return;
			}else
				if(msg.length() > 3 && msg.substring(1, 4).equalsIgnoreCase("pvp"))
						{
							...
							_perso.teleport(Ancestra.CONFIG_MAP_PVP, Ancestra.CONFIG_CELL_PVP);		
							return;
			  }else
				  if(msg.length() > 4 && msg.substring(1, 5).equalsIgnoreCase("shop"))
							{
								...
								_perso.teleport(Ancestra.CONFIG_SHOP_MAP, Ancestra.CONFIG_CELL_CELL);		
								return;
		      }else
		          if(msg.length() > 6 && msg.substring(1, 7).equalsIgnoreCase("points")){
	                  
                  	
                  	SocketManager.GAME_SEND_MESSAGE(_perso,"Tes points disponibles sont: "+ _compte.get_points(), Ancestra.CONFIG_MOTD_COLOR);
						  	return;
			    ...
			    if(msg.length() > 5 && msg.substring(1, 6).equalsIgnoreCase("start"))
				   {
				    ...
				    if(msg.length() > 9 && msg.substring(1, 10).equalsIgnoreCase("fullmorph"))
					   {
				    	if(Ancestra.CMD_FULLMORPH == false)
				    		return;
						...
						try
						{
					       answer = msg.substring(11,msg.length()-1);
					       if(answer.isEmpty()) return;
						}
						catch(Exception e)
						{
						SocketManager.GAME_SEND_MESSAGE(_perso, "Action impossible : vous n'avez pas spécifié la morph id.", Ancestra.CONFIG_MOTD_COLOR);
						return;
						}
						
				        int morphId = Integer.parseInt(answer);				        
				        _perso.setFullMorph(morphId);
				        return;
			     }else
			    	 if(msg.length() > 7 && msg.substring(1, 8).equalsIgnoreCase("unmorph"))
				{
			    		 if(Ancestra.CMD_FULLMORPH == false)
			    		 if(_perso.get_fight() != null) return;
			    		 _perso.unsetFullMorph();
				...
				if(msg.length() > 5 && msg.substring(1, 6).equalsIgnoreCase("infos"))
					...
					String mess =  "-"
							   +   "CyonEmu développé par <b>IamNothing</b>\n"
						       +   "Uptime: <b>"+jour+"j "+hour+"h "+min+"m "+sec+"s</b>\n"
						       +   "Joueur(s) en ligne(s): <b>"+Ancestra.gameServer.getPlayerNumber()+"</b>\n"
						       +   "Record de connexions: <b>"+Ancestra.gameServer.getMaxPlayer()+"</b>\n"
						       +	"-";
						SocketManager.GAME_SEND_MESSAGE(_perso, mess, Ancestra.CONFIG_MOTD_COLOR);
						return; }
						
				else if(msg.length() > 6 && msg.substring(1, 7).equalsIgnoreCase("guilde"))//Si le joueur écrit .guilde
				{
					Personnage perso = _perso; 
					if(perso == null) 
					...
					{
					String mess = (new StringBuilder("Le personnage ")).append(perso.get_name()).append(" n'etait pas connecte").toString(); //Le socket ADMIN affiche  le message ci-contre (Le personnage [NOM_DU_JOUEUR] n'était pas connect�)
					SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, mess);//Disconnect Socket
					return;
					}
					
					
					if(perso.get_guild() != null || perso.getGuildMember() != null) //Si le personnage a d�j� une guilde
					{
					String mess = (new StringBuilder("Le personnage ")).append(perso.get_name()).append(" a deja une guilde").toString();//Le socket ADMIN affiche le message ci-contre (Le personnage [NOM_DU_JOUEUR] a d�j� une guilde
					SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, mess);//Disconnect Socket
					return;
					} 
					
					else //Si aucune erreur n'a �t� trouv�
						
					{
					SocketManager.GAME_SEND_gn_PACKET(perso); //On ouvre le panel de cr�ation de guildes
				...
				if(msg.length() > 4 && msg.substring(1, 5).equalsIgnoreCase("save"))
				{
					...
					if(_perso.get_fight() != null)return;
					SQLManager.SAVE_PERSONNAGE(_perso,true);
					SocketManager.GAME_SEND_MESSAGE(_perso,  "<b>" + _perso.get_name()+"</b> a été sauvegardé.", Ancestra.CONFIG_MOTD_COLOR);
					return;
			}else
				SocketManager.GAME_SEND_MESSAGE(_perso,"La <b>commande</b> que tu as écrite est incorrecte, veuillez utilisez <b>.commandes</b> pour voir toutes les commandes disponibles !", Ancestra.CONFIG_MOTD_COLOR);
			    return;
			}
			if(_perso.get_fight() == null)
		...
		break;
            ...
            msg = packet.split("\\|", 2)[1];
            long x;
			if((x = System.currentTimeMillis() - _timeLastIncarnamMsg) < Ancestra.INCARNAM_TIME)
			{
				x = (Ancestra.INCARNAM_TIME  - x)/1000;//Chat antiflood
	...
	private void Game_onLeftFight(String packet)
	{
		...
		if(_perso.get_fight() == null)return;
		_perso.get_fight().playerPass(_perso); // Passe tour avant de kick pour eviter bug
	...
	private void game_tryCastSpell(String packet)
	{
		try
		{
			...
			if(_perso.get_fight() != null)
			{
				...
				if(SS == null) {
					SocketManager.GAME_SEND_GA_CLEAR_PACKET_TO_FIGHT(_perso.get_fight(), 7);
					SocketManager.GAME_SEND_Im_PACKET(_perso, "1169");
					SocketManager.GAME_SEND_GAF_PACKET_TO_FIGHT(_perso.get_fight(), 7, 0,_perso.get_GUID());
					return;
				}
