...
public class Formulas {
	...
	public static int getTacleChance(Fighter tacleador, Fighter tacleado) {
		int agiTR = tacleador.getTotalStats().getEffect(119);
		int agiT = tacleado.getTotalStats().getEffect(119);
		int a = agiTR + 25;
		int b = agiTR + agiT + 50;
		if (b <= 0)
			b = 1;
	...
	/*
	 * public static int getTacleChance(Fighter tacleur, ArrayList<Fighter>
	 * tacle) { int agiTR =
	 * tacleur.getTotalStats().getEffect(Constants.STATS_ADD_AGIL); int agiT =
	 * 0; for(Fighter T : tacle) agiT +=
	 * T.getTotalStats().getEffect(Constants.STATS_ADD_AGIL); return (int)(300 *
	 * agiTR + 25) / (agiTR + agiT + 50) - 100; }
	 */
	...
	public static int calculFinalDommage(Fight fight, Fighter caster,
		...
		if (!isHeal) {
			...
			perdomC = caster.getTotalStats().getEffect(
					Constants.STATS_ADD_PERDOM);
			multiplier = caster.getTotalStats().getEffect(
					Constants.STATS_MULTIPLY_DOMMAGE);
		} else {
		...
		case Constants.ELEMENT_NEUTRE:// neutre
			...
			respT = target.getTotalStats()
					.getEffect(Constants.STATS_ADD_RP_NEU);
			if (caster.getPersonnage() != null)// Si c'est un joueur
			{
				respT += target.getTotalStats().getEffect(
						Constants.STATS_ADD_RP_PVP_NEU);
				resfT += target.getTotalStats().getEffect(
		...
		case Constants.ELEMENT_TERRE:// force
			...
			respT = target.getTotalStats()
					.getEffect(Constants.STATS_ADD_RP_TER);
			if (caster.getPersonnage() != null)// Si c'est un joueur
			{
				respT += target.getTotalStats().getEffect(
		...
		case Constants.ELEMENT_EAU:// chance
			...
			respT = target.getTotalStats()
					.getEffect(Constants.STATS_ADD_RP_EAU);
			if (caster.getPersonnage() != null)// Si c'est un joueur
			{
				respT += target.getTotalStats().getEffect(
		...
		case Constants.ELEMENT_FEU:// intell
			...
			respT = target.getTotalStats()
					.getEffect(Constants.STATS_ADD_RP_FEU);
			if (caster.getPersonnage() != null)// Si c'est un joueur
			{
				respT += target.getTotalStats().getEffect(
		...
		case Constants.ELEMENT_AIR:// agilit�
			...
			respT = target.getTotalStats()
					.getEffect(Constants.STATS_ADD_RP_AIR);
			if (caster.getPersonnage() != null)// Si c'est un joueur
			{
				respT += target.getTotalStats().getEffect(
		...
		if (caster.getPersonnage() != null && isCaC) {
			int ArmeType = caster.getPersonnage().getObjetByPos(1)
					.getTemplate().getType();
		...
		num = a * (jet * ((100 + statC + perdomC + (multiplier * 100)) / 100))
				+ domC;// d�gats bruts
		...
		if (spellid != -1) {
			 ...
			 */
			case 66:
				statC = caster.getTotalStats().getEffect(
						Constants.STATS_ADD_AGIL);
				num = (jet * ((100 + statC + perdomC + (multiplier * 100)) / 100))
						+ domC;
		...
		// Renvoie
		int renvoie = target.getTotalStatsLessBuff().getEffect(
				Constants.STATS_RETDOM);
		if (renvoie > 0 && !isHeal) {
			if (renvoie > num)
				renvoie = (int) num;
			num -= renvoie;
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 107, "-1",
					target.getGUID() + "," + renvoie);
			...
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 100,
					caster.getGUID() + "", caster.getGUID() + ",-" + renvoie);
		}
		...
		if (!isHeal)
			if (armor > 0)
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 105,
						caster.getGUID() + "", target.getGUID() + "," + armor);
		// d�gats finaux
		if (num < 1)
			num = 0;
		...
		if (caster.getPersonnage() == null && !caster.isPerco()) {
			if (caster.getMob().getTemplate().getID() == 116)// Sacrifi� Dommage
																// = PDV*2
			{
		...
		//supression de la perte
		else {
			// Perte de 10% des PDV MAX par points de degat 10 PDV = 1PDV max en
			// moins
	...
	public static int calculZaapCost(Carte map1, Carte map2) {
		return (int) (10 * (Math.abs(map2.getX() - map1.getX())
	...
	private static int getArmorResist(Fighter target, int statID) {
		...
		for (SpellEffect SE : target.getBuffsByEffectID(265)) {
			...
			case 1:// Armure incandescente
				...
				if (statID != Constants.ELEMENT_FEU)
					continue;
			...
			case 6:// Armure Terrestre
				...
				if (statID != Constants.ELEMENT_TERRE
						&& statID != Constants.ELEMENT_NEUTRE)
					continue;
				// Les stats du f�ca sont prises en compte
				fighter = SE.getCaster();
				break;
			case 14:// Armure Venteuse
				...
				if (statID != Constants.ELEMENT_AIR)
					continue;
				// Les stats du f�ca sont prises en compte
				fighter = SE.getCaster();
				break;
			case 18:// Armure aqueuse
				...
				if (statID != Constants.ELEMENT_EAU)
					continue;
				// Les stats du f�ca sont prises en compte
				fighter = SE.getCaster();
				break;

			default:// Dans les autres cas on prend les stats de la cible et on
					// ignore l'element de l'attaque
				fighter = target;
				break;
			}
			int intell = fighter.getTotalStats().getEffect(
					Constants.STATS_ADD_INTE);
			int carac = 0;
			switch (statID) {
			case Constants.ELEMENT_AIR:
				carac = fighter.getTotalStats().getEffect(
						Constants.STATS_ADD_AGIL);
				break;
			...
			int a = value * (100 + (int) (intell / 2) + (int) (carac / 2))
					/ 100;
			armor += a;
		}
		for (SpellEffect SE : target.getBuffsByEffectID(105)) {
			int intell = target.getTotalStats().getEffect(
					Constants.STATS_ADD_INTE);
			int carac = 0;
			switch (statID) {
			case Constants.ELEMENT_AIR:
				carac = target.getTotalStats().getEffect(
						Constants.STATS_ADD_AGIL);
				break;
			case Constants.ELEMENT_FEU:
				carac = target.getTotalStats().getEffect(
	...
	public static int getPointsLost(char z, int value, Fighter caster,
			Fighter target) {
		float esquiveC = z == 'a' ? caster.getTotalStats().getEffect(
				Constants.STATS_ADD_AFLEE) : caster.getTotalStats().getEffect(
				Constants.STATS_ADD_MFLEE);
		float esquiveT = z == 'a' ? target.getTotalStats().getEffect(
				Constants.STATS_ADD_AFLEE) : target.getTotalStats().getEffect(
				Constants.STATS_ADD_MFLEE);
		float ptsMax = z == 'a' ? target.getTotalStatsLessBuff().getEffect(
				Constants.STATS_ADD_PA) : target.getTotalStatsLessBuff()
		...
		for (int i = 0; i < value; i++) {
			if (ptsMax == 0 && target.getMob() != null) {
				ptsMax = z == 'a' ? target.getMob().getPA() : target.getMob()
						.getPM();
			}
	...
	public static long getXpWinPerco(Percepteur perco,
			ArrayList<Fighter> winners, ArrayList<Fighter> loosers, long groupXP) {
	...
	public static long getXpWinPvm2(Fighter perso, ArrayList<Fighter> winners,
			ArrayList<Fighter> loosers, long groupXP, int star) {
		if (perso.getPersonnage() == null)
			return 0;
		if (winners.contains(perso))// Si winner
		{
			float sag = perso.getTotalStats().getEffect(
					Constants.STATS_ADD_SAGE);
			float coef = (sag + 100) / 100;
			int taux = Ancestra.XP_PVM;
			long xpWin = 0;
			int lvlmax = 0;
	...
	public static long getXpWinPvP(Fighter perso, ArrayList<Fighter> winners,
			ArrayList<Fighter> looser) {
		if (perso.getPersonnage() == null)
			return 0;
		if (winners.contains(perso.getGUID()))// Si winner
		{
			...
			long xpWin = (long) ((rapport
					* getXpNeededAtLevel(perso.getPersonnage().get_lvl()) / 100) * taux);
			return xpWin;
		}
		return 0;
	}
	...
	public static long getGuildXpWin(Fighter perso, AtomicReference<Long> xpWin) {
		...
		double xp = (double) xpWin.get(), Lvl = perso.get_lvl(), LvlGuild = perso
				.getPersonnage().get_guild().get_lvl(), pXpGive = (double) gm
				.getPXpGive() / 100;

		double maxP = xp * pXpGive * 0.10; // Le maximum donn� � la guilde est
											// 10% du montant pr�lev� sur l'xp
											// du combat
		double diff = Math.abs(Lvl - LvlGuild); // Calcul l'�cart entre le
												// niveau du personnage et le
												// niveau de la guilde
		double toGuild;
		if (diff >= 70) {
			toGuild = maxP * 0.10; // Si l'�cart entre les deux level est de 70
									// ou plus, l'experience donn�e a la guilde
									// est de 10% la valeur maximum de don
	...
	public static long getMountXpWin(Fighter perso, AtomicReference<Long> xpWin) {
		...
		int diff = Math.abs(perso.get_lvl()
				- perso.getPersonnage().getMount().get_level());
	...
	public static int calculHonorWin(ArrayList<Fighter> winners,
			ArrayList<Fighter> loosers, Fighter F) {
		...
		if (totalLevelWin - totalLevelLoose > Ancestra.LVL_PVP)
			return 0;

		int base = (int) (100 * (float) (totalGradeLoose / totalGradeWin))
				/ winners.size();
	...
	public static int ChanceFM(int pesoBaseObj, int pesoObjActual,
		...
		if (diferencia < 1)
			diferencia = 1;
		int a = (int) (((double) (pesoBaseObj + diferencia) * coef * ((double) Ancestra.PORC_FM /*
																								 * +
																								 * 49D
																								 */)) / 50D);
	...
	public static int getTraqueXP(int lvl) {
		if (lvl < 50)
			return 10000 * Ancestra.XP_PVM;
		if (lvl < 60)
			return 65000 * Ancestra.XP_PVM;
		if (lvl < 70)
			return 90000 * Ancestra.XP_PVM;
		if (lvl < 80)
			return 120000 * Ancestra.XP_PVM;
		if (lvl < 90)
			return 160000 * Ancestra.XP_PVM;
		if (lvl < 100)
			return 210000 * Ancestra.XP_PVM;
		if (lvl < 110)
			return 270000 * Ancestra.XP_PVM;
		if (lvl < 120)
			return 350000 * Ancestra.XP_PVM;
		if (lvl < 130)
			return 440000 * Ancestra.XP_PVM;
		if (lvl < 140)
			return 540000 * Ancestra.XP_PVM;
		if (lvl < 150)
			return 650000 * Ancestra.XP_PVM;
		if (lvl < 155)
			return 760000 * Ancestra.XP_PVM;
		if (lvl < 160)
			return 880000 * Ancestra.XP_PVM;
		if (lvl < 165)
			return 1000000 * Ancestra.XP_PVM;
		if (lvl < 170)
			return 1130000 * Ancestra.XP_PVM;
		if (lvl < 175)
			return 1300000 * Ancestra.XP_PVM;
		if (lvl < 180)
			return 1500000 * Ancestra.XP_PVM;
		if (lvl < 185)
			return 1700000 * Ancestra.XP_PVM;
		if (lvl < 190)
			return 2000000 * Ancestra.XP_PVM;
		if (lvl < 195)
			return 2500000 * Ancestra.XP_PVM;
		if (lvl < 200)
			return 3000000 * Ancestra.XP_PVM;
		return 0;
	}
	...
	public static int calculFinalCura(Fighter curador, int rango) {
		int statC = curador.getTotalStats().getEffect(Constants.STATS_ADD_INTE);
		int curas = curador.getTotalStats().getEffect(Constants.STATS_ADD_SOIN);
		if (statC < 0)
			statC = 0;
		int adic = 200;
		if (curador.getPersonnage() != null) {
			if (curador.getPersonnage().get_classe() == 7)
				adic = 100;
		}
		return (int) (rango * ((100 + statC) / adic) + curas);
	}

	public static int getKamasWinPVP(Fighter i, ArrayList<Fighter> winners,
			int maxk, int mink) {
		maxk++;
		int rkamas = (int) (Math.random() * (10000 - 50000)) + 10000;
		return rkamas * Ancestra.KAMAS;
	}

	public static long XPDefie(objects.Fight.Fighter perso,
			@SuppressWarnings("rawtypes") ArrayList winners,
			@SuppressWarnings("rawtypes") ArrayList looser) {
		int lvlLoosers = 0;
		for (@SuppressWarnings("rawtypes")
		Iterator iterator = looser.iterator(); iterator.hasNext();) {
			objects.Fight.Fighter entry = (objects.Fight.Fighter) iterator
					.next();
			lvlLoosers += entry.get_lvl();
		}
		...
		Iterator iterator1 = winners.iterator(); iterator1.hasNext();) {
			objects.Fight.Fighter entry = (objects.Fight.Fighter) iterator1
					.next();
			lvlWinners += entry.get_lvl();
		}
		...
		if (rapport >= 1.0F)
			malus = 1;
		long xpWin = (long) (((rapport * (float) getXpNeededAtLevel(perso
				.getPersonnage().get_lvl())) / 10F) * (float) taux)
				/ (long) malus;
		return xpWin;
	}
