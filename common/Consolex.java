...
public class Consolex implements Runnable {
	...
	public void evalCommand(String command) {
		...
		if (fct.equals("SAVE")) {
			...
			SocketManager.GAME_SEND_MESSAGE_TO_ALL(PrefixConsole + announce,
			...
			String mess = "===========\n" + Ancestra.makeHeader() + "UpTime: "
					+ jour + "d " + hour + "h " + min + "m " + sec + "s\n"
					+ "En Ligne: " + Ancestra.gameServer.getPlayerNumber()
	...
	public enum ConsoleColorEnum {
		// FONT
		BOLD(1), UNDERLINE(4), BLACK(30), RED(31), GREEN(32), YELLOW(33), BLUE(
				34), WHITE(37),
		// SPECIAL
		RESET(0);
		private int color;
	...
	public static void println(String msg, ConsoleColorEnum color) {
		AnsiConsole.out.println(new StringBuilder().append("\033[")
				.append(color.get()).append("m").append(msg).append("\033[")
				.append(ConsoleColorEnum.RESET.get()).append("m").toString());
	}

	public static void print(String msg, ConsoleColorEnum color) {
		AnsiConsole.out.print(new StringBuilder().append("\033[")
				.append(color.get()).append("m").append(msg).append("\033[")
				.append(ConsoleColorEnum.RESET.get()).append("m").toString());
	}
}
