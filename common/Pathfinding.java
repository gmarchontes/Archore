...
public class Pathfinding {
	...
	public static int isValidPath(Carte map, int cellID,
			AtomicReference<String> pathRef, Fight fight) {
		synchronized (_nSteps) {
			...
			for (int i = 0; i < path.length(); i += 3) {
				...
				int dirCaseID = CryptManager.cellCode_To_ID(SmallPath
						.substring(1));
				_nSteps = 0;
				// Si en combat et Si Pas d�but du path, on v�rifie tacle
				if (fight != null && i != 0
						&& getEnemyFighterArround(newPos, map, fight) != null) {
					pathRef.set(newPath);
					return Steps;
				}
				...
				if (fight != null && i != 0) {
					for (Piege p : fight.get_traps()) {
						int dist = getDistanceBetween(map,
								p.get_cell().getID(), newPos);
						if (dist <= p.get_size()) {
							// on arrete le d�placement sur la 1ere case du
							// piege
							pathRef.set(newPath);
							return Steps;
						}
	...
	public static ArrayList<Fighter> getEnemyFighterArround(int cellID,
			Carte map, Fight fight) {
		char[] dirs = { 'b', 'd', 'f', 'h' };
		ArrayList<Fighter> enemy = new ArrayList<Fighter>();

		for (char dir : dirs) {
			Fighter f = map.getCase(
					GetCaseIDFromDirrection(cellID, dir, map, false))
					.getFirstFighter();
			if (f != null) {
	...
	public static Fighter getEnemigoAlrededor(int celdaID, Carte mapa,
			Fight pelea) {
		char[] dirs = { 'b', 'd', 'f', 'h' };
		for (char dir : dirs) {
			Case sigCelda = mapa.getCase(GetCaseIDFromDirrection(celdaID, dir,
					mapa, false));
			if (sigCelda == null)
				continue;
	...
	public static boolean isNextTo(int cell1, int cell2) {
		if (cell1 + 14 == cell2 || cell1 + 15 == cell2 || cell1 - 14 == cell2
				|| cell1 - 15 == cell2)
			return true;
		else
			return false;
	...
	public static String ValidSinglePath(int CurrentPos, String Path,
			Carte map, Fight fight) {
		_nSteps = 0;
		...
		for (_nSteps = 1; _nSteps <= 64; _nSteps++) {
			if (GetCaseIDFromDirrection(lastPos, dir, map, fight != null) == dirCaseID) {
				...
				lastPos = GetCaseIDFromDirrection(lastPos, dir, map,
						fight != null);
			...
			if (fight != null) {
				if (getEnemyFighterArround(lastPos, map, fight) != null)// Si
																		// ennemie
																		// proche
				{
					return "stop:" + lastPos;
				...
				for (Piege p : fight.get_traps()) {
					int dist = getDistanceBetween(map, p.get_cell().getID(),
							lastPos);
					if (dist <= p.get_size()) {
	...
	public static int newCaseAfterPush(Carte map, Case CCase, Case TCase,
			int value) {
	...
	public static ArrayList<Fighter> getCiblesByZoneByWeapon(Fight fight,
			int type, Case cell, int castCellID) {
		ArrayList<Fighter> cibles = new ArrayList<Fighter>();
		char c = getDirBetweenTwoCase(castCellID, cell.getID(),
				fight.get_map(), true);
		if (c == 0) {
		...
		case Constants.ITEM_TYPE_MARTEAU:
			...
			if (f != null)
				cibles.add(f);
			Fighter g = get1StFighterOnCellFromDirection(fight.get_map(),
					castCellID, (char) (c - 1));
			if (g != null)
				cibles.add(g);// Ajoute case a gauche
			Fighter h = get1StFighterOnCellFromDirection(fight.get_map(),
					castCellID, (char) (c + 1));
			if (h != null)
		...
		case Constants.ITEM_TYPE_BATON:
			Fighter j = get1StFighterOnCellFromDirection(fight.get_map(),
					castCellID, (char) (c - 1));
			if (j != null)
				cibles.add(j);// Ajoute case a gauche
			Fighter k = get1StFighterOnCellFromDirection(fight.get_map(),
					castCellID, (char) (c + 1));
			if (k != null)
	...
	private static Fighter get1StFighterOnCellFromDirection(Carte map, int id,
			char c) {
		if (c == (char) ('a' - 1))
			c = 'h';
		if (c == (char) ('h' + 1))
			c = 'a';
		return map.getCase(GetCaseIDFromDirrection(id, c, map, false))
				.getFirstFighter();
	}

	private static Fighter getFighter2CellBefore(int CellID, char c, Carte map) {
		int new2CellID = GetCaseIDFromDirrection(
				GetCaseIDFromDirrection(CellID, c, map, false), c, map, false);
		return map.getCase(new2CellID).getFirstFighter();
	}

	public static char getDirBetweenTwoCase(int cell1ID, int cell2ID,
			Carte map, boolean Combat) {
	...
	public static ArrayList<Case> getCellListFromAreaString(Carte map,
		...
		case 'C':// Cercle
			for (int a = 0; a < taille; a++) {
				...
				for (Case aCell : cases2) {
					for (char d : dirs) {
						Case cell = map.getCase(Pathfinding
								.GetCaseIDFromDirrection(aCell.getID(), d, map,
										true));
						if (cell == null)
							continue;
		...
		case 'X':// Croix
			...
			for (char d : dirs) {
				int cID = cellID;
				for (int a = 0; a < taille; a++) {
					cases.add(map.getCase(GetCaseIDFromDirrection(cID, d, map,
							true)));
		...
		case 'L':// Ligne
			char dir = Pathfinding.getDirBetweenTwoCase(castCellID, cellID,
					map, true);
			for (int a = 0; a < taille; a++) {
				cases.add(map.getCase(GetCaseIDFromDirrection(cellID, dir, map,
						true)));
		...
		default:
			GameServer.addToLog("[FIXME]Type de port�e non reconnue: "
					+ zoneStr.charAt(0));
			break;
		}
		return cases;
	...
	public static int getNearestCellAround(Carte map, int startCell,
			int endCell, ArrayList<Case> forbidens) {
		...
		for (char d : dirs) {
			int c = Pathfinding
					.GetCaseIDFromDirrection(startCell, d, map, true);
	...
	public static ArrayList<Case> getShortestPathBetween(Carte map, int start,
		...
		while (!stop && stepNum++ <= limit) {
			int nearestCell = getNearestCellAround(map, curCase.getID(), dest,
					closeCells);
			if (nearestCell == -1) {
				...
				break;
			} else if (distMax > Pathfinding.getDistanceBetween(map,
					nearestCell, dest)) {
				curPath.add(map.getCase(dest));
				break;
			} else// on continue
	...
	public static String getShortestStringPathBetween(Carte map, int start,
			int dest, int distMax) {
		if (start == dest)
			return null;
	...
	public static ArrayList<Integer> triCellList(Fight fight, Fighter fighter,
		...
		while (copie.size() > 0) {
			dist = 100;
			for (int i : copie) {
				int d = getDistanceBetween(fight.get_map(), fighter
						.get_fightCell().getID(), i);
				if (dist > d) {
					dist = d;
					curCell = i;
	...
	public static boolean isBord1(int id) {
		int[] bords = { 1, 30, 59, 88, 117, 146, 175, 204, 233, 262, 291, 320,
				349, 378, 407, 436, 465, 15, 44, 73, 102, 131, 160, 189, 218,
				247, 276, 305, 334, 363, 392, 421, 450, 479 };
	...
	public static boolean isBord2(int id) {
		int[] bords = { 16, 45, 74, 103, 132, 161, 190, 219, 248, 277, 306,
				335, 364, 393, 422, 451, 29, 58, 87, 116, 145, 174, 203, 232,
				261, 290, 319, 348, 377, 406, 435, 464 };
		ArrayList<Integer> test = new ArrayList<Integer>();
		for (int i : bords) {
			test.add(i);
		}
	...
	public static ArrayList<Fighter> getFightersAround(int cellID, Carte map,
			Fight fight) {
		char[] dirs = { 'b', 'd', 'f', 'h' };
