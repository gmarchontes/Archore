...
public class Constants {
	...
	public static final String SERVER_MAKER = "IamNothing ";
	// Versions
	...
	public static int G_OTHDINDE = 16384; // G�rer les montures des autres
											// membres
	...
	public static int H_OCANTOPEN = 16; // Entrer impossible pour les
										// non-guildeux
	public static int C_GNOCODE = 32; // Coffre sans code pour la guilde
	public static int C_OCANTOPEN = 64; // Coffre impossible pour les
										// non-guildeux
	...
	public static final int FIGHT_TYPE_PVMA = 2;// Conquista
	...
	public static final int[] BEGIN_TURN_BUFF = { 91, 92, 93, 94, 95, 96, 97,
			98, 99, 100, 108 };
	// Buff des Armes
	public static final int[] ARMES_EFFECT_IDS = { 91, 92, 93, 94, 95, 96, 97,
			98, 99, 100, 101 };
	// Buff a ne pas booster en cas de CC
	public static final int[] NO_BOOST_CC_IDS = { 101 };
	// Invocation Statiques
	public static final int[] STATIC_INVOCATIONS = { 282, 556 };// Arbre et
																// Cawotte
																// s'tout :p

	// Verif d'Etat au lancement d'un sort {spellID,stateID}, � completer avant
	// d'activer
	public static final int[][] STATE_REQUIRED = {
			{ 699, Constants.ETAT_SAOUL }, { 690, Constants.ETAT_SAOUL } };
	// Action de M�tier {skillID,objetRecolt�,objSp�cial}
	public static final int[][] JOB_ACTION = {
			// Bucheron
			{ 101 }, { 6, 303 }, { 39, 473 }, { 40, 476 }, { 10, 460 },
			{ 141, 2357 },
			{ 139, 2358 },
			{ 37, 471 },
			{ 154, 7013 },
			...
			{ 161, 7033 },
			// P�cheur
			{ 133 }, { 128, 598, 1786 }, { 128, 1757, 1759 },
			{ 128, 1750, 1754 }, { 124, 603, 1762 }, { 124, 1782, 1790 },
			{ 124, 1844, 607 }, { 136, 2187 }, { 125, 1847, 1849 },
			{ 125, 1794, 1796 }, { 140, 1799, 1759 }, { 129, 600, 1799 },
			{ 129, 1805, 1807 }, { 126, 1779, 1792 },
			{ 130, 1784, 1788 },
			{ 127, 1801, 1803 },
			{ 131, 602, 1853 },
			// Alchi
			{ 23 }, { 68, 421 }, { 54, 428 }, { 71, 395 }, { 72, 380 },
			{ 73, 593 }, { 74, 594 },
			{ 160, 7059 },
			// Paysan
			{ 122 }, { 47 }, { 45, 289, 2018 }, { 53, 400, 2032 },
			{ 57, 533, 2036 }, { 46, 401, 2021 }, { 50, 423, 2026 },
	...
	public static TreeMap<Integer, SortStats> getStartSorts(int classID) {
		...
		switch (classID) {
		case CLASS_FECA:
			start.put(3, World.getSort(3).getStatsByLevel(1));// Attaque
																// Naturelle
			start.put(6, World.getSort(6).getStatsByLevel(1));// Armure
																// Terrestre
			start.put(17, World.getSort(17).getStatsByLevel(1));// Glyphe
																// Agressif
			break;
		case CLASS_SRAM:
			start.put(61, World.getSort(61).getStatsByLevel(1));// Sournoiserie
			start.put(72, World.getSort(72).getStatsByLevel(1));// Invisibilit�
			start.put(65, World.getSort(65).getStatsByLevel(1));// Piege
																// sournois
			break;
		case CLASS_ENIRIPSA:
			start.put(125, World.getSort(125).getStatsByLevel(1));// Mot
																	// Interdit
			start.put(123, World.getSort(123).getStatsByLevel(1));// Mot Drainant
																	
			start.put(121, World.getSort(121).getStatsByLevel(1));// Mot Curatif
			break;
		case CLASS_ECAFLIP:
			start.put(102, World.getSort(102).getStatsByLevel(1));// Pile ou
																	// Face
			start.put(103, World.getSort(103).getStatsByLevel(1));// Chance
																	// d'ecaflip
			start.put(101, World.getSort(101).getStatsByLevel(1));// Roulette
			
			break;
		case CLASS_CRA:
			start.put(161, World.getSort(161).getStatsByLevel(1));// Fleche
																	// Magique
			start.put(169, World.getSort(169).getStatsByLevel(1));// Fleche de
																	// Recul
			start.put(164, World.getSort(164).getStatsByLevel(1));// Fleche
																	// Empoisonn�e(ex
																	// Fleche
																	// chercheuse)
			break;
		...
		case CLASS_SADIDA:
			...
			start.put(200, World.getSort(200).getStatsByLevel(1));// Poison
																	// Paralysant
			start.put(193, World.getSort(193).getStatsByLevel(1));// La
																	// bloqueuse
			break;
		case CLASS_OSAMODAS:
			start.put(34, World.getSort(34).getStatsByLevel(1));// Invocation de
																// tofu
			start.put(21, World.getSort(21).getStatsByLevel(1));// Griffe
																// Spectrale
		...
		case CLASS_PANDAWA:
			...
			start.put(692, World.getSort(692).getStatsByLevel(1));// Gueule de
																	// bois
			start.put(687, World.getSort(687).getStatsByLevel(1));// Poing
																	// enflamm�
			break;
		case CLASS_ENUTROF:
			start.put(51, World.getSort(51).getStatsByLevel(1));// Lancer de
																// Piece
			start.put(43, World.getSort(43).getStatsByLevel(1));// Lancer de
																// Pelle
			start.put(41, World.getSort(41).getStatsByLevel(1));// Sac anim�
			break;
		case CLASS_SACRIEUR:
			start.put(432, World.getSort(432).getStatsByLevel(1));// Pied du
																	// Sacrieur
			start.put(431, World.getSort(431).getStatsByLevel(1));// Chatiment
																	// Forc�
	...
	public static void onLevelUpSpells(Personnage perso, int lvl) {
		...
		case CLASS_FECA:
			...
			if (lvl == 200)
				perso.learnSpell(1901, 1, true, false);// Invocation de Dopeul
														// F�ca
			break;
		...
		case CLASS_OSAMODAS:
			...
			if (lvl == 100)
				perso.learnSpell(31, 1, true, false);// Invocation de Dragonnet
														// Rouge
			if (lvl == 200)
				perso.learnSpell(1902, 1, true, false);// Invocation de Dopeul
														// Osamodas
			break;
		...
		case CLASS_ENUTROF:
			...
			if (lvl == 200)
				perso.learnSpell(1903, 1, true, false);// Invocation de Dopeul
														// Enutrof
			break;
		...
		case CLASS_SRAM:
			...
			if (lvl == 200)
				perso.learnSpell(1904, 1, true, false);// Invocation de Dopeul
														// Sram
			break;
		...
		case CLASS_XELOR:
			...
			if (lvl == 200)
				perso.learnSpell(1905, 1, true, false);// Invocation de Dopeul
														// X�lor
			break;
		...
		case CLASS_ECAFLIP:
			...
			if (lvl == 21)
				perso.learnSpell(105, 1, true, false);// Bond
			if (lvl == 26)
			...
			if (lvl == 200)
				perso.learnSpell(1906, 1, true, false);// Invocation de Dopeul
														// Ecaflip
			break;
		...
		case CLASS_ENIRIPSA:
			...
			if (lvl == 17)
				perso.learnSpell(128, 1, true, false);// Frayeur
			if (lvl == 21)
			...
			if (lvl == 200)
				perso.learnSpell(1907, 1, true, false);// Invocation de Dopeul
														// Eniripsa
			break;
		...
		case CLASS_IOP:
			...
			if (lvl == 200)
				perso.learnSpell(1908, 1, true, false);// Invocation de Dopeul
														// Iop
			break;
		...
		case CLASS_CRA:
			...
			if (lvl == 200)
				perso.learnSpell(1909, 1, true, false);// Invocation de Dopeul
														// Cra
			break;
		...
		case CLASS_SADIDA:
			...
			if (lvl == 21)
				perso.learnSpell(189, 1, true, false);// Invocation de la
														// Sacrifi�e
			if (lvl == 26)
				perso.learnSpell(181, 1, true, false);// Tremblement
			if (lvl == 31)
				perso.learnSpell(199, 1, true, false);// Connaissance des
														// Poup�es
			if (lvl == 36)
			...
			if (lvl == 54)
				perso.learnSpell(190, 1, true, false);// Invocation de la
														// Gonflable
			if (lvl == 60)
			...
			if (lvl == 100)
				perso.learnSpell(187, 1, true, false);// Invocation de la
														// Surpuissante
			if (lvl == 200)
				perso.learnSpell(1910, 1, true, false);// Invocation de Dopeul
														// Sadida
			break;
		...
		case CLASS_SACRIEUR:
			...
			if (lvl == 200)
				perso.learnSpell(1911, 1, true, false);// Invocation de Dopeul
														// Sacrieur
			break;
		...
		case CLASS_PANDAWA:
			...
			if (lvl == 13)
				perso.learnSpell(688, 1, true, false);// Vuln�rabilit�
														// Incandescente
			if (lvl == 17)
			...
			if (lvl == 200)
				perso.learnSpell(1912, 1, true, false);// Invocation de Dopeul
														// Pandawa
			break;
		case CLASS_ZOBAL:
			if (lvl == 13)
	...
	public static ArrayList<JobAction> getPosActionsToJob(int tID, int lvl) {
		...
		case JOB_BIJOUTIER:
			// Faire Anneau
			list.add(new JobAction(11, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			// Faire Amullette
			list.add(new JobAction(12, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			break;

		case JOB_TAILLEUR:
			// Faire Sac
			list.add(new JobAction(64, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			// Faire Cape
			list.add(new JobAction(123, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			// Faire Chapeau
			list.add(new JobAction(63, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			break;

		case JOB_F_BOUCLIER:
			// Forger Bouclier
			list.add(new JobAction(156, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			break;

		case JOB_BRICOLEUR:
			// Faire clef
			list.add(new JobAction(171, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			// Faire objet brico
			list.add(new JobAction(182, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			break;

		case JOB_CORDONIER:
			// Faire botte
			list.add(new JobAction(13, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			// Faire ceinture
			list.add(new JobAction(14, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			break;

		case JOB_S_ARC:
			// Sculter Arc
			list.add(new JobAction(17, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			// ReSculter Arc
		...
		case JOB_S_BATON:
			// Sculter Baton
			list.add(new JobAction(147, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			// ReSculter Baton
			list.add(new JobAction(148, 3, 0, true, getChanceForMaxCase(lvl),
					-1));
			break;

		case JOB_S_BAGUETTE:
			// Sculter Baguette
			list.add(new JobAction(149, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			// ReSculter Baguette
		...
		case JOB_F_EPEE:
			// Forger Ep�e
			list.add(new JobAction(20, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			// Reforger Ep�e
			list.add(new JobAction(146, 3, 0, true, getChanceForMaxCase(lvl),
					-1));
			break;

		case JOB_F_DAGUE:
			// Forger Dague
			list.add(new JobAction(142, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			// Reforger Dague
		...
		case JOB_F_MARTEAU:
			// Forger Marteau
			list.add(new JobAction(19, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			// Reforger Marteau
			list.add(new JobAction(144, 3, 0, true, getChanceForMaxCase(lvl),
					-1));
			break;

		case JOB_F_PELLE:
			// Forger Pelle
			list.add(new JobAction(21, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			// Reforger Pelle
			list.add(new JobAction(146, 3, 0, true, getChanceForMaxCase(lvl),
					-1));
		...
		case JOB_F_HACHES:
			// Forger Hache
			list.add(new JobAction(65, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			// Reforger Hache
			list.add(new JobAction(143, 3, 0, true, getChanceForMaxCase(lvl),
					-1));
			break;
		...
		case JOB_CHASSEUR:
			// Pr�parer une Viande
			list.add(new JobAction(134, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			break;

		case JOB_BOUCHER:
			// Pr�parer une Viande
			list.add(new JobAction(132, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			break;

		case JOB_POISSONNIER:
			// Preparer un Poisson
			list.add(new JobAction(135, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			break;

		case JOB_BOULANGER:
			// Cuir le Pain
			list.add(new JobAction(27, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			// Faire des Bonbons
		...
		case JOB_MINEUR:
			if (lvl > 99) {
				// Miner Dolomite
				list.add(new JobAction(161, -19 + dropWin, -18 + dropWin,
						false, 12000 - timeWin, 60));
			}
			if (lvl > 79) {
				// Miner Or
				list.add(new JobAction(30, -15 + dropWin, -14 + dropWin, false,
						12000 - timeWin, 55));
			}
			if (lvl > 69) {
				// Miner Bauxite
				list.add(new JobAction(31, -13 + dropWin, -12 + dropWin, false,
						12000 - timeWin, 50));
			}
			if (lvl > 59) {
				// Miner Argent
				list.add(new JobAction(29, -11 + dropWin, -10 + dropWin, false,
						12000 - timeWin, 40));
			}
			if (lvl > 49) {
				// Miner Etain
				list.add(new JobAction(55, -9 + dropWin, -8 + dropWin, false,
						12000 - timeWin, 35));
				// Miner Silicate
				list.add(new JobAction(162, -9 + dropWin, -8 + dropWin, false,
						12000 - timeWin, 35));
			}
			if (lvl > 39) {
				// Miner Mangan�se
				list.add(new JobAction(56, -7 + dropWin, -6 + dropWin, false,
						12000 - timeWin, 30));
			}
			if (lvl > 29) {
				// Miner Kobalte
				list.add(new JobAction(28, -5 + dropWin, -4 + dropWin, false,
						12000 - timeWin, 25));
			}
			if (lvl > 19) {
				// Miner Bronze
				list.add(new JobAction(26, -3 + dropWin, -2 + dropWin, false,
						12000 - timeWin, 20));
			}
			if (lvl > 9) {
				// Miner Cuivre
				list.add(new JobAction(25, -1 + dropWin, 0 + dropWin, false,
						12000 - timeWin, 15));
			}
			// Miner Fer
			list.add(new JobAction(24, 1 + dropWin, 2 + dropWin, false,
					12000 - timeWin, 10));
			// Fondre
			list.add(new JobAction(32, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			// Polir
			list.add(new JobAction(48, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			break;
		...
		case JOB_PECHEUR:
			...
			// Vider
			list.add(new JobAction(133, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			break;
		...
		case JOB_ALCHIMISTE:
			if (lvl > 49) {
				...
				list.add(new JobAction(160, -9 + dropWin, -8 + dropWin, false,
						12000 - timeWin, 35));
				// Cueillir Edelweiss
				list.add(new JobAction(74, -9 + dropWin, -8 + dropWin, false,
						12000 - timeWin, 35));
			}
			if (lvl > 39) {
				// Cueillir Orchid�e
				list.add(new JobAction(73, -7 + dropWin, -6 + dropWin, false,
						12000 - timeWin, 30));
			}
			if (lvl > 29) {
				// Cueillir Menthe
				list.add(new JobAction(72, -5 + dropWin, -4 + dropWin, false,
						12000 - timeWin, 25));
			}
			if (lvl > 19) {
				// Cueillir Tr�fle
				list.add(new JobAction(71, -3 + dropWin, -2 + dropWin, false,
						12000 - timeWin, 20));
			}
			if (lvl > 9) {
				// Cueillir Chanvre
				list.add(new JobAction(54, -1 + dropWin, 0 + dropWin, false,
						12000 - timeWin, 15));
			}
			// Cueillir Lin
			list.add(new JobAction(68, 1 + dropWin, 2 + dropWin, false,
					12000 - timeWin, 10));
			// Fabriquer une Potion
			list.add(new JobAction(23, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			break;
		...
		case JOB_BUCHERON:
			if (lvl > 99) {
				// Couper Bambou Sacr�
				list.add(new JobAction(158, -19 + dropWin, -18 + dropWin,
						false, 12000 - timeWin, 75));
			}
			if (lvl > 89) {
				// Couper Orme
				list.add(new JobAction(35, -17 + dropWin, -16 + dropWin, false,
						12000 - timeWin, 70));
			}
			if (lvl > 79) {
				// Couper Charme
				list.add(new JobAction(38, -15 + dropWin, -14 + dropWin, false,
						12000 - timeWin, 65));
				// Couper Bambou Sombre
				list.add(new JobAction(155, -15 + dropWin, -14 + dropWin,
						false, 12000 - timeWin, 65));
			}
			if (lvl > 74) {
				// Couper Kalyptus
				list.add(new JobAction(174, -14 + dropWin, -13 + dropWin,
						false, 12000 - timeWin, 55));
			}
			if (lvl > 69) {
				// Couper Eb�ne
				list.add(new JobAction(34, -13 + dropWin, -12 + dropWin, false,
						12000 - timeWin, 50));
			}
			if (lvl > 59) {
				// Couper Merisier
				list.add(new JobAction(41, -11 + dropWin, -10 + dropWin, false,
						12000 - timeWin, 45));
			}
			if (lvl > 49) {
				// Couper If
				list.add(new JobAction(33, -9 + dropWin, -8 + dropWin, false,
						12000 - timeWin, 40));
				// Couper Bambou
				list.add(new JobAction(154, -9 + dropWin, -8 + dropWin, false,
						12000 - timeWin, 40));
			}
			if (lvl > 39) {
				// Couper Erable
				list.add(new JobAction(37, -7 + dropWin, -6 + dropWin, false,
						12000 - timeWin, 35));
			}
			if (lvl > 34) {
				// Couper Bombu
				list.add(new JobAction(139, -6 + dropWin, -5 + dropWin, false,
						12000 - timeWin, 30));
				// Couper Oliviolet
				list.add(new JobAction(141, -6 + dropWin, -5 + dropWin, false,
						12000 - timeWin, 30));
			}
			if (lvl > 29) {
				// Couper Ch�ne
				list.add(new JobAction(10, -5 + dropWin, -4 + dropWin, false,
						12000 - timeWin, 25));
			}
			if (lvl > 19) {
				// Couper Noyer
				list.add(new JobAction(40, -3 + dropWin, -2 + dropWin, false,
						12000 - timeWin, 20));
			}
			if (lvl > 9) {
				// Couper Ch�taignier
				list.add(new JobAction(39, -1 + dropWin, 0 + dropWin, false,
						12000 - timeWin, 15));
			}
			// Couper Fr�ne
			list.add(new JobAction(6, 1 + dropWin, 2 + dropWin, false,
					12000 - timeWin, 10));
			// Scie
			list.add(new JobAction(101, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
			break;
		...
		case JOB_PAYSAN:
			if (lvl > 69) {
				// Faucher Chanvre
				list.add(new JobAction(54, -13 + dropWin, -12 + dropWin, false,
						12000 - timeWin, 45));
			}
			if (lvl > 59) {
				// Faucher Malt
				list.add(new JobAction(58, -11 + dropWin, -10 + dropWin, false,
						12000 - timeWin, 40));
			}
			if (lvl > 49) {
				// Faucher Riz
				list.add(new JobAction(159, -9 + dropWin, -8 + dropWin, false,
						12000 - timeWin, 35));
				// Faucher Seigle
				list.add(new JobAction(52, -9 + dropWin, -8 + dropWin, false,
						12000 - timeWin, 35));
			}
			if (lvl > 39) {
				// Faucher Lin
				list.add(new JobAction(50, -7 + dropWin, -6 + dropWin, false,
						12000 - timeWin, 30));
			}
			if (lvl > 29) {
				// Faucher Houblon
				list.add(new JobAction(46, -5 + dropWin, -4 + dropWin, false,
						12000 - timeWin, 25));
			}
			if (lvl > 19) {
				// Faucher Avoine
				list.add(new JobAction(57, -3 + dropWin, -2 + dropWin, false,
						12000 - timeWin, 20));
			}
			if (lvl > 9) {
				// Faucher Orge
				list.add(new JobAction(53, -1 + dropWin, 0 + dropWin, false,
						12000 - timeWin, 15));
			}
			// Faucher bl�
			list.add(new JobAction(45, 1 + dropWin, 2 + dropWin, false,
					12000 - timeWin, 10));
			// Moudre
			list.add(new JobAction(47, getTotalCaseByJobLevel(lvl), 0, true,
					getChanceForMaxCase(lvl), -1));
	...
	public static int getObjectByJobSkill(int skID, boolean special) {
		for (int v = 0; v < JOB_ACTION.length; v++)
			if (JOB_ACTION[v][0] == skID)
				return (JOB_ACTION[v].length > 1 && special ? JOB_ACTION[v][2]
						: JOB_ACTION[v][1]);
		return -1;
	}
	...
	public static Stats getMountStats(int color, int lvl) {
		...
		case 3:
			...
			stats.addOneStat(STATS_ADD_AGIL, (int) (lvl / 1.25));// 100/1.25 =
																	// 80
			break;
		// Rousse |
		case 10:
		...
		case 18:
			...
			stats.addOneStat(STATS_ADD_SAGE, (int) (lvl / 2.50)); // 100/2.50 =
																	// 40
			break;
		// Rousse-Amande
		case 38:
		...
		// Rousse-Indigo
		case 62:
			stats.addOneStat(STATS_ADD_VITA, (int) (lvl * 1.50)); // 100*1.50 =
																	// 150
	...
	public static void applyPlotIOAction(Personnage perso, int mID, int cID) {
		...
		case 2196:// Cr�ation de guilde
			...
			break;
		default:
			GameServer.addToLog("PlotIOAction non gere pour la map " + mID
					+ " cell=" + cID);
		}
	}
	...
	public static int getNearCellidUnused(Personnage _perso) {
		...
		int cellRight = 0;
		int cellLeft = 0;
		if (_perso.get_curCarte().getSubArea().get_area().get_id() == 7
				|| _perso.get_curCarte().getSubArea().get_area().get_id() == 11) {
			cellFront = 19;
			cellBack = -19;
			cellRight = 18;
			cellLeft = -18;
		...
		if (_perso.get_curCarte()
				.getCase(_perso.get_curCell().getID() + cellFront)
				.getDroppedItem() == null
				&& _perso.get_curCarte().GetCases()
						.get(_perso.get_curCell().getID() + cellFront)
						.getPersos().isEmpty()
				&& _perso.get_curCarte().GetCases()
						.get(_perso.get_curCell().getID() + cellFront)
						.isWalkable(false)) {
			return cellFront;
		} else if (_perso.get_curCarte()
				.getCase(_perso.get_curCell().getID() - cellBack)
				.getDroppedItem() == null
				&& _perso.get_curCarte().GetCases()
						.get(_perso.get_curCell().getID() - cellBack)
						.getPersos().isEmpty()
				&& _perso.get_curCarte().GetCases()
						.get(_perso.get_curCell().getID() - cellBack)
						.isWalkable(false)) {
			return cellBack;
				...
				.getCase(_perso.get_curCell().getID() + cellRight)
				.getDroppedItem() == null
				&& _perso.get_curCarte().GetCases()
						.get(_perso.get_curCell().getID() + cellRight)
						.getPersos().isEmpty()
				&& _perso.get_curCarte().GetCases()
						.get(_perso.get_curCell().getID() + cellRight)
						.isWalkable(false)) {
			return cellRight;
				...
				.getCase(_perso.get_curCell().getID() - cellLeft)
				.getDroppedItem() == null
				&& _perso.get_curCarte().GetCases()
						.get(_perso.get_curCell().getID() - cellLeft)
						.getPersos().isEmpty()
				&& _perso.get_curCarte().GetCases()
						.get(_perso.get_curCell().getID() - cellLeft)
						.isWalkable(false)) {
	...
	public static boolean isValidPlaceForItem(ObjTemplate template, int place) {
		...
		// Barre d'objets TODO : Normalement le client bloque les items
		// interdits
	...
	public static int getCertificatDopeul(int IDmob) {
		switch (IDmob) {
		case 168:
			return 10289;
		case 165:
	...
	public static int getIDdoplonByMapID(int IDmap) {
		...
		case 1470: // Enutrof
			return 10305;
		case 1461: // Ecaflip (map de dessous, puisque l'autre n'est pas dans
					// l'emu)
			return 10303;
		case 6949: // Sacrieur
			return 10310;
		case 1556: // Cra (map en bas dans la maison, celle dans haut n'est pas
					// dans l'emu)
			return 10302;
		case 1549: // Iop
			return 10307;
		case 1469: // Xel
			return 10313;
		case 487: // Eniripsa (dehors, puisque l'int�rieur n'est pas pr�sent
					// dans l'emu)
			return 10304;
	...
	public static String accionesOficio(int oficio) {
		switch (oficio) {
		case 62:
			return "163;164";
		case 63:
	...
	public static int getValorStatRuna(int statID) {
		int multi = 1;
		if (statID == 118 || statID == 126 || statID == 125 || statID == 119
				|| statID == 123 || statID == 158 || statID == 174)// Fuerza,
		// iniciativa
		{
			multi = 1;
		} else if (statID == 138 || statID == 666 || statID == 226
				|| statID == 220)// Da�os %,Reenvio de da�os,Trampas %
		{
			multi = 2;
		} else if (statID == 124 || statID == 176)// Sabiduria, prospeccion
		{
			multi = 3;
				...
				|| statID == 243 || statID == 244)// Resistencia + fuego,
													// aire, tierra, agua,
													// neutral
		{
			multi = 4;
		} else if (statID == 210 || statID == 211 || statID == 212
				|| statID == 213 || statID == 214)// Resistencia % fuego,
													// aire, tierra, agua,
													// neutral
		{
			multi = 5;
