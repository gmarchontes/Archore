...
public class World {
	...
	private static int nextHdvID; // Contient le derniere ID utilis� pour cr�e
									// un HDV, pour obtenir un ID non utilis� il
									// faut imp�rativement l'incr�menter
	private static int nextLigneID; // Contient le derniere ID utilis� pour cr�e
	...
	private static int nextObjetID; // Contient le derniere ID utilis� pour cr�e
									// un Objet
	...
	public static class Area {
		...
		public Area(int id, int superArea, String name) {
			...
			// Si le continent n'est pas encore cr�er, on le cr�er et on
			// l'ajoute au monde
			if (_superArea == null) {
	...
	public static class Exchange {
		...
		synchronized public void toogleOK(int guid) {
			...
			if (i == 1) {
				ok1 = !ok1;
				SocketManager.GAME_SEND_EXCHANGE_OK(perso1.get_compte()
						.getGameThread().get_out(), ok1, guid);
				SocketManager.GAME_SEND_EXCHANGE_OK(perso2.get_compte()
						.getGameThread().get_out(), ok1, guid);
			} else if (i == 2) {
				ok2 = !ok2;
				SocketManager.GAME_SEND_EXCHANGE_OK(perso1.get_compte()
						.getGameThread().get_out(), ok2, guid);
				SocketManager.GAME_SEND_EXCHANGE_OK(perso2.get_compte()
						.getGameThread().get_out(), ok2, guid);
			} else
				return;
		...
		synchronized public void setKamas(int guid, long k) {
			...
			if (perso1.get_GUID() == guid)
				...
				i = 2;
			SocketManager.GAME_SEND_EXCHANGE_OK(perso1.get_compte()
					.getGameThread().get_out(), ok1, perso1.get_GUID());
			SocketManager.GAME_SEND_EXCHANGE_OK(perso2.get_compte()
					.getGameThread().get_out(), ok1, perso1.get_GUID());
			SocketManager.GAME_SEND_EXCHANGE_OK(perso1.get_compte()
					.getGameThread().get_out(), ok2, perso2.get_GUID());
			SocketManager.GAME_SEND_EXCHANGE_OK(perso2.get_compte()
					.getGameThread().get_out(), ok2, perso2.get_GUID());
			...
			if (i == 1) {
				...
				SocketManager.GAME_SEND_EXCHANGE_OTHER_MOVE_OK(perso2
						.get_compte().getGameThread().get_out(), 'G', "", k
						+ "");
			} else if (i == 2) {
				kamas2 = k;
		...
		synchronized public void cancel() {
			if (perso1.get_compte() != null)
				if (perso1.get_compte().getGameThread() != null)
					SocketManager.GAME_SEND_EV_PACKET(perso1.get_compte()
							.getGameThread().get_out());
			if (perso2.get_compte() != null)
				if (perso2.get_compte().getGameThread() != null)
					SocketManager.GAME_SEND_EV_PACKET(perso2.get_compte()
							.getGameThread().get_out());
		...
		synchronized public void apply() {
			...
			for (Couple<Integer, Integer> couple : items1) {
				...
				if (!perso1.hasItemGuid(couple.first))// Si le perso n'a pas
														...
														// pas arriver)
				{
					couple.second = 0;// On met la quantit� a 0 pour �viter les
										// problemes
					continue;
				}
			...
			SocketManager.GAME_SEND_EXCHANGE_VALID(perso1.get_compte()
					.getGameThread().get_out(), 'a');
			SocketManager.GAME_SEND_EXCHANGE_VALID(perso2.get_compte()
					.getGameThread().get_out(), 'a');
	...
	public static void createWorld() {
		...
		System.out.println(ObjTemplates.size()
				+ " templates d'objet ont ete chargees.");
		...
		System.out.println(IOTemplate.size()
				+ " template d'IO ont ete charges.");
	...
	public static NPC_question getNPCQuestion(int guid) {
		...
		if(questObjective != null) // Il y a un objectif de quete avec cette question
		{
			NPC_question questQuestion = new NPC_question(guid,
														  questObjective.getValue().get("optAnswer"),
														  "",
														  "",
														  0);
	...
	public static Compte getCompteByName(String name) {
		return (ComptebyName.get(name.toLowerCase()) != null ? Comptes
				.get(ComptebyName.get(name.toLowerCase())) : null);
	}
		...
		if (perso.get_guild() != null) {
																...
																// est meneur
			{
				...
				for (Personnage newMeneur : perso.get_guild().getMembers()) {
					if (newMeneur == null || newMeneur == perso)
						continue;
	...
	public static void saveAll(Personnage saver) {
		...
		try {
			...
			for (Compte acc : Comptes.values()) {
				...
				SQLManager.UPDATE_ACCOUNT_DATA(acc);// sauvegarde des comptes et
													// de l'etable
			}
			...
			if (saveTry < 10) {
				...
				for (Compte c : Comptes.values()) {
					if (c.getRealmThread() != null)
						SocketManager
								.REALM_SEND_AH_PACKET(c.getRealmThread()._out);
				}
	...
	public static Objet newObjet(int Guid, int template, int qua, int pos,
			String strStats) {
	...
	public static void addHdvItem(int compteID, int hdvID, HdvEntry toAdd) {
		if (_hdvsItems.get(compteID) == null) // Si le compte n'est pas dans la
												// memoire
			_hdvsItems.put(compteID,
	...
	public static void addFullMorph(int morphID, int stuffId, int gfxID,
			String Spells, String name) {
	...
	public static void addQuest(int questId, String steps, int endQuestion,
	...
	public static Entry<Integer, Map<String, String>> getObjectiveByNpcTarget(
			int npcId) {

		for (Entry<Integer, Map<String, String>> entry : questObjetives
				.entrySet()) {
	...
	public static Entry<Integer, Map<String, String>> getObjectiveByOptAnswer(int answerId) {
	...
	public static void PriestRequest(Personnage perso, Carte carte, int IdPretre) {
		...
		if (Homme.getWife() != 0) {
			SocketManager.GAME_SEND_MESSAGE_TO_MAP(carte, Homme.get_name()
					+ " est d�j� marier !", Ancestra.CONFIG_MOTD_COLOR);
			return;
		}
		if (Femme.getWife() != 0) {
	...
	public static String getChallengeFromConditions(boolean sevEnn,
		...
		String noBossChals = ";2;5;9;17;19;24;38;47;50;";// Liste des challenge
															// impossibles
															// contre un boss
	...
	public static ArrayList<String> getRandomChallenge(int nombreChal,
			String challenges) {
		String MovingChals = ";1;2;8;36;37;39;40;";// Challenges de d�placements
													// incompatibles
		boolean hasMovingChal = false;
		String TargetChals = ";3;4;10;25;31;32;34;35;38;42;";// ceux qui ciblent
		boolean hasTargetChal = false;
		String SpellChals = ";5;6;9;11;19;20;24;41;";// ceux qui obligent �
														// caster sp�cialement
		...
		while (compteur < 100 && toReturn.size() < nombreChal) {
			...
			if (!toReturn.contains(chal))// si le challenge n'y etait pas encore
			{
				if (MovingChals.contains(";" + chal.split(",")[0] + ";"))// s'il
																			...
																			// une
																			// liste
					if (!hasMovingChal)// et qu'aucun de la liste n'a ete choisi
										// deja
					{
						hasMovingChal = true;
						toReturn.add(chal);
