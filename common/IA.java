...
public class IA {
	public static class IAThread implements Runnable {
		...
		private static void apply_type6(Fight.Fighter F, Fight fight)// Cambiar
																		// para
																		// dopul
																		// osa
		{
		...
		private static void apply_type10(Fighter fighter, Fight fight) { // esfera
																			// xelor
			int veces = 0;
			while (!stop && fighter.canPlay()) {
				if (++veces >= 8)
				...
				if (veces > 15)
					return;
				Fighter enemigo = getNearestEnnemy(fight, fighter);
				if (enemigo == null)
					return;
				int ataque = attackIfPossible2(fight, fighter);
				while (ataque == 0 && !stop) {
		...
		private static void apply_type12(Fighter fighter, Fight fight) { // chafer
																			// y
																			// chaferloko
			int boucle = 0;
			while (!stop && fighter.canPlay()) {
				...
				if (boucle > 15)
					return;
				Fighter enemigo = getNearestEnnemy(fight, fighter); // Enemigos
				if (enemigo == null) {
					moveFarIfPossible(fight, fighter);
					return;
				}
				int ataque = attackIfPossible3(fight, fighter);
				while (ataque == 0 && !stop) {
					if (ataque == 5)
						stop = true;
					ataque = attackIfPossible3(fight, fighter);
				}
				while (buffIfPossible(fight, fighter, fighter)) {
				}// auto-buff
				enemigo = getNearestEnnemy(fight, fighter);
				if (enemigo == null) {
					moveFarIfPossible(fight, fighter);
					return;
				}
				if (!moveNearIfPossible(fight, fighter, enemigo)) {
					stop = true;
				}
			}
		}
		
		private static void apply_type13(Fighter F, Fight fight) //IA propre La Folle
		{
			while(!stop && F.canPlay())
			{
		...
		private static boolean invocIfPossible(Fight fight,
			...
			try {
				...
				case 1:
					SocketManager.GAME_SEND_EMOTICONE_TO_FIGHT(fight, 7,
							fighter.getGUID(), 1);
					break;
				case 2:
					SocketManager.GAME_SEND_EMOTICONE_TO_FIGHT(fight, 7,
		...
		private static Sort.SortStats getInvocSpell(Fight fight,
				Fight.Fighter fighter, int nearestCell) {
			if (fighter.getMob() == null)
				return null;
			for (Entry<Integer, SortStats> SS : fighter.getMob().getSpells()
					.entrySet()) {
				if (!fight.CanCastSpell3(fighter, (Sort.SortStats) SS
						.getValue(), fight.get_map().getCase(nearestCell)))
					continue;
						...
						if (PDVPER < PDVPERmin && PDVPER < 95) {
							...
							if (f.isPerco()) {
								for (Entry<Integer, SortStats> ss : World
										.getGuild(
												f.getPerco().GetPercoGuildID())
										.getSpells().entrySet()) {
									if (ss.getValue() == null)
										continue;
									if (infl < calculInfluenceHeal(ss
											.getValue())
											&& calculInfluenceHeal(ss
													.getValue()) != 0
											&& fight.CanCastSpell(f,
													ss.getValue(),
													F.get_fightCell(), -1))// Si
																			// le
																			// sort
																			// est
																			// plus
		...
		private static boolean buffIfPossible(Fight fight,
				Fight.Fighter fighter, Fight.Fighter target) {
			if (target == null)
				return false;
		...
		private static boolean buffIfPossible2(Fight pelea, Fighter lanzador,
				Fighter objetivo) {
			if (objetivo == null)
				return false;
		...
		private static Sort.SortStats getBuffSpell(Fight fight,
			...
			if (F.isPerco()) {
				for (Entry<Integer, SortStats> SS : World
					...
					if (infl < calculInfluence(SS.getValue(), F, T)
							...
							&& fight.CanCastSpell(F, SS.getValue(),
									T.get_fightCell(), -1))// Si le sort est
															// plus interessant
					{
				...
				for (Entry<Integer, SortStats> SS : F.getMob().getSpells()
						.entrySet()) {
					if ((infl >= calculInfluence(
							(Sort.SortStats) SS.getValue(), F, T))
							|| (calculInfluence((Sort.SortStats) SS.getValue(),
									F, T) <= 0)
							|| (!fight.CanCastSpell3(F,
		...
		private static Sort.SortStats getHealSpell(Fight fight,
				Fight.Fighter F, Fight.Fighter T) {
			int infl = 0;
			Sort.SortStats ss = null;
			if (F.isPerco()) {
				...
				for (Entry<Integer, SortStats> SS : F.getMob().getSpells()
						.entrySet()) {
					if ((infl >= calculInfluenceHeal((Sort.SortStats) SS
							.getValue()))
		...
		private static Fight.Fighter getNearestFriend(Fight fight,
				Fight.Fighter fighter) {
			int dist = 1000;
			...
			for (Fight.Fighter f : fight.getFighters(3)) {
				if ((f.isDead()) || (f == fighter)
					...
					continue;
				int d = Pathfinding.getDistanceBetween(fight.get_map(), fighter
						.get_fightCell().getID(), f.get_fightCell().getID());
				if (d >= dist)
					continue;
				dist = d;
				curF = f;
		...
		private static Fight.Fighter getNearestEnnemy(Fight fight,
				Fight.Fighter fighter) {
			int dist = 1000;
			Fight.Fighter curF = null;
			for (Fight.Fighter f : fight.getFighters(3)) {
		...
		private static Map<Integer, Fight.Fighter> getLowHpEnnemyList(
				Fight fight, Fight.Fighter fighter) {
			...
			while (i < i2) {
				...
				Fight.Fighter test = (Fight.Fighter) ennemy.get(Integer
						.valueOf(curHP));
		...
		private static Map<Integer, Fighter> getLowHpEnnemyList2(Fight pelea,
				Fighter lanzador) {
			Fighter enemigoMasCercano = getNearestEnnemy(pelea, lanzador);
			Map<Integer, Fighter> listaEnemigos = new TreeMap<Integer, Fighter>();
			Map<Integer, Fighter> enemigosNoInvo = new TreeMap<Integer, Fighter>();
			Map<Integer, Fighter> enemigosInvo = new TreeMap<Integer, Fighter>();
			for (Fighter f : pelea.getFighters(3)) {
				if (f.hasLeft())
					continue;
				if (f.isDead())
					continue;
				if (f.isInvocation())
					enemigosInvo.put(f.getPDV(), f);
				else
					enemigosNoInvo.put(f.getPDV(), f);
			}
			if (enemigoMasCercano != null)
				listaEnemigos
						.put(enemigoMasCercano.getPDV(), enemigoMasCercano);
			int i = 0, i2 = enemigosNoInvo.size(), i3 = enemigosInvo.size();
			int curHP;
			while (i < i2) {
				curHP = 200000;
				for (Entry<Integer, Fighter> t : enemigosNoInvo.entrySet()) {
					if (t.getValue().getPDV() < curHP)
						curHP = t.getValue().getPDV();
				}
				Fighter test = enemigosNoInvo.get(curHP);
				listaEnemigos.put(test.getPDV(), test);
				enemigosNoInvo.remove(curHP);
				i++;
			}
			i = 0;
			while (i < i3) {
				curHP = 200000;
				for (Entry<Integer, Fighter> t : enemigosInvo.entrySet()) {
					if (t.getValue().getPDV() < curHP)
						curHP = t.getValue().getPDV();
				}
				Fighter test = enemigosInvo.get(curHP);
				listaEnemigos.put(test.getPDV(), test);
				enemigosInvo.remove(curHP);
				i++;
			}
			return listaEnemigos;
		}

		//IA anti attaque si invi
		...
		private static int attackIfPossible(Fight fight, Fight.Fighter fighter) {
			...
			for (Entry<Integer, Fighter> t : ennemyList.entrySet()) {
				SS = getBestSpellForTarget(fight, fighter,
						(Fight.Fighter) t.getValue());
			...
			if (fighter.isPerco()) {
				for (Entry<Integer, SortStats> S : World
						.getGuild(fighter.getPerco().GetPercoGuildID())
						.getSpells().entrySet()) {
					if (S.getValue() == null)
						continue;
					int targetVal = getBestTargetZone(fight, fighter,
							S.getValue(), fighter.get_fightCell().getID());
		...
		private static int attackIfPossible2(Fight fight, Fighter fighter) {
			...
			if (objetivo == null || SH == null)
				return 666;
			int ataque = fight.tryCastSpell(fighter, SH, objetivo
					.get_fightCell().getID());
			if (ataque != 0)
				return ataque;
			return 0;
		...
		private static int attackIfPossible3(Fight fight, Fighter fighter) {
			Map<Integer, Fighter> ennemyList = getLowHpEnnemyList2(fight,
					fighter);
			SortStats SS = null;
			Fighter objetivo = null;
			for (Entry<Integer, Fighter> t : ennemyList.entrySet()) {
			...
			if (objetivo == null || SS == null)
				return 666;
			int ataque = fight.tryCastSpell(fighter, SS, objetivo
					.get_fightCell().getID());
			if (ataque != 0)
				return ataque;
			return 0;
		...
		private static boolean moveToAttackIfPossible(Fight fight,
				Fight.Fighter fighter) {
			ArrayList<?> cells = Pathfinding.getListCaseFromFighter(fight,
					fighter);
			if (cells == null)
				return false;
			int distMin = Pathfinding.getDistanceBetween(fight.get_map(),
					fighter.get_fightCell().getID(),
					getNearestEnnemy(fight, fighter).get_fightCell().getID());
			ArrayList<SortStats> sorts = getLaunchableSort(fighter, fight,
					distMin);
			if (sorts == null)
				return false;
			ArrayList<Fighter> targets = getPotentialTarget(fight, fighter,
					sorts);
			if (targets == null) {
				return false;
			}
			int CellDest = 0;
			boolean found = false;
			for (Iterator<?> localIterator1 = cells.iterator(); localIterator1
					.hasNext();) {
				int i = ((Integer) localIterator1.next()).intValue();

				for (Sort.SortStats S : sorts) {
					for (Fight.Fighter T : targets) {
						if (fight.CanCastSpell2(fighter, S, T.get_fightCell(),
								i)) {
							CellDest = i;
							found = true;
						}
						...
						if (targetVal > 0) {
							...
							if (fight.get_map().getCase(cellID) != null) {
								if (fight.CanCastSpell2(fighter, S, fight
										.get_map().getCase(cellID), i)) {
									CellDest = i;
									found = true;
								}
							}
			...
			if (CellDest == 0)
				return false;
			ArrayList<Case> path = Pathfinding.getShortestPathBetween(
					fight.get_map(), fighter.get_fightCell().getID(), CellDest,
					0);
			if (path == null)
				return false;
			String pathstr = "";
		...
		private static ArrayList<Sort.SortStats> getLaunchableSort(
				Fight.Fighter fighter, Fight fight, int distMin) {
			ArrayList<SortStats> sorts = new ArrayList<SortStats>();
			if (fighter.getMob() == null)
				return null;
			for (Entry<Integer, SortStats> S : fighter.getMob().getSpells()
					.entrySet()) {
				if (((Sort.SortStats) S.getValue()).getPACost() > fighter
						.getCurPA(fight)) {
					continue;
				}
				if (!Fight.LaunchedSort.coolDownGood(fighter,
						((Sort.SortStats) S.getValue()).getSpellID()))
					continue;
				if ((((Sort.SortStats) S.getValue()).getMaxLaunchbyTurn()
						- Fight.LaunchedSort.getNbLaunch(fighter,
								((Sort.SortStats) S.getValue()).getSpellID()) <= 0)
						&& (((Sort.SortStats) S.getValue())
								.getMaxLaunchbyTurn() > 0))
					continue;
				if (calculInfluence((Sort.SortStats) S.getValue(), fighter,
						fighter) >= 0)
					continue;
		...
		private static ArrayList<Sort.SortStats> TriInfluenceSorts(
		...
		private static ArrayList<Fight.Fighter> getPotentialTarget(Fight fight,
				Fight.Fighter fighter, ArrayList<Sort.SortStats> sorts) {
		...
		private static Sort.SortStats getBestSpellForTarget(Fight fight,
				Fight.Fighter F, Fight.Fighter T) {
			int inflMax = 0;
			Sort.SortStats ss = null;
			if (F.isPerco()) {
				for (Entry<Integer, SortStats> SS : World
					...
					int PA = 6;
					int usedPA[] = { 0, 0 };
					if (!fight.CanCastSpell(F, SS.getValue(),
					...
					for (Entry<Integer, SortStats> SS2 : World
						...
						for (Entry<Integer, SortStats> SS3 : World
							...
							if ((PA - usedPA[0] - usedPA[1]) < SS3.getValue()
								...
								continue;
							if (!fight
									.CanCastSpell(F, SS3.getValue(), F
											.get_fightCell(), T.get_fightCell()
											.getID()))
								continue;
				...
				for (Entry<Integer, SortStats> SS : F.getMob().getSpells()
					...
					if (fight.CanCastSpell3(F, (Sort.SortStats) SS.getValue(),
							T.get_fightCell())) {
						...
						if (curInfl != 0) {
							...
							for (Entry<Integer, SortStats> SS2 : F.getMob()
									.getSpells().entrySet()) {
								...
								if (curInfl != 0) {
									...
									for (Entry<Integer, SortStats> SS3 : F
											.getMob().getSpells().entrySet()) {
		...
		private static SortStats getBestSpellForTarget2(Fight pelea,
				Fighter lanzador, Fighter objetivo) {
			SortStats ss = null;
		...
		private static int getBestTargetZone(Fight fight,
				Fight.Fighter fighter, Sort.SortStats spell, int launchCell) {
			if ((spell.getPorteeType().charAt(0) == 'P')
					&& (spell.getPorteeType().charAt(1) == 'a'))
				return 0;
		...
		private static int calculInfluence(Sort.SortStats ss, Fight.Fighter C,
				Fight.Fighter T) {
			int infTot = 0;
			for (SpellEffect SE : ss.getEffects()) {
				int inf = 0;
				...
				case 219:
					...
					break;
				case 265:
					inf = -400 * Formulas.getMiddleJet(SE.getJet());
				}
