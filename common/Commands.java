...
public class Commands {
	...
	public void consoleCommand(String packet) {
		...
		} else if (_compte.get_gmLvl() == 2) {
			commandGmTwo(command, infos, msg);
		} else if (_compte.get_gmLvl() == 3) {
			commandGmThree(command, infos, msg);
		} else if (_compte.get_gmLvl() >= 4) {
	...
	public void commandGmOne(String command, String[] infos, String msg) {
		...
		if (command.equalsIgnoreCase("INFOS")) {
			...
			String mess = "===========\n" + Ancestra.makeHeader() + "Uptime: "
					+ jour + "j " + hour + "h " + min + "m " + sec + "s\n"
					+ "Joueurs en lignes: "
			...
			for (byte b = 0; b < 30; b++) {
				if (b == Ancestra.gameServer.getClients().size())
					break;
			...
			if (!perso.isOnline()) {
				String mess = "Le personnage " + perso.get_name()
			...
			if (!perso.isOnline()) {
				mess = "(Le personnage " + perso.get_name()
						+ " n'etait pas connecte)";
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, mess);
			}
		} else if (command.equalsIgnoreCase("ANNOUNCE")) {
			infos = msg.split(" ", 2);
			SocketManager.GAME_SEND_MESSAGE_TO_ALL(infos[1],
			...
			SocketManager.GAME_SEND_ERASE_ON_MAP_TO_MAP(target.get_curCarte(),
					target.get_GUID());
			SocketManager.GAME_SEND_ADD_PLAYER_TO_MAP(target.get_curCarte(),
					target);
			...
			SocketManager.GAME_SEND_MESSAGE_TO_ALL(prefix + infos[1],
					Ancestra.CONFIG_MOTD_COLOR);
			return;
			...
			if (perso == null || time < 0) {
				String mess = "Le personnage n'existe pas ou la dur&e est invalide.";
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, mess);
				return;
			}
			Personnage P = World.getPersoByName(infos[1]);
			SocketManager.GAME_SEND_MESSAGE_TO_ALL(
					"Le joueur <b>" + P.get_name()
							+ "</b> vient d'être <b>muté</b> (<b>" + time
							+ "</b> secondes) par <b>" + _perso.get_name()
							+ "</b> pour " + cause + "",
					Ancestra.CONFIG_MOTD_COLOR);
			String mess = "Vous avez mute " + perso.get_name() + " pour "
					+ time + " secondes";
			...
			perso = World.getPersoByName(name);
			if (perso == null) {
				String mess = "Le personnage n'�xiste pas.";
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, mess);
				return;
			}
			...
			SocketManager.GAME_SEND_ADD_PLAYER_TO_MAP(target.get_curCarte(),
					target);
			String str = "La taille du joueur a �t� modifi�e";
			SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, str);
			return;
			...
			String str = "Le nombre de point de sort a �t� modifi�e";
			SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, str);
			return;
			...
			if (align < Constants.ALIGNEMENT_NEUTRE
					|| align > Constants.ALIGNEMENT_MERCENAIRE) {
				String str = "Valeur invalide";
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, str);
				return;
			}
			...
			String str = "L'alignement du joueur a �t� modifi�";
			SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, str);
			return;
			...
			String str = "Vous avez ajouter " + honor + " honneur a "
					+ target.get_name();
			...
			SocketManager.GAME_SEND_STATS_PACKET(target);
			String str = "Le capital a �t� modifi�e";
			SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, str);
			return;
		} else
		...
		if (command.equalsIgnoreCase("ITEMSET")) {
			...
			boolean useMax = false;
			if (infos.length == 3)
				useMax = infos[2].equals("MAX");// Si un jet est sp�cifi�

			for (ObjTemplate t : IS.getItemTemplates()) {
				Objet obj = t.createNewItem(1, useMax);
				if (_perso.addObjet(obj, true))// Si le joueur n'avait pas
												// d'item similaire
					World.addObjet(obj, true);
			}
	...
	public void commandGmTwo(String command, String[] infos, String msg) {
		...
		if (command.equalsIgnoreCase("MUTE")) {
			...
			try {
				razon = msg
						.substring(infos[0].length() + infos[1].length() + 1);
			} catch (Exception e) {
			}
			;
			perso = World.getPersoByName(name);
			...
			if (perso.isOnline()) {
				Personnage P = World.getPersoByName(infos[1]);
				if (razon != "") {
					String msj = "Vous viens d'�tre expuls� pour cette raison:\n"
							+ razon + "\n\n" + _perso.get_name();
					SocketManager.SEND_MESSAGE_DECO(P, 18, msj);
				} else {
					String msj = "Vous viens d'�tre expuls�.\n"
							+ _perso.get_name();
					SocketManager.SEND_MESSAGE_DECO(P, 18, msj);
				}
				SocketManager.GAME_SEND_MESSAGE_TO_ALL(
						"Le joueur <b>" + P.get_name()
								+ "</b> vient d'�tre kick par <b>"
								+ _perso.get_name() + "</b> pour " + razon
								+ ".", Ancestra.CONFIG_MOTD_COLOR);
		...
		if (command.equalsIgnoreCase("MOVENPC")) {
			...
			SocketManager.GAME_SEND_ERASE_ON_MAP_TO_MAP(_perso.get_curCarte(),
					id);
			...
			if (SQLManager.DELETE_NPC_ON_MAP(_perso.get_curCarte().get_id(),
					exC)
					&& SQLManager.ADD_NPC_ON_MAP(
							_perso.get_curCarte().get_id(), npc.get_template()
			...
			if (target.get_fight() == null)
				SocketManager.GAME_SEND_ALTER_GM_PACKET(target.get_curCarte(),
						target);
		} else {
	...
	public void commandGmThree(String command, String[] infos, String msg) {
		 ...
		 * SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, "Ok."); } else
		 */
		if (command.equalsIgnoreCase("SAVE") && !Ancestra.isSaving) {
			...
			String mess = "["
					+ Pathfinding.getCellXCoord(_perso.get_curCarte(), cell)
					+ ","
					+ Pathfinding.getCellYCoord(_perso.get_curCarte(), cell)
					+ "]";
			SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, mess);
			return;
			...
			if (P.get_compte() == null)
				SQLManager.LOAD_ACCOUNT_BY_GUID(P.getAccID());
			if (P.get_compte() == null) {
			...
			if (P.get_compte().getGameThread() != null)
				P.get_compte().getGameThread().kick();
			SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out,
					"Vous avez banni " + P.get_name());
			SocketManager.GAME_SEND_MESSAGE_TO_ALL("Le joueur " + P.get_name()
					+ " a �t� banni pour " + cause + "",
					Ancestra.CONFIG_MOTD_COLOR);
			return;
			...
			if (cell < 0 || _perso.get_curCarte().getCase(cell) == null
					|| !_perso.get_curCarte().getCase(cell).isWalkable(true)) {
				cell = _perso.get_curCell().getID();
			}
			String places = _perso.get_curCarte().get_placesStr();
			String[] p = places.split("\\|");
			...
			System.out.println("0 => " + team0 + "\n1 =>" + team1 + "\nCell: "
					+ CryptManager.cellID_To_Code(cell));
			for (int a = 0; a <= team0.length() - 2; a += 2)
			...
			String mess = "L'action a ete ajoute";
			_perso.get_curCarte().addEndFightAction(type,
					new Action(id, args, cond));
			boolean ok = SQLManager.ADD_ENDFIGHTACTION(_perso.get_curCarte()
			...
			_perso.get_curCarte().addStaticGroup(_perso.get_curCell().getID(),
					groupData);
			String str = "Le grouppe a ete fixe";
			// Sauvegarde DB de la modif
			if (SQLManager.SAVE_NEW_FIXGROUP(_perso.get_curCarte().get_id(),
			...
			NPC npc = _perso.get_curCarte().addNpc(id,
			...
			if (SQLManager.ADD_NPC_ON_MAP(_perso.get_curCarte().get_id(), id,
					_perso.get_curCell().getID(), _perso.get_orientation()))
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, str);
			else
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out,
			...
			boolean success = SQLManager.REMOVE_TRIGGER(_perso.get_curCarte()
					.get_id(), cellID);
			String str = "";
			if (success)
			...
			if (size == -1 || owner == -2 || price == -1
					|| _perso.get_curCarte().getMountPark() != null) {
				String str = "Infos invalides ou map deja config.";
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, str);
				return;
			}
			MountPark MP = new MountPark(owner, _perso.get_curCarte(), _perso
			...
			{
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out,
						"Un shutdown est deja programmer.");
	...
	public void commandGmFour(String command, String[] infos, String msg) {
		...
		if (command.equalsIgnoreCase("MESSAGEPOPUP")) {
			infos = msg.split(" ", 2);
			SocketManager.GAME_SEND_PANEL_INFOS(infos[1]);
			return;
			...
			String str = "Le niveau GM du joueur a �t� modifi�";
			SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, str);
			return;
			...
			if (perso == null) {
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(this._out,
			...
			if (Constants.BAN_IP.contains(perso.get_compte().get_curIP())) {
				if (SQLManager.DELETE_BANIP(perso.get_compte().get_curIP())) {
					...
					SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(this._out,
							"L'IP a correctement �tait d�banni");
					return;
				}
			}
	        ...
	        try
	        {
	         tps = Integer.parseInt(infos[1]);
	        }catch(Exception e){};
	        
			   for(Personnage p:_perso.get_curCarte().getPersos()) {
				   if(p.get_GUID() != _perso.get_GUID()) // Si ce n'est pas le muteur
					   p.get_compte().mute(true, tps);
			   }
			
			   SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out,"Map " + _perso.get_curCarte().get_id() + 
																   " mutté pour " + tps + " minutes");
	    }else
		 if(command.equalsIgnoreCase("UNMUTEMAP")){
			int tps = 0;
			for(Personnage p:_perso.get_curCarte().getPersos())
				p.get_compte().mute(false, tps);
	...
	private int calculPrice(Objet obj, int logAmount) {
		...
		if (stats > 0)
			return (int) (((Math.cbrt(stats) * Math.pow(obj.getTemplate()
					.getLevel(), 2)) * 10 + Formulas.getRandomValue(1, obj
					.getTemplate().getLevel() * 100)) * amount);
		else
			return (int) ((Math.pow(obj.getTemplate().getLevel(), 2) * 10 + Formulas
					.getRandomValue(1, obj.getTemplate().getLevel() * 100)) * amount);
	}
}