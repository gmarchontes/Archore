...
public class Trunk {
	...
	public Trunk(int id, int house_id, short mapid, int cellid, String object,
			long kamas, String key, int owner_id) {
		_id = id;
		_house_id = house_id;
		_mapid = mapid;
		_cellid = cellid;
	...
	public static Trunk get_trunk_id_by_coord(int map_id, int cell_id) {
		for (Entry<Integer, Trunk> trunk : World.getTrunks().entrySet()) {
			if (trunk.getValue().get_mapid() == map_id
					&& trunk.getValue().get_cellid() == cell_id) {
	...
	public void HopIn(Personnage P)// Ouvrir coffre
	{
		...
		if (t == null)
			return;
		if (t.get_owner_id() == P.getAccID()
				|| (P.get_guild() == null ? false : P.get_guild().get_id() == h
						.get_guild_id() && h.canDo(Constants.C_GNOCODE))) {
			OpenTrunk(P, "-", true);
		} else if (P.get_guild() == null && h.canDo(Constants.C_OCANTOPEN))// si
																			// on
																			// compare
																			// par
																			// id
																			...
																			// null)
		{
			SocketManager
					.GAME_SEND_MESSAGE(
							P,
							"Ce coffre ne peut �tre ouvert que par les membres de la guilde !",
							Ancestra.CONFIG_MOTD_COLOR);
			return;
		} else if (t.get_owner_id() > 0)// Une personne autre le poss�de, il
	...
	public static void OpenTrunk(Personnage P, String packet, boolean isTrunk)// Ouvrir
																				// un
																				// coffre
	{
		Trunk t = P.getInTrunk();
		...
		if (packet.compareTo(t.get_key()) == 0 || isTrunk)// Si c'est chez lui
															// ou que le mot de
															// passe est bon
		{
			SocketManager.GAME_SEND_ECK_PACKET(P.get_compte().getGameThread()
					.get_out(), 5, "");
	...
	public void addInTrunk(int guid, int qua, Personnage P) {
		if (P.getInTrunk().get_id() != get_id())
			return;

		if (_object.size() >= 80) // Le plus grand c'est pour si un admin ajoute
									// des objets via la bdd...
		{
			SocketManager.GAME_SEND_MESSAGE(P,
					"Le nombre d'objets maximal de ce coffre � �t� atteint !",
					Ancestra.CONFIG_MOTD_COLOR);
			return;
		}
		...
		for (Personnage perso : P.get_curCarte().getPersos()) {
			if (perso.getInTrunk() != null
					&& get_id() == perso.getInTrunk().get_id()) {
