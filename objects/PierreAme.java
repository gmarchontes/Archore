...
public class PierreAme extends Objet {
	...
	public PierreAme(int Guid, int qua, int template, int pos, String strStats) {
		this.guid = Guid;
		this.template = World.getObjTemplate(template); // 7010 = Pierre d'ame
														// pleine
		this.quantity = 1;
	...
	public void parseStringToStats(String monsters) // Dans le format
													// "monstreID,lvl|monstreID,lvl..."
	{
	...
	public String parseStatsString() {
		...
		for (Couple<Integer, Integer> coupl : _monsters) {
			...
			try {
				stats.append("26f#0#0#").append(
						Integer.toHexString(coupl.first));
			} catch (Exception e) {
				e.printStackTrace();
				continue;
			}
	...
	public String parseGroupData()// Format :
									// id,lvlMin,lvlMax;id,lvlMin,lvlMax...
	{
