...
public class NPC_tmpl {
	...
	public static class NPC_question {
		...
		public NPC_question(int _id, String _reponses, String _args,
				String _cond, int falseQuestion) {
			this._id = _id;
		...
		public String parseToDQPacket(Personnage perso) {
			if (!ConditionParser.validConditions(perso, _cond))
				return World.getNPCQuestion(falseQuestion).parseToDQPacket(
						perso);
	...
	public static class NPC {
		...
		public String parseGM() {
			...
			if (_template.get_scaleX() == _template.get_scaleY()) {
				...
				taille.append(_template.get_scaleX()).append("x")
						.append(_template.get_scaleY());
			}
			sock.append(_template.get_gfxID()).append("^")
					.append(taille.toString()).append(";");
			sock.append(_template.get_sex()).append(";");
			sock.append(
					(_template.get_color1() != -1 ? Integer
							.toHexString(_template.get_color1()) : "-1"))
					.append(";");
			sock.append(
					(_template.get_color2() != -1 ? Integer
							.toHexString(_template.get_color2()) : "-1"))
					.append(";");
			sock.append(
					(_template.get_color3() != -1 ? Integer
							.toHexString(_template.get_color3()) : "-1"))
					.append(";");
			sock.append(_template.get_acces()).append(";");
			sock.append(
					(_template.get_extraClip() != -1 ? (_template
							.get_extraClip()) : (""))).append(";");
		...
		public void addAction(Action act) {
			...
			/*for (Action a : c)
				if (a.getID() == act.getID())
					_actions.remove(a); */ // Empêche la possibilité d'avoir plusieurs meme actions 
			_actions.add(act);
		}
