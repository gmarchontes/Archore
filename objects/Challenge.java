...
public class Challenge {
	...
	public void challenge_loose(Fighter fighter) {
		...
		if (fighter != null && fighter.getPersonnage() != null)
			name = fighter.getPersonnage().get_name();
		challengeWin = false;
		challengeAlive = false;
		SocketManager.GAME_SEND_CHALLENGE_FIGHT(fight, 7, "KO" + type);
		SocketManager
				.GAME_SEND_Im_PACKET_TO_CHALLENGE(fight, 1, "0188;" + name);
	}
	...
	public String parsePacket() {
		...
		packet.append(type)
				.append(";")
				.append(_cible != null ? "1" : "0")
				.append(";")
				.append(_cible != null ? (Integer.valueOf(_cible.getGUID()))
						: "").append(";").append(xpWin).append(";0;")
				.append(dropWin).append(";0;");
		if (!challengeAlive) {
			if (challengeWin)
	...
	public void show_cibleToPerso(Personnage p) {
		if (!challengeAlive || _cible == null || _cible.get_fightCell() == null
				|| p == null)
			return;

		ArrayList<PrintWriter> Pws = new ArrayList<PrintWriter>();
		Pws.add(p.get_compte().getGameThread().get_out());
		SocketManager.GAME_SEND_FIGHT_SHOW_CASE(Pws, _cible.getGUID(), _cible
				.get_fightCell().getID());
	}
	...
	public void show_cibleToFight() {
		...
		for (Fighter fighter : fight.getFighters(1)) {
			...
			if (fighter.getPersonnage() == null
				...
				continue;
			Pws.add(fighter.getPersonnage().get_compte().getGameThread()
					.get_out());
		}
		SocketManager.GAME_SEND_FIGHT_SHOW_CASE(Pws, _cible.getGUID(), _cible
				.get_fightCell().getID());
	...
	public void fightStart()// D�finit les cibles au d�but du combat
	{
		...
		case 32:// Elitiste
		case 35:// Tueur � gages
			if (_cible == null && _ordreJeu.size() > 0)// Si aucun cible n'est
														// choise on en choisie
														// une
			{
				...
				for (Fighter f : Choix) {
					if (f.getPersonnage() != null)
						continue;
					if (f.getMob() != null && f.getTeam2() == 2 && !f.isDead())
						_cible = f;
				}
			}
		...
		case 10:// Cruel
			int levelMin = 2000;
			for (Fighter fighter : fight.getFighters(2))// La cible sera le
														// niveau le plus faible
			{
				if (fighter.getPersonnage() == null && fighter.getMob() != null
						&& fighter.get_lvl() < levelMin) {
	...
	public void fightEnd()// V�rifie la validit� des challenges en fin de combat
							// (si n�cessaire)
	{
		if (!challengeAlive)
			return;
	...
	public void fighterDie(Fighter Deadtarget)// Si un Fighter meurt on v�rifie
												// la validit� des challenges
	{
		if (!challengeAlive)
			return;
	...
	public void onFighters_attacked(ArrayList<Fighter> targets, Fighter caster,
			int effectID)// Si on attaque un fighter (soins ou attaque)
	{
		if (!challengeAlive)
			return;
		...
		case 17:// Intouchable
			...
			break;
		case 18:// Incurable
			if ((caster.getTeam() == 0) && !caster.isInvocation()
					&& HealingEffects.contains("|" + effectID + "|")) {
				challenge_loose(caster);
				break;
			}
			break;
			...
			if ((caster.getTeam() == 0)
				...
				for (Fighter target : targets) {
					if (target.getTeam() == 1) {
						if (Args.isEmpty())
							Args += "" + target.getGUID();
						else if (!Args.contains("" + target.getGUID()))
							challenge_loose(caster);
					}
				}
			}
		...
		case 46:// Chacun son monstre
			if ((caster.getTeam() == 0)
				...
				for (Fighter target : targets) {
					if (target.getTeam() == 1) {
						if (!Args.contains(";" + target.getGUID() + ","))
							Args += ";" + target.getGUID() + ","
									+ caster.getGUID() + ";";
						else if (Args.contains(";" + target.getGUID() + ",")
								&& !Args.contains(";" + target.getGUID() + ","
										+ caster.getGUID() + ";"))
							challenge_loose(target);
					}
				}
			}
			break;
	...
	public void onMob_die(Fighter mob, Fighter caster) {
		...
		case 35:// Tueur � gages
			...
			_cible = null;
			if (_cible == null && _ordreJeu.size() > 0)// Choix d'une nouvelle
														// cible
			{
				List<Fighter> Choix = new ArrayList<Fighter>();
				Choix.addAll(_ordreJeu);
	...
	public void onPlayer_action(Fighter fighter, int actionID) {
		if (!challengeAlive || fighter == null || fighter.getTeam() == 1)
			return;
		if (System.currentTimeMillis() - lastActions_time < 500)
			return;
		...
		case 24:// Born�
			if (!lastActions.contains(action.toString())
					&& lastActions.contains(";" + fighter.getGUID() + ","))
				challenge_loose(fight.getCurFighter());
			lastActions += action.toString();
			break;
		}
	...
	public void onfight_EndTurn(Fighter fighter) {
		...
		Neighbours = Pathfinding.getFightersAround(fighter.get_fightCell()
				.getID(), fight.get_map(), fight);
		...
		case 1:// Zombie
			...
			break;
		case 2:// Statue
			if (fighter.get_fightCell().getID() != Arg)
				challenge_loose(fighter);
			break;
		case 7:// Jardinier
			if (fighter.getPersonnage() != null) {
