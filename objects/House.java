...
public class House {
	...
	public House(int id, short map_id, int cell_id, int owner_id, int sale,
			int guild_id, int access, String key, int guildrights, int mapid,
			int caseid) {
		_id = id;
		_map_id = map_id;
		_cell_id = cell_id;
	...
	public static House get_house_id_by_coord(int map_id, int cell_id) {
		for (Entry<Integer, House> house : World.getHouses().entrySet()) {
			if (house.getValue().get_map_id() == map_id
					&& house.getValue().get_cell_id() == cell_id) {
	...
	public static void LoadHouse(Personnage P, int newMapID)// Affichage des
															// maison + Blason
	{
		...
		for (Entry<Integer, House> house : World.getHouses().entrySet()) {
			if (house.getValue().get_map_id() == newMapID) {
				...
				packet.append("P").append(house.getValue().get_id())
						.append("|");
				if (house.getValue().get_owner_id() > 0) {
					...
					if (C == null)// Ne devrait pas arriver
					{
						...
						packet.append(
								World.getCompte(house.getValue().get_owner_id())
										.get_pseudo()).append(";");
					}
				} else {
					packet.append(";");
				...
				if (house.getValue().get_guild_id() > 0) // Maison de guilde
				{
					...
					if (G != null) {
						...
						if (G.getMembers().size() < 10)// Ce n'est plus une
														// maison de guilde
						{
							SQLManager.HOUSE_GUILD(house.getValue(), 0, 0);
						} else {
							// Affiche le blason pour les membre de guilde OU
							// Affiche le blason pour les non membre de guilde
							if (P.get_guild() != null
									&& P.get_guild().get_id() == house
											.getValue().get_guild_id()
									&& house.getValue().canDo(
											Constants.H_GBLASON))// meme guilde
							{
								packet.append(";").append(Gname).append(";")
										.append(Gemblem);
							} else if (house.getValue().canDo(
									Constants.H_OBLASON))// Pas de
															// guilde/guilde-diff�rente
							{
								packet.append(";").append(Gname).append(";")
										.append(Gemblem);
							}
				...
				if (house.getValue().get_owner_id() == P.getAccID()) {
					...
					packet1.append("L+|").append(house.getValue().get_id())
							.append(";").append(house.getValue().get_access())
							.append(";");

					if (house.getValue().get_sale() <= 0) {
						packet1.append("0;")
								.append(house.getValue().get_sale());
	...
	public void HopIn(Personnage P)// Entrer dans la maison
	{
		...
		if (P.get_fight() != null || P.get_isTalkingWith() != 0
				|| P.get_isTradingWith() != 0 || P.getCurJobAction() != null
		...
		if (h == null)
			return;
		if (h.get_owner_id() == P.getAccID()
				|| (P.get_guild() != null
						&& P.get_guild().get_id() == h.get_guild_id() && canDo(Constants.H_GNOCODE)))// C'est
																										// sa
																										// maison
																										// ou
																										// m�me
		...
		{
			OpenHouse(P, "-", true);
		} else if (h.get_owner_id() > 0) // Une personne autre la acheter, il
											// faut le code pour rentrer
		{
			SocketManager.GAME_SEND_KODE(P, "CK0|8");// 8 �tant le nombre de
														// chiffre du code
		} else if (h.get_owner_id() == 0) // Maison non acheter, mais achetable,
											// on peut rentrer sans code
		{
			OpenHouse(P, "-", false);
		} else {
			return;
	...
	public static void OpenHouse(Personnage P, String packet, boolean isHome)// Ouvrir
																				// une
																				// maison
																				// ;o
	{
		...
		House h = P.getInHouse();
		if ((!h.canDo(Constants.H_OCANTOPEN) && (packet.compareTo(h.get_key()) == 0))
				|| isHome)// Si c'est chez lui ou que le mot de passe est bon
		{
			P.teleport((short) h.get_mapid(), h.get_caseid());
			closeCode(P);
		} else if ((packet.compareTo(h.get_key()) != 0)
				|| h.canDo(Constants.H_OCANTOPEN))// Mauvais code
		{
	...
	public static void HouseAchat(Personnage P)// Acheter une maison
	{
		...
		for (Trunk t : Trunk.getTrunksByHouse(h)) {
			if (h.get_owner_id() > 0) {
				t.moveTrunktoBank(World.getCompte(h.get_owner_id()));// D�placement
																		// des
																		// items
																		// vers
																		// la
		...
		if (h.get_owner_id() > 0) {
			...
			if (Seller.get_curPerso() != null) {
				SocketManager.GAME_SEND_MESSAGE(
						Seller.get_curPerso(),
						"Une maison vous appartenant � �t� vendue "
								+ h.get_sale() + " kamas.",
	...
	public static void SellPrice(Personnage P, String packet)// Vendre une
																// maison
	{
		House h = P.getInHouse();
	...
	public static String parseHouseToGuild(Personnage P) {
		...
		String packet = "+";
		for (Entry<Integer, House> house : World.getHouses().entrySet()) {
			if (((House) house.getValue()).get_guild_id() == P.get_guild()
					.get_id()
					&& ((House) house.getValue()).get_guild_rights() > 0)
				if (isFirst) {
					packet += house.getKey() + ";";
					if (World.getPersonnage(house.getValue().get_owner_id()) == null)
						packet += "Due�o privado;";
					else
						packet += World
								.getPersonnage(house.getValue().get_owner_id())
								.get_compte().get_pseudo()
								+ ";";
					packet += World.getCarte(
							(short) house.getValue().get_mapid()).getX()
							+ ","
							+ World.getCarte(
									(short) house.getValue().get_mapid())
									.getY() + ";";
					packet += "0;";
	...
	public static void parseHG(Personnage P, String packet) {
		...
		if (packet != null) {
			if (packet.charAt(0) == '+') {
				// Ajoute en guilde
				byte HouseMaxOnGuild = (byte) Math.floor(P.get_guild()
						.get_lvl() / 10);
				if (HouseOnGuild(P.get_guild().get_id()) >= HouseMaxOnGuild)
					return;
				if (P.get_guild().getMembers().size() < 10)
					return;
				...
				SQLManager.HOUSE_GUILD(h, h.get_guild_id(),
			...
			if (h.get_guild_id() <= 0) {
				...
				SocketManager.GAME_SEND_hOUSE(
						P,
						"G" + h.get_id() + ";" + P.get_guild().get_name() + ";"
								+ P.get_guild().get_emblem() + ";"
								+ h.get_guild_rights());
			}
		}
	}
	...
	public void parseIntToRight(int total) {
		...
		if (haveRight.size() > 0) // Si les droits contiennent quelque chose ->
									// Vidage (M�me si le TreeMap supprimerais
									// les entr�es doublon lors de l'ajout)
			haveRight.clear();

		initRight(); // Remplissage des droits

		Integer[] mapKey = haveRight.keySet().toArray(
				new Integer[haveRight.size()]); // R�cup�re les clef de map dans
	...
	public static void Leave(Personnage P, String packet) {
		...
		if (Target == null || !Target.isOnline() || Target.get_fight() != null
				|| Target.get_curCarte().get_id() != P.get_curCarte().get_id())
			return;
		Target.teleport(h.get_map_id(), h.get_cell_id());
		SocketManager.GAME_SEND_Im_PACKET(Target, "018;" + P.get_name());
	}

	public static House get_HouseByPerso(Personnage P)// Connaitre la MAPID +
														// CELLID de sa maison
	{
		for (Entry<Integer, House> house : World.getHouses().entrySet()) {
			if (house.getValue().get_owner_id() == P.getAccID()) {
