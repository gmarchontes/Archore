...
public class Sort {
	...
	public static class SortStats {
		...
		public void applySpellEffectToFight(Fight fight, Fighter perso,
				Case cell, ArrayList<Case> cells, boolean isCC) {
		...
		public void applySpellEffectToFight(Fight fight, Fighter perso,
			...
			int num = 0;
			for (SpellEffect SE : effets) {
				if (fight.get_state() >= Constants.FIGHT_STATE_FINISHED)
					return;
				if (SE.getChance() != 0 && SE.getChance() != 100)// Si pas 100%
																	// lancement
				{
				...
				ArrayList<Case> cells = Pathfinding.getCellListFromAreaString(
						fight.get_map(), cell.getID(), perso.get_fightCell()
								.getID(), porteeType, POnum, isCC);
				...
				for (Case C : cells) {
					...
					if ((((TE >> 1) & 1) == 1)
							&& (F.getGUID() == perso.getGUID()))
						continue;
					// Ne touche pas les ennemies
					if ((((TE >> 2) & 1) == 1)
							&& (F.getTeam() != perso.getTeam()))
						continue;
	...
	public Sort(int aspellID, String aNombre, int aspriteID,
			String aspriteInfos, String ET) {
		spellID = aspellID;
		nombre = aNombre;
		spriteID = aspriteID;
