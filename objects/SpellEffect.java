...
public class SpellEffect {
	...
	public SpellEffect(int id, int value2, int aduration, int turns2,
			boolean debuff, Fighter aCaster, String args2, int aspell) {
		effectID = id;
		value = value2;
		turns = turns2;
		debuffable = debuff;
	...
	public static ArrayList<Fighter> getTargets(SpellEffect SE, Fight fight,
			ArrayList<Case> cells) {
	...
	public int getMaxMinHechizo(Fighter objetivo, int valor) {
		int val = valor;
		if (objetivo.hasBuff(782)) {
			int max = Integer.parseInt(args.split(";")[1]);
			if (max == -1)
				max = Integer.parseInt(args.split(";")[0]);
			valor = max;
		}
		if (objetivo.hasBuff(781))
	...
	public void applyToFight(Fight fight, Fighter perso, Case Cell,
			ArrayList<Fighter> cibles) {
		cell = Cell;
		applyToFight(fight, perso, cibles, false);
	}

	public static int applyOnHitBuffs(int finalDommage, Fighter target,
			Fighter caster, Fight fight) {
			...
			for (SpellEffect buff : target.getBuffsByEffectID(id)) {
				...
				case 9:// Derobade
					...
					int d = Pathfinding.getDistanceBetween(fight.get_map(),
							target.get_fightCell().getID(), caster
									.get_fightCell().getID());
					if (d > 1)
						continue;
					...
					int newCellID = Pathfinding.newCaseAfterPush(
							fight.get_map(), caster.get_fightCell(),
							target.get_fightCell(), nbrCase);
					if (newCellID < 0)// S'il a ï¿½tï¿½ bloquï¿½
					{
						int a = -newCellID;
						a = nbrCase - a;
						newCellID = Pathfinding.newCaseAfterPush(
								fight.get_map(), caster.get_fightCell(),
								target.get_fightCell(), a);
						if (newCellID == 0)
							continue;
					...
					for (Piege p : P) {
						int dist = Pathfinding.getDistanceBetween(fight
								.get_map(), p.get_cell().getID(), target
								.get_fightCell().getID());
						// on active le piege
						if (dist <= p.get_size())
							p.onTraped(target);
				...
				case 79:// chance ï¿½ca
					try {
						...
						if (jet < chance)// Soin
						{
							...
							if (-finalDommage > (target.getPDVMAX() - target
									.getPDV()))
								finalDommage = -(target.getPDVMAX() - target
										.getPDV());
						} else
							// Dommage
				...
				case 107:// renvoie Dom
					...
					float coef = 1 + (target.getTotalStats().getEffect(
							Constants.STATS_ADD_SAGE) / 100);
					int renvoie = 0;
					try {
						if (Integer.parseInt(args[1]) != -1) {
							renvoie = (int) (coef * Formulas.getRandomValue(
					...
					SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 107,
							"-1", target.getGUID() + "," + renvoie);
				...
				case 788://Chatiments
					...
					if(gain < max)gain = max;
					//on ajoute le buff
					target.addBuff(stat, gain, 10000, 1, false, buff.getSpell(), buff.getArgs(), caster);
					SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, stat, caster.getGUID()+"", target.getGUID()+","+gain+","+10000);
	...
	public void applyToFight(Fight fight, Fighter acaster,
			ArrayList<Fighter> cibles, boolean isCaC) {
		GameServer.addToLog("Effet id: " + effectID + " Args: " + args
				+ " turns: " + turns + " cibles: " + cibles.size()
				+ " chance: " + chance);
		try {
			if (turns != -1)// Si ce n'est pas un buff qu'on applique en dï¿½but
							// de tour
				turns = Integer.parseInt(args.split(";")[3]);
		...
		//if ((fight.get_type() == Constants.FIGHT_TYPE_CHALLENGE)
		//		&& (fight.get_challenges().size() > 0)) {
		//	for (Entry<Integer, Challenge> c : fight.get_challenges()
		//			.entrySet()) {
		//		if (c.getValue() == null)
		//			continue;
		...
		case 163:// - Esquive PM
			...
			break;
		case 164:// Daï¿½os reducidos en x%
		...
		case 219:// Faiblesse % neutre
			...
			break;
		case 220:// Reevnia daï¿½os, Daï¿½os devueltos
		...
		case 800:// glyphe sur sois meme
			applyEffect_800(fight);
			break;
			
		case 801:// Glyphe soin
			applyEffect_801(cibles, fight, effectID);
			break;
		...
		case 671: // Daï¿½os : X% de la vida del atacante (neutre)
		...
		case 765:// sacrifice
			...
			break;
		case 776:// Cura durante el ataque
		...
		default:
			GameServer.addToLog("effet non implante : " + effectID + " args: "
					+ args);
			break;
		}
	}
	...
	private void applyEffect_950(Fight fight, ArrayList<Fighter> cibles) {
		...
		for (Fighter target : cibles) {
			if (spell == 139 && target.getTeam() != caster.getTeam())// Mot
																		// d'altruisme
																		// on
																		// saute
																		// les
			...
			if (turns <= 0) {
				...
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 950,
						caster.getGUID() + "", target.getGUID() + "," + id
								+ ",1");
			} else {
				target.setState(id, turns);
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 950,
						caster.getGUID() + "", target.getGUID() + "," + id
								+ ",1");
				target.addBuff(effectID, value, turns, 1, false, spell, args,
						target);
			}
		}
	}
	...
	private void applyEffect_951(Fight fight, ArrayList<Fighter> cibles) {
		...
		for (Fighter target : cibles) {
			...
			// on envoie le packet
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 950,
					caster.getGUID() + "", target.getGUID() + "," + id + ",0");
		}
	}
	...
	private void applyEffect_50(Fight fight) {
		...
		SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 950,
				target.getGUID() + "", target.getGUID() + ","
						+ Constants.ETAT_PORTE + ",1");
		SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 950,
				caster.getGUID() + "", caster.getGUID() + ","
						+ Constants.ETAT_PORTEUR + ",1");
	...
	private void applyEffect_788(ArrayList<Fighter> cibles, Fight fight) {
		// caster.addBuff(effectID, value, turns, 1, false, spell, args,
		// caster);
		for (Fighter target : cibles) {
			target.addBuff(effectID, value, turns, 1, false, spell, args,
					target);
	...
	private void applyEffect_672(ArrayList<Fighter> cibles, Fight fight)
	{
		...
		double pVie = (double)caster.getPDV();
	...
	private void applyEffect_783(ArrayList<Fighter> cibles, Fight fight) {
		...
		char d = Pathfinding.getDirBetweenTwoCase(ccase.getID(), cell.getID(),
				fight.get_map(), true);
		// On calcule l'id de la case a cotï¿½ du lanceur dans la direction
		// obtenue
		int tcellID = Pathfinding.GetCaseIDFromDirrection(ccase.getID(), d,
				fight.get_map(), true);
		...
		int limite = 0;
		while (true) {
			if (Pathfinding.GetCaseIDFromDirrection(c1, d, fight.get_map(),
					true) == cell.getID())
				break;
			if (Pathfinding.GetCaseIDFromDirrection(c1, d, fight.get_map(),
					true) == -1)
				return;
			c1 = Pathfinding.GetCaseIDFromDirrection(c1, d, fight.get_map(),
					true);
			limite++;
			if (limite > 50)
				return;
		...
		P.addAll(fight.get_traps());
		for (Piege p : P) {
			int dist = Pathfinding.getDistanceBetween(fight.get_map(), p
					.get_cell().getID(), target.get_fightCell().getID());
			// on active le piege
			if (dist <= p.get_size())
				p.onTraped(target);
		}
		SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 5,
				caster.getGUID() + "", target.getGUID() + "," + cell.getID());
	}
	...
	private void applyEffect_784(ArrayList<Fighter> cibles, Fight fight) {
		...
		for (int i = 1; i < list.size(); i++)
			// on boucle si tout le monde est ï¿½ la place
			if (!list.isEmpty()) // d'un autre
				for (Fighter F : list) {
					if (F == null || F.isDead()
							|| !origPos.containsKey(F.getGUID())) {
						continue;
					}
					if (F.get_fightCell().getID() == origPos.get(F.getGUID())
							.getID()) {
						continue;
					}
	...
	private void applyEffect_8(ArrayList<Fighter> cibles, Fight fight) {
		...
		for (Piege p : P) {
			int dist = Pathfinding.getDistanceBetween(fight.get_map(), p
					.get_cell().getID(), target.get_fightCell().getID());
			int dist2 = Pathfinding.getDistanceBetween(fight.get_map(), p
					.get_cell().getID(), caster.get_fightCell().getID());
			// on active le piege
			if (dist <= p.get_size())
				p.onTraped(target);
		...
		SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 4,
				caster.getGUID() + "",
				target.getGUID() + "," + exCaster.getID());
		SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 4,
				caster.getGUID() + "",
				caster.getGUID() + "," + exTarget.getID());
	...
	private void applyEffect_266(Fight fight, ArrayList<Fighter> cibles) {
		...
		int vol = 0;
		for (Fighter target : cibles) {
			target.addBuff(Constants.STATS_REM_CHAN, val, turns, 1, false,
					spell, args, caster);
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
					Constants.STATS_REM_CHAN, caster.getGUID() + "",
					target.getGUID() + "," + val + "," + turns);
			vol += val;
		}
		if (vol == 0)
			return;
		// on ajoute le buff
		caster.addBuff(Constants.STATS_ADD_CHAN, vol, turns, 1, false, spell,
				args, caster);
		SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
				Constants.STATS_ADD_CHAN, caster.getGUID() + "",
				caster.getGUID() + "," + vol + "," + turns);
	}
	...
	private void applyEffect_267(Fight fight, ArrayList<Fighter> cibles) {
		...
		int vol = 0;
		for (Fighter target : cibles) {
			target.addBuff(Constants.STATS_REM_VITA, val, turns, 1, false,
					spell, args, caster);
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
					Constants.STATS_REM_VITA, caster.getGUID() + "",
					target.getGUID() + "," + val + "," + turns);
			vol += val;
		}
		if (vol == 0)
			return;
		// on ajoute le buff
		caster.addBuff(Constants.STATS_ADD_VITA, vol, turns, 1, false, spell,
				args, caster);
		SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
				Constants.STATS_ADD_VITA, caster.getGUID() + "",
				caster.getGUID() + "," + vol + "," + turns);
	}
	...
	private void applyEffect_268(Fight fight, ArrayList<Fighter> cibles) {
		...
		int vol = 0;
		for (Fighter target : cibles) {
			target.addBuff(Constants.STATS_REM_AGIL, val, turns, 1, false,
					spell, args, caster);
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
					Constants.STATS_REM_AGIL, caster.getGUID() + "",
					target.getGUID() + "," + val + "," + turns);
			vol += val;
		}
		if (vol == 0)
			return;
		// on ajoute le buff
		caster.addBuff(Constants.STATS_ADD_AGIL, vol, turns, 1, false, spell,
				args, caster);
		SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
				Constants.STATS_ADD_AGIL, caster.getGUID() + "",
				caster.getGUID() + "," + vol + "," + turns);
	}
	...
	private void applyEffect_269(Fight fight, ArrayList<Fighter> cibles) {
		...
		int vol = 0;
		for (Fighter target : cibles) {
			target.addBuff(Constants.STATS_REM_INTE, val, turns, 1, false,
					spell, args, caster);
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
					Constants.STATS_REM_INTE, caster.getGUID() + "",
					target.getGUID() + "," + val + "," + turns);
			vol += val;
		}
		if (vol == 0)
			return;
		// on ajoute le buff
		caster.addBuff(Constants.STATS_ADD_INTE, vol, turns, 1, false, spell,
				args, caster);
		SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
				Constants.STATS_ADD_INTE, caster.getGUID() + "",
				caster.getGUID() + "," + vol + "," + turns);
	}
	...
	private void applyEffect_270(Fight fight, ArrayList<Fighter> cibles) {
		...
		int vol = 0;
		for (Fighter target : cibles) {
			target.addBuff(Constants.STATS_REM_SAGE, val, turns, 1, false,
					spell, args, caster);
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
					Constants.STATS_REM_SAGE, caster.getGUID() + "",
					target.getGUID() + "," + val + "," + turns);
			vol += val;
		}
		if (vol == 0)
			return;
		// on ajoute le buff
		caster.addBuff(Constants.STATS_ADD_SAGE, vol, turns, 1, false, spell,
				args, caster);
		SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
				Constants.STATS_ADD_SAGE, caster.getGUID() + "",
				caster.getGUID() + "," + vol + "," + turns);
	}
	...
	private void applyEffect_271(Fight fight, ArrayList<Fighter> cibles) {
		...
		int vol = 0;
		for (Fighter target : cibles) {
			target.addBuff(Constants.STATS_REM_FORC, val, turns, 1, false,
					spell, args, caster);
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
					Constants.STATS_REM_FORC, caster.getGUID() + "",
					target.getGUID() + "," + val + "," + turns);
			vol += val;
		}
		if (vol == 0)
			return;
		// on ajoute le buff
		caster.addBuff(Constants.STATS_ADD_FORC, vol, turns, 1, false, spell,
				args, caster);
		SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
				Constants.STATS_ADD_FORC, caster.getGUID() + "",
				caster.getGUID() + "," + vol + "," + turns);
	}
	...
	private void applyEffect_210(Fight fight, ArrayList<Fighter> cibles) {
		...
		if (val == -1) {
			GameServer
					.addToLog("Erreur de valeur pour getRandomJet (applyEffect_"
							+ effectID + ")");
			return;
		}
		for (Fighter target : cibles) {
	...
	private void applyEffect_265(Fight fight, ArrayList<Fighter> cibles) {
		...
		for (Fighter target : cibles) {
			...
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, effectID,
					caster.getGUID() + "", target.getGUID() + "," + val + ","
							+ turns);
		}
	}
		...
		for (Fighter target : cibles) {
			if (spell == 686) {
				if (target.getPersonnage() != null
						&& target.getPersonnage().get_sexe() == 1
						|| target.getMob() != null
			...
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, effectID,
					caster.getGUID() + "", target.getGUID() + "," + defaut
							+ "," + id + "," + turns);
		}
	}
	...
	private void applyEffect_176(ArrayList<Fighter> objetivos, Fight pelea) {
		...
		try {
			Thread.sleep(200);// un tiempo de pausa de un quinto de segundo
		} catch (InterruptedException e1) {
			e1.printStackTrace();
		}
	}
	...
	private void applyEffect_150(Fight fight, ArrayList<Fighter> cibles) {
		if (turns == 0)
			return;
		for (Fighter target : cibles) {
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 150,
					caster.getGUID() + "", target.getGUID() + ",4");
	...
	private void applyEffect_402(Fight fight) {
		...
		String po = World.getSort(spell).getStatsByLevel(spellLvl)
				.getPorteeType();
	...
	private void applyEffect_800(Fight fight) {
	...
	private void applyEffect_801(ArrayList<Fighter> cibles, Fight fight, int jet) {
		
		
		
			for (Fighter target : cibles) {
				int soins = target.getTotalStats().getEffect(Constants.STATS_ADD_SOIN);
				int statC = target.getTotalStats().getEffect(Constants.STATS_ADD_INTE);
				if (statC < 0)
					statC = 0;
				int finalSoin = (jet * (100 + statC) / 10000 + soins);
				
				if ((value - finalSoin) > 0)
					SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 100,
							target.getGUID() + "", target.getGUID() + ","
									+ (value - finalSoin));
				if (finalSoin > 0){
					...
					if (turns <= 1 || duration <= 1){
						int dif = (target.getPDVMAX() - target.getPDV());	
						if (dif < finalSoin) {
	...
	private void applyEffect_116(ArrayList<Fighter> cibles, Fight fight)// Malus
																		// PO
	{
		int val = Formulas.getRandomJet(jet);
		if (val == -1) {
	...
	private void applyEffect_117(ArrayList<Fighter> cibles, Fight fight)// Bonus
																		// PO
	{
		int val = Formulas.getRandomJet(jet);
		if (val == -1) {
	...
	private void applyEffect_118(ArrayList<Fighter> cibles, Fight fight)// Bonus
																		// Force
	{
		int val = Formulas.getRandomJet(jet);
		if (val == -1) {
	...
	private void applyEffect_119(ArrayList<Fighter> cibles, Fight fight)// Bonus
																		// Agilitï¿½
	{
		int val = Formulas.getRandomJet(jet);
		if (val == -1) {
	...
	private void applyEffect_120(ArrayList<Fighter> cibles, Fight fight)// Bonus
																		// PA
	{
		int val = Formulas.getRandomJet(jet);
		if (val == -1) {
		...
		SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, effectID,
				caster.getGUID() + "", caster.getGUID() + "," + val + ","
						+ turns);
	}

	private void applyEffect_78(ArrayList<Fighter> cibles, Fight fight)// Bonus
																		// PA
	{
		int val = Formulas.getRandomJet(jet);
		if (val == -1) {
	...
	private void applyEffect_180(Fight fight)// invocation
	{
		...
		for (Piege p : P) {
			int dist = Pathfinding.getDistanceBetween(fight.get_map(), p
					.get_cell().getID(), F.get_fightCell().getID());
			// on active le piege
			if (dist <= p.get_size())
				p.onTraped(F);
		}
	...
	private void applyEffect_111(ArrayList<Fighter> cibles, Fight fight) {
		...
		for (Fighter target : cibles) {
			...
			if (target.canPlay() && target == caster)
				target.setCurPA(fight, target.getPA() + val);
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, effectID,
					caster.getGUID() + "", target.getGUID() + "," + val + ","
							+ turns);
		}
	...
	private void applyEffect_128(ArrayList<Fighter> cibles, Fight fight) {
		...
		for (Fighter target : cibles) {
			...
			if (target.canPlay() && target == caster)
				target.setCurPM(fight, target.getPM() + val);
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, effectID,
					caster.getGUID() + "", target.getGUID() + "," + val + ","
							+ turns);
		}
	...
	private void applyEffect_152(Fight pelea, ArrayList<Fighter> objetivos) {
		...
		if (val == -1) {
			System.out
					.println("Error de valor por getRandomJet (aplicarEfecto_"
							+ effectID + ")");
			return;
		}
		int val2 = val;
	...
	private void applyEffect_186(Fight fight, ArrayList<Fighter> cibles) {
		int val = Formulas.getRandomJet(jet);
		if (val == -1) {
			System.out.println("Error de valor por getRandomJet (applyEffect_"
					+ effectID + ")");
			return;
		}
		int val2 = val;
	...
	private void applyEffect_671(ArrayList<Fighter> objetivos, Fight pelea) {
			if (turns <= 0) {
				...
				int dano = Formulas.getRandomJet(args.split(";")[5]);// % de pdv
																		// infligido
				dano = getMaxMinHechizo(objetivo, dano);
				int val = caster.getPDV() / 100 * dano;// Valor de daï¿½os
				val -= resF;
	...
	private void applyEffect_143(ArrayList<Fighter> objetivos, Fight pelea) {
		if (turns <= 0) {
			...
			if (jet.length < 6) {
				...
				cura = Formulas.getRandomJet(jet[5]);
			}
			int dmg2 = cura;
			for (Fighter objetivo : objetivos) {
				cura = getMaxMinHechizo(objetivo, cura);
				int curaFinal = Formulas.calculFinalCura(caster, cura);
				if ((curaFinal + objetivo.getPDV()) > objetivo.getPDVMAX())
					curaFinal = objetivo.getPDVMAX() - objetivo.getPDV();
				if (curaFinal < 1)
					curaFinal = 0;
				objetivo.removePDV(-curaFinal);
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(pelea, 7, 108,
						caster.getGUID() + "", objetivo.getGUID() + ","
								+ curaFinal);
				cura = dmg2;
			}
		} else {
	...
	private void applyEffect_115(ArrayList<Fighter> cibles, Fight fight) {
		...
		if (val == -1) {
			GameServer
					.addToLog("Erreur de valeur pour getRandomJet (applyEffect_115)");
			return;
		}
		for (Fighter target : cibles) {
			target.addBuff(effectID, val, turns, 1, false, spell, args, caster);
	...
	private void applyEffect_77(ArrayList<Fighter> cibles, Fight fight) {
		...
		for (Fighter target : cibles) {
			...
			if (val < 1)
				continue;
			target.addBuff(Constants.STATS_REM_PM, val, turns, 0, true, spell,
					args, caster);
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
					Constants.STATS_REM_PM, caster.getGUID() + "",
					target.getGUID() + ",-" + val + "," + turns);
			num += val;
		}
		if (num != 0) {
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
					Constants.STATS_ADD_PM, caster.getGUID() + "",
					caster.getGUID() + "," + num + "," + turns);
			caster.addBuff(Constants.STATS_ADD_PM, num, 0, 0, true, spell,
					args, caster);
			// Gain de PM pendant le tour de jeu
			if (caster.canPlay())
				caster.setCurPM(fight, caster.getPM() + num);
		}
	}
	...
	private void applyEffect_84(ArrayList<Fighter> cibles, Fight fight) {
		...
		for (Fighter target : cibles) {
			...
			if (val < 1)
				continue;
			if (spell == 95) {
				target.addBuff(Constants.STATS_REM_PA, val, 1, 1, true, spell,
						args, caster);
			} else {
				target.addBuff(Constants.STATS_REM_PA, val, turns, 0, true,
						spell, args, caster);
			}
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
					Constants.STATS_REM_PA, caster.getGUID() + "",
					target.getGUID() + ",-" + val + "," + turns);
			num += val;
		}
		if (num != 0) {
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
					Constants.STATS_ADD_PA, caster.getGUID() + "",
					caster.getGUID() + "," + num + "," + turns);
			caster.addBuff(Constants.STATS_ADD_PA, num, 0, 0, true, spell,
					args, caster);
			// Gain de PA pendant le tour de jeu
			if (caster.canPlay())
				caster.setCurPA(fight, caster.getPA() + num);
		}
	}
	...
	private void applyEffect_168(Fight fight, ArrayList<Fighter> cibles) {
		if (turns <= 0) {
			for (Fighter target : cibles) {
				target.addBuff(effectID, value, 1, 1, false, spell, args,
						caster);
				if (turns <= 1 || duration <= 1)
			...
			for (Fighter target : cibles) {
				if (spell == 197 || spell == 112) {
					target.addBuff(effectID, value, turns, turns, false, spell,
							args, caster);
				} else if (spell == 115)// Odorat
				{
					...
					target.addBuff(effectID, lostPa, turns, turns, false,
							spell, args, caster);
					if (turns <= 1 || duration <= 1)
						SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
								168, target.getGUID() + "", target.getGUID()
										+ ",-" + lostPa);
				} else {
					target.addBuff(effectID, value, 1, 1, false, spell, args,
							caster);
	...
	private void applyEffect_169(Fight fight, ArrayList<Fighter> cibles) {
		if (turns <= 0) {
			...
			if (cibles.isEmpty() && spell == 120
					&& caster.get_oldCible() != null) {
				caster.get_oldCible().addBuff(effectID, value, turns, turns,
						false, spell, args, caster);
				if (turns <= 1 || duration <= 1)
					SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 169,
							caster.get_oldCible().getGUID() + "", caster
									.get_oldCible().getGUID() + ",-" + value);
			}
			for (Fighter target : cibles) {
				if (spell == 192)// Ronce apaisante
				{
					target.addBuff(effectID, value, turns, 0, debuffable,
							spell, args, caster);
				} else if (spell == 197) {
					target.addBuff(effectID, value, turns, turns, false, spell,
							args, caster);
				} else if (spell == 115)// Odorat
				{
					...
					target.addBuff(effectID, lostPm, turns, turns, false,
							spell, args, caster);
					if (turns <= 1 || duration <= 1)
						SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
								169, target.getGUID() + "", target.getGUID()
										+ ",-" + lostPm);
				} else {
					target.addBuff(effectID, value, 1, 1, false, spell, args,
							caster);
	...
	private void applyEffect_101(ArrayList<Fighter> cibles, Fight fight) {
		if (turns <= 0) {
			for (Fighter target : cibles) {
				int retrait = Formulas
						.getPointsLost('a', value, caster, target);
				if ((value - retrait) > 0)
					SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 308,
							caster.getGUID() + "", target.getGUID() + ","
									+ (value - retrait));
				if (retrait > 0) {
					target.addBuff(Constants.STATS_REM_PA, retrait, 1, 1,
							false, spell, args, caster);
					if (turns <= 1 || duration <= 1)
						SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
								101, target.getGUID() + "", target.getGUID()
										+ ",-" + retrait);
				}
			}
		} else {
			for (Fighter target : cibles) {
				...
				if (retrait > 0) {
					if (spell == 89) {
						target.addBuff(effectID, retrait, 0, 1, false, spell,
								args, caster);
					} else {
						target.addBuff(effectID, retrait, 1, 1, false, spell,
								args, caster);
					}
					if (turns <= 1 || duration <= 1)
						SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
	...
	private void applyEffect_127(ArrayList<Fighter> cibles, Fight fight) {
		if (turns <= 0) {
			for (Fighter target : cibles) {
				int retrait = Formulas
						.getPointsLost('m', value, caster, target);
				if ((value - retrait) > 0)
					SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 309,
							caster.getGUID() + "", target.getGUID() + ","
									+ (value - retrait));
				if (retrait > 0) {
					target.addBuff(Constants.STATS_REM_PM, retrait, 1, 1,
							false, spell, args, caster);
					if (turns <= 1 || duration <= 1)
						SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
								127, target.getGUID() + "", target.getGUID()
										+ ",-" + retrait);
				}
			}
		} else {
			for (Fighter target : cibles) {
				...
				if (retrait > 0) {
					...
					{
						target.addBuff(effectID, retrait, turns, turns, false,
								spell, args, caster);
					} else {
						target.addBuff(effectID, retrait, 1, 1, false, spell,
								args, caster);
	...
	private void applyEffect_107(ArrayList<Fighter> cibles, Fight fight) {
		if (turns < 1)
			...
			for (Fighter target : cibles) {
				target.addBuff(effectID, 0, turns, 0, true, spell, args, caster);// on
																					// applique
																					// un
																					// buff
			}
	...
	private void applyEffect_79(ArrayList<Fighter> cibles, Fight fight) {
		if (turns < 1)
			...
			for (Fighter target : cibles) {
				target.addBuff(effectID, -1, turns, 0, true, spell, args,
						caster);// on applique un buff
			}
		}
	}
	...
	private void applyEffect_4(Fight fight, ArrayList<Fighter> cibles) {
		...
		if (cell.isWalkable(true) && !fight.isOccuped(cell.getID()))// Si la
																	// case est
																	// prise, on
																	// va ï¿½viter
																	// que les
																	...
																	// *-*
		{
			...
			GameServer
					.addToLog("Tentative de teleportation echouee : case non libre:");
	...
	private void applyEffect_765B(Fight fight, Fighter target) {
		...
		target.get_fightCell().addFighter(target);
		SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 4,
				target.getGUID() + "", target.getGUID() + "," + cell1.getID());
		SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 4,
				sacrified.getGUID() + "",
				sacrified.getGUID() + "," + cell2.getID());
	...
	private void applyEffect_109(Fight fight)// Dommage pour le lanceur (fixes)
	{
		if (turns <= 0) {
			...
			int finalDommage = Formulas.calculFinalDommage(fight, caster,
					caster, Constants.ELEMENT_NULL, dmg, false, false, spell);

			finalDommage = applyOnHitBuffs(finalDommage, caster, caster, fight);// S'il
																				// y
																				// a
																				// des
																				// buffs
			...
			caster.addBuff(effectID, 0, turns, 0, true, spell, args, caster);// on
																				// applique
																				// un
																				// buff
		}
	...
	private void applyEffect_82(ArrayList<Fighter> cibles, Fight fight) {
		if (turns <= 0) {
			for (Fighter target : cibles) {
				...
				int dmg = Formulas.getRandomJet(args.split(";")[5]);
				// si la cible a le buff renvoie de sort et que le sort peut
				// etre renvoyer
				if (target.hasBuff(106) && target.getBuffValue(106) >= spellLvl
						&& spell != 0) {
					SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 106,
							target.getGUID() + "", target.getGUID() + ",1");
					// le lanceur devient donc la cible
					target = caster;
				}
				int finalDommage = Formulas.calculFinalDommage(fight, caster,
						target, Constants.ELEMENT_NULL, dmg, false, false,
						spell);

				finalDommage = applyOnHitBuffs(finalDommage, target, caster,
						fight);// S'il y a des buffs spï¿½ciaux
				...
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 100,
						target.getGUID() + "", caster.getGUID() + "," + heal);
	...
	private void applyEffect_6(ArrayList<Fighter> cibles, Fight fight) {
		if (turns <= 0) {
			for (Fighter target : cibles) {
				...
				int newCellID = Pathfinding.newCaseAfterPush(fight.get_map(),
						eCell, target.get_fightCell(), -value);
				if (newCellID == 0)
					return;
				...
				if (newCellID < 0)// S'il a ï¿½tï¿½ bloquï¿½
				{
					...
					newCellID = Pathfinding.newCaseAfterPush(fight.get_map(),
							caster.get_fightCell(), target.get_fightCell(), a);
					if (newCellID == 0)
						return;
					if (fight.get_map().getCase(newCellID) == null)
						return;
				...
				for (Piege p : P) {
					int dist = Pathfinding.getDistanceBetween(fight.get_map(),
							p.get_cell().getID(), target.get_fightCell()
									.getID());
					// on active le piege
					if (dist <= p.get_size())
						p.onTraped(target);
			...
			for (Fighter target : cibles) {
				...
				if (newCellID < 0)// S'il a ï¿½tï¿½ bloquï¿½
				{
					...
					if (target.hasBuff(184)) {
						finalDommage = finalDommage
								- target.getBuff(184).getValue();// Rï¿½duction
																	// physique
						SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
								105, caster.getGUID() + "", target.getGUID()
										+ "," + target.getBuff(184).getValue());
					}
					if (target.hasBuff(105)) {
						finalDommage = finalDommage
								- target.getBuff(105).getValue();// Immu
						SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
								105, caster.getGUID() + "", target.getGUID()
										+ "," + target.getBuff(105).getValue());
					}
					if (finalDommage > 0) {
						target.removePDV(finalDommage);
						SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
								100, caster.getGUID() + "", target.getGUID()
										+ ",-" + finalDommage);
						if (target.getPDV() <= 0) {
	...
	private void applyEffect_91(ArrayList<Fighter> cibles, Fight fight,
		...
		if (isCaC) {
			for (Fighter target : cibles) {
				...
				int dmg = Formulas.getRandomJet(args.split(";")[5]);
				int finalDommage = Formulas.calculFinalDommage(fight, caster,
						target, Constants.ELEMENT_EAU, dmg, false, true, spell);
			...
			for (Fighter target : cibles) {
				...
				int dmg = Formulas.getRandomJet(args.split(";")[5]);
				int finalDommage = Formulas
						.calculFinalDommage(fight, caster, target,
								Constants.ELEMENT_EAU, dmg, false, false, spell);
	...
	private void applyEffect_92(ArrayList<Fighter> cibles, Fight fight,
		...
		if (isCaC) {
			for (Fighter target : cibles) {
				...
				int dmg = Formulas.getRandomJet(args.split(";")[5]);
				int finalDommage = Formulas.calculFinalDommage(fight, caster,
						target, Constants.ELEMENT_TERRE, dmg, false, true,
						spell);
			...
			for (Fighter target : cibles) {
				...
				int dmg = Formulas.getRandomJet(args.split(";")[5]);
				int finalDommage = Formulas.calculFinalDommage(fight, caster,
						target, Constants.ELEMENT_TERRE, dmg, false, false,
						spell);
	...
	private void applyEffect_93(ArrayList<Fighter> cibles, Fight fight,
		...
		if (isCaC) {
			for (Fighter target : cibles) {
				...
				int dmg = Formulas.getRandomJet(args.split(";")[5]);
				int finalDommage = Formulas.calculFinalDommage(fight, caster,
						target, Constants.ELEMENT_AIR, dmg, false, true, spell);
			...
			for (Fighter target : cibles) {
				...
				int finalDommage = Formulas
						.calculFinalDommage(fight, caster, target,
								Constants.ELEMENT_AIR, dmg, false, false, spell);
	...
	private void applyEffect_94(ArrayList<Fighter> cibles, Fight fight,
		...
		if (isCaC)// CaC Eau
		{
			for (Fighter target : cibles) {
				...
				int dmg = Formulas.getRandomJet(args.split(";")[5]);
				int finalDommage = Formulas.calculFinalDommage(fight, caster,
						target, Constants.ELEMENT_FEU, dmg, false, true, spell);
			...
			for (Fighter target : cibles) {
				...
				int finalDommage = Formulas
						.calculFinalDommage(fight, caster, target,
								Constants.ELEMENT_FEU, dmg, false, false, spell);
	...
	private void applyEffect_95(ArrayList<Fighter> cibles, Fight fight,
		...
		if (isCaC)// CaC Eau
		{
			for (Fighter target : cibles) {
				...
				int dmg = Formulas.getRandomJet(args.split(";")[5]);
				int finalDommage = Formulas.calculFinalDommage(fight, caster,
						target, Constants.ELEMENT_NEUTRE, dmg, false, true,
						spell);
			...
			for (Fighter target : cibles) {
				...
				int dmg = Formulas.getRandomJet(args.split(";")[5]);
				int finalDommage = Formulas.calculFinalDommage(fight, caster,
						target, Constants.ELEMENT_NEUTRE, dmg, false, false,
						spell);
	...
	private void applyEffect_85(ArrayList<Fighter> cibles, Fight fight) {
		if (turns <= 0) {
			for (Fighter target : cibles) {
				...
				int resP = target.getTotalStats().getEffect(
						Constants.STATS_ADD_RP_EAU);
				int resF = target.getTotalStats().getEffect(
						Constants.STATS_ADD_R_EAU);
				if (target.getPersonnage() != null)// Si c'est un joueur, on
													// ajoute les resists
													// bouclier
				{
					resP += target.getTotalStats().getEffect(
							Constants.STATS_ADD_RP_PVP_EAU);
					resF += target.getTotalStats().getEffect(
							Constants.STATS_ADD_R_PVP_EAU);
				}
				int dmg = Formulas.getRandomJet(args.split(";")[5]);// %age de
																	// pdv
																	// infligï¿½
				...
				val = applyOnHitBuffs(val, target, caster, fight);// S'il y a
																	// des buffs
																	// spï¿½ciaux
	...
	private void applyEffect_86(ArrayList<Fighter> cibles, Fight fight) {
		if (turns <= 0) {
			for (Fighter target : cibles) {
				...
				int resP = target.getTotalStats().getEffect(
						Constants.STATS_ADD_RP_TER);
				int resF = target.getTotalStats().getEffect(
						Constants.STATS_ADD_R_TER);
				if (target.getPersonnage() != null)// Si c'est un joueur, on
													// ajoute les resists
													// bouclier
				{
	...
	private void applyEffect_87(ArrayList<Fighter> cibles, Fight fight) {
		if (turns <= 0) {
			for (Fighter target : cibles) {
				...
				int resP = target.getTotalStats().getEffect(
						Constants.STATS_ADD_RP_AIR);
				int resF = target.getTotalStats().getEffect(
						Constants.STATS_ADD_R_AIR);
				if (target.getPersonnage() != null)// Si c'est un joueur, on
													// ajoute les resists
													// bouclier
				{
	...
	private void applyEffect_88(ArrayList<Fighter> cibles, Fight fight) {
		if (turns <= 0) {
			for (Fighter target : cibles) {
				...
				int resP = target.getTotalStats().getEffect(
						Constants.STATS_ADD_RP_FEU);
				int resF = target.getTotalStats().getEffect(
						Constants.STATS_ADD_R_FEU);
				if (target.getPersonnage() != null)// Si c'est un joueur, on
													// ajoute les resists
													// bouclier
				{
	...
	private void applyEffect_97(ArrayList<Fighter> cibles, Fight fight,
		...
		if (isCaC)// CaC Terre
		{
			...
			for (Fighter target : cibles) {
				...
				} else if (chance > 0 && spell == 108)// Esprit fï¿½lin ?
				{
					int fDommage = Formulas.calculFinalDommage(fight, caster,
							caster, Constants.ELEMENT_TERRE, dmg, false, false,
							spell);
					fDommage = applyOnHitBuffs(fDommage, caster, caster, fight);// S'il
																				// y
																				// a
																				// des
																				// buffs
					...
					int finalDommage = Formulas.calculFinalDommage(fight,
							caster, target, Constants.ELEMENT_TERRE, dmg,
							false, false, spell);
					finalDommage = applyOnHitBuffs(finalDommage, target,
							caster, fight);// S'il y a des buffs spï¿½ciaux
					if (finalDommage > target.getPDV())
						finalDommage = target.getPDV();// Target va mourrir
					target.removePDV(finalDommage);
					int cura = finalDommage;
					if (target.hasBuff(786)) {
						...
						caster.removePDV(-cura);
						SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
								100, target.getGUID() + "", caster.getGUID()
										+ ",+" + cura);
					}
					finalDommage = -(finalDommage);
					SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 100,
	...
	private void applyEffect_99(ArrayList<Fighter> cibles, Fight fight,
		...
		if (isCaC)// CaC Feu
		{
			...
			for (Fighter target : cibles) {
				if (spell == 36 && target == caster)// Frappe du Craqueleur ne
													// tape pas l'osa
				{
					continue;
				}
	...
	private void applyEffect_90(ArrayList<Fighter> cibles, Fight fight) {
		if (turns <= 0)// Si Direct
		{
			...
			int finalDommage = applyOnHitBuffs(val, caster, caster, fight);// S'il
																			// y
																			// a
																			// des
																			// buffs
			...
			for (Fighter target : cibles) {
				if ((val + target.getPDV()) > target.getPDVMAX())
					val = target.getPDVMAX() - target.getPDV();// Target va
																// mourrir
				target.removePDV(-val);
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 100,
						caster.getGUID() + "", target.getGUID() + ",+" + val);
	...
	private void applyEffect_108(ArrayList<Fighter> cibles, Fight fight) {
		...
		if (turns <= 0) {
			...
			for (Fighter target : cibles) {
				...
				if (spell == 59 && Integer.parseInt(args.split(";")[0]) == 200
						&& caster.getTeam() != target.getTeam()) {
					continue;
				}
				if (spell == 59 && Integer.parseInt(args.split(";")[0]) == 100
						&& caster.getTeam() == target.getTeam()) {
					continue;
				}
				int finalSoin = Formulas.calculFinalDommage(fight, caster,
						target, Constants.ELEMENT_FEU, dmg, true, false, spell);
	...
	private void applyEffect_320(Fight fight, ArrayList<Fighter> cibles) {
		...
		int num = 0;
		for (Fighter target : cibles) {
			target.addBuff(Constants.STATS_REM_PO, value, turns, 0, true,
					spell, args, caster);
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
					Constants.STATS_REM_PO, caster.getGUID() + "",
					target.getGUID() + "," + value + "," + turns);
			num += value;
		}
		if (num != 0) {
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
					Constants.STATS_ADD_PO, caster.getGUID() + "",
					caster.getGUID() + "," + num + "," + turns);
			caster.addBuff(Constants.STATS_ADD_PO, num, 1, 0, true, spell,
					args, caster);
			// Gain de PO pendant le tour de jeu
			if (caster.canPlay())
	...
	private void applyEffect_780(Fight fight) {
		...
		if (!target.isInvocation())
			...
			fight.get_ordreJeu().add(
					(fight.get_ordreJeu().indexOf(target.getInvocator()) + 1),
					target);
