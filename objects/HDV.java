...
public class HDV {
	...
	private class Categorie {
		Map<Integer, Template> _templates = new HashMap<Integer, Template>();// Dans
																				// le
																				// format
																				// <templateID,Template>
		...
		public boolean delEntry(HdvEntry toDel) {
			...
			_templates.get(toDel.getObjet().getTemplate().getID()).delEntry(
					toDel);

			if ((toReturn = _templates.get(
					toDel.getObjet().getTemplate().getID()).isEmpty()))
	...
	private class Template {
		...
		public void addEntry(HdvEntry toAdd) {
			...
			for (Ligne curLine : _lignes.values())// Boucle dans toutes les
													// lignes pour essayer de
													// trouver des objets de
													// m�mes stats
			{
				if (curLine.addEntry(toAdd))// Si une ligne l'accepte, arr�te la
											// m�thode.
					return;
			}
		...
		public boolean delEntry(HdvEntry toDel) {
			...
			if (_lignes.get(toDel.getLigneID()).isEmpty())// Si la ligne est
															// devenue vide
			{
	...
	public class Ligne {
		private int ligneID;
		private ArrayList<ArrayList<HdvEntry>> _entries = new ArrayList<ArrayList<HdvEntry>>(
				3);// La premi�re ArrayList est un tableau de 3 (0=1 1=10 2=100
					// de quantit�)
		...
		public Ligne(int ligneID, HdvEntry toAdd) {
			...
			for (int i = 0; i < 3; i++) {
				_entries.add(new ArrayList<HdvEntry>());// Boucle 3 fois pour
														// ajouter 3 List vide
														// dans la SuperList
			}
			addEntry(toAdd);
		...
		public boolean haveSameStats(HdvEntry toAdd) {
			return _strStats.equalsIgnoreCase(toAdd.getObjet().parseToSave())
					&& toAdd.getObjet().getTemplate().getType() != 85;// R�cup�re
																		// les
																		// stats
																		// de
																		// l'objet
		...
		public int[] getFirsts() {
			...
			for (int i = 0; i < _entries.size(); i++) {
				try {
					toReturn[i] = _entries.get(i).get(0).getPrice();// R�cup�re
																	// le
																	// premier
																	// objet de
																	// chaque
		...
		public ArrayList<HdvEntry> getAll() {
			...
			int totalSize = _entries.get(0).size() + _entries.get(1).size()
					+ _entries.get(2).size();
			ArrayList<HdvEntry> toReturn = new ArrayList<HdvEntry>(totalSize);

			for (int qte = 0; qte < _entries.size(); qte++)// Boucler dans les
															// quantit�
			{
		...
		public String parseToEHl() {
			...
			int[] price = getFirsts();
			toReturn.append(ligneID).append(";").append(_strStats).append(";")
					.append((price[0] == 0 ? "" : price[0])).append(";")
					.append((price[1] == 0 ? "" : price[1])).append(";")
		...
		public String parseToEHm() {
			...
			int[] prix = getFirsts();
			toReturn.append(ligneID).append("|").append(templateID).append("|")
					.append(_strStats).append("|")
					.append((prix[0] == 0 ? "" : prix[0])).append("|")
					.append((prix[1] == 0 ? "" : prix[1])).append("|")
					.append((prix[2] == 0 ? "" : prix[2]));
		...
		public boolean isEmpty() {
			for (int i = 0; i < _entries.size(); i++) {
				try {
					if (_entries.get(i).get(0) != null)// V�rifie s'il existe un
														// objet dans chacune
														// des 3 quantit�
						return false;
				} catch (IndexOutOfBoundsException e) {
	...
	/**
	 * Contient toutes les informations necessaire sur la vente d'un objet. -Son
	 * prix -Sa quantit� -Le nombres d'heures depuis la mise en vente -Le
	 * propri�taire -Une r�f�rence vers l'objet � vendre.
	 * 
	 * @author Mathieu
	 * 
	 */
	public static class HdvEntry implements Comparable<HdvEntry> {
		...
		public HdvEntry(int price, byte amount, int owner, Objet obj) {
			...
			this._obj = obj;
			this._owner = owner;
			// TODO : Ajouter le nouvel objet dans la bonne categorie et le bon
			// template
		}
		...
		public String parseToEL() {
			...
			int count = getAmount(true);// Transf�re dans le format (1,10,100)
										// le montant qui etait dans le format
										// (1,2,3)
			toReturn.append(_ligneID).append(";").append(count).append(";")
					.append(_obj.getTemplate().getID()).append(";")
					.append(_obj.parseStatsString()).append(";").append(_price)
		...
		public String parseToEmK() {
			...
			int count = getAmount(true);// Transf�re dans le format (1,10,100)
										// le montant qui etait dans le format
										// (1,2,3)
			toReturn.append(_obj.getGuid()).append("|").append(count)
					.append("|").append(_obj.getTemplate().getID()).append("|")
					.append(_obj.parseStatsString()).append("|").append(_price)
					.append("|350");// 350 = temps restant
		...
		/*
		 * public String parseItem(char divider) { int count =
		 * getAmount(true);//Transf�re dans le format (1,10,100) le montant qui
		 * etait dans le format (1,2,3) return
		 * _ligneID+divider+count+divider+_obj
		 * .getTemplate().getID()+divider+_obj
		 * .parseStatsString()+divider+_price+divider+"350";//350 = temps
		 * restant }
		 */
	...
	public HDV(int hdvID, float taxe, short sellTime, short maxItemCompte,
			short lvlMax, String categories) {
		this._hdvID = hdvID;
		this._taxe = taxe;
	...
	public Ligne getLigne(int ligneID) {
		try {
			...
			return _categories.get(categ).getTemplate(template)
					.getLigne(ligneID);
	...
	public void addEntry(HdvEntry toAdd) {
		...
		_path.put(toAdd.getLigneID(), new Couple<Integer, Integer>(categ,
				template));
	...
	public boolean delEntry(HdvEntry toDel) {
		boolean toReturn = _categories.get(
				toDel.getObjet().getTemplate().getType()).delEntry(toDel);
		if (toReturn) {
	...
	public synchronized boolean buyItem(int ligneID, byte amount, int price,
		...
		try {
			...
			newOwner.addKamas(price * -1);// Retire l'argent � l'acheteur (prix
											// et taxe de vente)

			if (toBuy.getOwner() != -1) {
				Compte C = World.getCompte(toBuy.getOwner());
				if (C != null) {
					C.setBankKamas(C.getBankKamas() + toBuy.getPrice());// Ajoute
																		// l'argent
																		// au
																		// vendeur
				}
			}
			SocketManager.GAME_SEND_STATS_PACKET(newOwner);// Met a jour les
															// kamas de
															// l'acheteur

			newOwner.addObjet(toBuy.getObjet(), true);// Ajoute l'objet au
														// nouveau propri�taire
			toBuy.getObjet().getTemplate()
					.newSold(toBuy.getAmount(true), price);// Ajoute la ventes
															// au statistiques

			delEntry(toBuy);// Retire l'item de l'HDV ainsi que de la liste du
							// vendeur

			if (World.getCompte(toBuy.getOwner()) != null
					&& World.getCompte(toBuy.getOwner()).get_curPerso() != null) {
				SocketManager
						.GAME_SEND_Im_PACKET(World.getCompte(toBuy.getOwner())
								.get_curPerso(), "065;" + price + "~"
								+ toBuy.getObjet().getTemplate().getID() + "~"
								+ toBuy.getObjet().getTemplate().getID() + "~1");
				// Si le vendeur est connecter, envoie du packet qui lui annonce
				// la vente de son objet
			}
			if (toBuy.getOwner() == -1) {
