	...
	public void apply(Personnage perso, Personnage target, int itemID,
		...
		case -3:// Lanzar una mision
			if (Ancestra.SHOW_RECV)
				System.out.println("Action imposible( Lanc� un mision)");
			break;
		case -2:// cr�er guilde
			if (perso.is_away())
				return;
			...
			if (perso.hasItemGuid(1575)) {
				...
				SocketManager
						.GAME_SEND_MESSAGE(
								perso,
								"Pour pouvoir cr�er une guilde, il faut poss�der une Guildalogemme",
		...
		case -1:// Ouvrir banque
			...
			if (cost > 0) {
				...
				if (kamasRemaining < 0)// Si le joueur n'a pas assez de kamas
										// SUR LUI pour ouvrir la banque
				{
					if (bankKamas >= cost) {
						perso.setBankKamas(bankKamas - cost); // On modifie les
																// kamas de la
																// banque
					} else if (totalKamas >= cost) {
						perso.set_kamas(0); // On puise l'enti�reter des kamas
											// du joueurs. Ankalike ?
						perso.setBankKamas(totalKamas - cost); // On modifie les
																// kamas de la
																// banque
						SocketManager.GAME_SEND_STATS_PACKET(perso);
						SocketManager.GAME_SEND_Im_PACKET(perso, "020;"
								+ playerKamas);
					} else {
						SocketManager.GAME_SEND_MESSAGE_SERVER(perso, "10|"
								+ cost);
						return;
					}
				} else // Si le joueur a les kamas sur lui on lui retire
						// directement
				{
			...
			SocketManager.GAME_SEND_ECK_PACKET(perso.get_compte()
					.getGameThread().get_out(), 5, "");
		...
		case 1:// Discours NPC
			...
			if (args.equalsIgnoreCase("DV")) {
				...
				SocketManager.GAME_SEND_QUESTION_PACKET(out,
						quest.parseToDQPacket(perso));
			}
			break;
		...
		case 5:// objet
			try {
				...
				// Si en ligne (normalement oui)
				if (perso.isOnline())// on envoie le packet qui indique
										// l'ajout//retrait d'un item
				{
					SocketManager.GAME_SEND_Ow_PACKET(perso);
					if (send) {
						if (count >= 0) {
							SocketManager.GAME_SEND_Im_PACKET(perso, "021;"
									+ count + "~" + tID);
						} else if (count < 0) {
							SocketManager.GAME_SEND_Im_PACKET(perso, "022;"
									+ -count + "~" + tID);
						}
					}
				}
		...
		case 6:// Apprendre un m�tier
			try {
				...
				if (mID == 2 || mID == 11 || mID == 13 || mID == 14
					...
					if (perso.totalJobBasic() > 2)// On compte les m�tiers d�ja
													// acquis si c'est sup�rieur
													// a 2 on ignore
					{
				...
				if (mID == 43 || mID == 44 || mID == 45 || mID == 46
					 ...
					 */
					{
						...
						if (perso.getMetierByID(mID) != null)// M�tier d�j�
																// appris
						{
							SocketManager.GAME_SEND_Im_PACKET(perso, "111");
						}
						...
						{
							perso.learnJob(World.getMetier(mID));
							// perso.getMetierByID(mID).addXp(perso,
							// 582000);//Level 100 direct
						}
						/*
						 * }else { SocketManager.GAME_SEND_Im_PACKET(perso,
						 * "12");
						 */
					}
				}
				...
				case Constants.STATS_ADD_SAGE:
					if (perso.getScrollSabiduria() >= 201) {
						SocketManager
								.GAME_SEND_MESSAGE(
										perso,
										"Vous avez arriv� aux maximum du scroll dans c'est caracteristique.",
										Ancestra.CONFIG_MOTD_COLOR);
						return;
					}
		...
		case 13: // Reset Carac
			try {
				perso.resetearStats();
				perso.addCapital((perso.get_lvl() - 1) * 5
						- perso.get_capital());
				SocketManager.GAME_SEND_STATS_PACKET(perso);
			} catch (Exception e) {
				GameServer.addToLog(e.getMessage());
				...
				if (ObjetNeed == 0) {
					...
					if (MapNeed == 0) {
						...
						if (perso.hasItemTemplate(ObjetNeed, 1)
								&& perso.get_curCarte().get_id() == MapNeed) {
							// Le perso a l'item
							...
							SocketManager
									.GAME_SEND_MESSAGE(
											perso,
											"Vous n'�tes pas sur la bonne map du donjon pour �tre t�l�porter.",
											"009900");
						} else {
							// Le perso ne poss�de pas l'item
							SocketManager.GAME_SEND_MESSAGE(perso,
									"Vous ne possedez pas la clef n�cessaire.",
									"009900");
						}
					}
				}
		...
		case 18:// T�l�portation chez sois
			...
			break;
		case 19:// T�l�portation maison de guilde (ouverture du panneau de
				// guilde)
		...
		case 23:// UnlearnJob
			try {
				...
				if (m == null) // N'a pas ce métier
					return;
				perso.unlearnJob(Job);
				SocketManager.GAME_SEND_JR_PACKET(perso, Job);
				SQLManager.ACTUALIZAR_MetierS(perso);
			} catch (Exception e) {
				GameServer.addToLog(e.getMessage());
			};
			break;
		case 24:// SimpleMorph
			try {
				...
				SocketManager.GAME_SEND_ERASE_ON_MAP_TO_MAP(
						perso.get_curCarte(), perso.get_GUID());
		...
		case 25:// SimpleUnMorph
			...
			SocketManager.GAME_SEND_ADD_PLAYER_TO_MAP(perso.get_curCarte(),
					perso);
			break;
		case 26:// T�l�portation enclo de guilde (ouverture du panneau de
				// guilde)
			...
			try {
				for (String MobAndLevel : args.split("\\|")) {
					...
					if (World.getMonstre(monsterID) == null
							|| World.getMonstre(monsterID).getGradeByLevel(
									monsterLevel) == null) {
				...
				MobGroup group = new MobGroup(
						perso.get_curCarte()._nextObjectID, perso.get_curCell()
								.getID(), ValidMobGroup);
			...
			if (perso.get_traque().get_time() < System.currentTimeMillis() - 600000
				...
				if (victimas.size() == 0) {
					SocketManager
							.GAME_SEND_MESSAGE(
									perso,
									"Nous n'avons pas trouve de cible a ta hauteur, reviens plus tard.",
									"000000");
					break;
				}
				...
				SocketManager.GAME_SEND_MESSAGE(
						perso,
						"Vous etes desormais en chasse de : "
								+ tempP.get_name(), "000000");
				...
				SocketManager
						.GAME_SEND_MESSAGE(
								perso,
								"Vous venez juste de signer un contrat, vous devez vous reposer.",
								"000000");
			}
			break;
		...
		case 100:// Donner l'abilit� 'args' � une dragodinde
			...
			World.addDragodinde(new Dragodinde(mount.get_id(), mount
					.get_color(), mount.get_sexe(), mount.get_amour(), mount
					.get_endurance(), mount.get_level(), mount.get_exp(), mount
					.get_nom(), mount.get_fatigue(), mount.get_energie(), mount
					.get_reprod(), mount.get_maturite(), mount.get_serenite(),
					mount.getItemsId(), mount.get_ancetres(), args));
			perso.setMount(World.getDragoByID(mount.get_id()));
			SocketManager.GAME_SEND_Re_PACKET(perso, "+",
					World.getDragoByID(mount.get_id()));
		...
		case 102:// Marier des personnages
			World.PriestRequest(perso, perso.get_curCarte(),
					perso.get_isTalkingWith());
			break;
		case 103:// Divorce
		...
		case 228:// Faire animation Hors Combat
			try {
				...
				SocketManager.GAME_SEND_GA_PACKET_TO_MAP(perso.get_curCarte(),
						"0", 228, perso.get_GUID() + ";" + cellid + ","
								+ Animations.PrepareToGA(animation), "");
			} catch (Exception e) {
				GameServer.addToLog(e.getMessage());
			}
			;
		...
		case 32: // cambiar sexo
			...
			break;
		case 33:// UnlearnSpell
			try {
				int Spell = Integer.parseInt(args);
				if (Spell < 1)
					return;
				if(perso.forgetSpell(Spell))
				{
					if(Ancestra.CONFIG_DEBUG)
						SocketManager.GAME_SEND_SPELL_UPGRADE_SUCCED(perso, Spell, perso.getSortStatBySortIfHas(Spell).getLevel());
						SocketManager.GAME_SEND_STATS_PACKET(perso);
				}
			} catch (Exception e) {
				GameServer.addToLog(e.getMessage());
