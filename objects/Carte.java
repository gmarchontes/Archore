...
public class Carte {
	...
	public static class MountPark {
		...
		private int _price;
		private Map<Integer, Integer> MountParkDATA = new TreeMap<Integer, Integer>();// DragoID,
																						// IDperso

		public MountPark(int owner, Carte map, int cellid, int size,
				String data, int guild, int price) {
			_owner = owner;
			...
			for (String firstCut : data.split(";"))// PosseseurID,DragoID;PosseseurID2,DragoID2;PosseseurID,DragoID3
			{
				try {
					...
					Dragodinde DD = World.getDragoByID(Integer
							.parseInt(secondCut[1]));
					if (DD == null)
						continue;
					MountParkDATA.put(Integer.parseInt(secondCut[1]),
							Integer.parseInt(secondCut[0]));
				} catch (Exception E) {
				}
				;
			}
		...
		public String parseData(int PID, boolean isPublic) {
			...
			for (Entry<Integer, Integer> MPdata : MountParkDATA.entrySet()) {
				if (MPdata.getValue() == PID && isPublic)// Montrer que ses
															// montures
															// uniquement en
															// public
				{
		...
		public String parseDBData() {
			...
			for (Entry<Integer, Integer> MPdata : MountParkDATA.entrySet()) {
				if (str.length() > 0)
					str.append(";");
				str.append(MPdata.getValue()).append(",")
						.append(World.getDragoByID(MPdata.getKey()).get_id());
			}
			return str.toString();
		}
		...
		public static void removeMountPark(int GuildID) {
			for (Entry<Short, objects.Carte.MountPark> mp : World
					.getMountPark().entrySet())// Pour chaque enclo si ils en
												// ont plusieurs
			{
				if (mp.getValue().get_guild().get_id() == GuildID) {
					if (!mp.getValue().getData().isEmpty()) {
						for (Entry<Integer, Integer> MPdata : mp.getValue()
								.getData().entrySet()) {
							World.removeDragodinde(MPdata.getKey());// Suppression
																	// des
																	// dindes
																	// dans le
																	// world
							SQLManager.REMOVE_MOUNT(MPdata.getKey());// Suppression
																		// des
																		// dindes
																		// dans
																		// chaque
	...
	public static class Case {
		private int _id;
		private Map<Integer, Personnage> _persos; // = new TreeMap<Integer,
													// Personnage>();
		private Map<Integer, Fighter> _fighters; // = new TreeMap<Integer,
													// Fighter>();
		...
		public boolean canDoAction(int id) {
			...
			case 104:// Ouvrir
			case 105:// Code
				return (_object.getID() == 7350 || _object.getID() == 7351 || _object
						.getID() == 7353);
		...
		public void startAction(Personnage perso, GameAction GA) {
			...
			case 102:// Puiser
				if (!_object.isInteractive())
					return;// Si l'objet est utilis�
				if (_object.getState() != Constants.IOBJECT_STATE_FULL)
				...
				SocketManager.GAME_SEND_GA_PACKET_TO_MAP(
						...
						501,
						perso.get_GUID() + "",
						_id + "," + _object.getUseDuration() + ","
								+ _object.getUnknowValue());
				SocketManager.GAME_SEND_GDF_PACKET_TO_MAP(perso.get_curCarte(),
						this);
				break;
			...
			case 157: // Zaapis
				...
				if (perso.get_curCarte()._subArea.get_area().get_id() == 7
						&& (perso.get_align() == 1 || perso.get_align() == 0 || perso
								.get_align() == 3))// Ange, Neutre ou S�rianne
				{
					Zaapis = Constants.ZAAPI
							.get(Constants.ALIGNEMENT_BONTARIEN).split(",");
					if (perso.get_align() == 1)
						price = 10;
				} else if (perso.get_curCarte()._subArea.get_area().get_id() == 11
						&& (perso.get_align() == 2 || perso.get_align() == 0 || perso
								.get_align() == 3))// D�mons, Neutre ou S�rianne
				{
					Zaapis = Constants.ZAAPI.get(
							Constants.ALIGNEMENT_BRAKMARIEN).split(",");
					if (perso.get_align() == 2)
						price = 10;
				} else {
					Zaapis = Constants.ZAAPI.get(Constants.ALIGNEMENT_NEUTRE)
							.split(",");
				}
			...
			case 176:// Achat enclo
				...
				SocketManager.GAME_SEND_R_PACKET(perso, "D" + MP.get_price()
						+ "|" + MP.get_price());
				break;
			...
			case 178:// Modifier prix de vente
				...
				SocketManager.GAME_SEND_R_PACKET(perso, "D" + MP1.get_price()
						+ "|" + MP1.get_price());
				break;
			...
			case 81:// V�rouiller maison
				House h = House.get_house_id_by_coord(perso.get_curCarte()
						.get_id(), CcellID);
				if (h == null)
					return;
				perso.setInHouse(h);
			...
			case 84:// Rentrer dans une maison
				House h2 = House.get_house_id_by_coord(perso.get_curCarte()
						.get_id(), CcellID);
				if (h2 == null)
					return;
				perso.setInHouse(h2);
			...
			case 97:// Acheter maison
				House h3 = House.get_house_id_by_coord(perso.get_curCarte()
						.get_id(), CcellID);
				if (h3 == null)
					return;
				perso.setInHouse(h3);
			...
			case 104:// Ouvrir coffre priv�
				Trunk trunk = Trunk.get_trunk_id_by_coord(perso.get_curCarte()
						.get_id(), CcellID);
				if (trunk == null) {
					return;
				}
			...
			case 105:// V�rouiller coffre
				Trunk t = Trunk.get_trunk_id_by_coord(perso.get_curCarte()
						.get_id(), CcellID);
				if (t == null) {
					return;
				}
			...
			case 108:// Modifier prix de vente
				House h4 = House.get_house_id_by_coord(perso.get_curCarte()
						.get_id(), CcellID);
				if (h4 == null)
					return;
				perso.setInHouse(h4);
			...
			case 181:
				SocketManager
						.ENVIAR_GDF_FORZADO_PERSONAJE(perso, CcellID, 3, 1);
			...
			default:
				if (Ancestra.CONFIG_DEBUG)
					GameServer
							.addToLog("Case.startAction non definie pour l'actionID = "
									+ actionID);
				break;
			}
		}
		...
		public void finishAction(Personnage perso, GameAction GA) {
			...
			case 102:// Puiser
				...
				SocketManager.GAME_SEND_GDF_PACKET_TO_MAP(perso.get_curCarte(),
						this);
				int qua = Formulas.getRandomValue(1, 10);// On a entre 1 et 10
															// eaux
			...
			default:
				if (Ancestra.CONFIG_DEBUG)
					GameServer
							.addToLog("[FIXME]Case.finishAction non definie pour l'actionID = "
									+ actionID);
				break;
			}
		}
	...
	public Carte(short _id, String _date, byte _w, byte _h, String _key,
			String places, String dData, String cellsData, String monsters,
			String mapPos, byte maxGroup, byte maxSize) {
		this._id = _id;
		this._date = _date;
		this._w = _w;
		this._h = _h;
		...
		if (!dData.isEmpty()) {
			...
			for (String o : cellsDataArray) {
				...
				_cases.put(Number, new Case(this, Number, Walkable,
						LineOfSight, obj));
			}
		}
		...
		if (Ancestra.CONFIG_USE_MOBS) {
			if (_maxGroup == 0)
				return;
			spawnGroup(Constants.ALIGNEMENT_NEUTRE, _maxGroup, false, -1);// Spawn
																			// des
																			// groupes
																			// d'alignement
																			// neutre
			spawnGroup(Constants.ALIGNEMENT_BONTARIEN, 1, false, -1);// Spawn du
																		// groupe
																		// de
																		// gardes
																		// bontarien
																		// s'il
																		// y a
			spawnGroup(Constants.ALIGNEMENT_BRAKMARIEN, 1, false, -1);// Spawn
																		// du
																		// groupe
																		// de
																		// gardes
	...
	public Carte(short id, String date, byte w, byte h, String key,
			String places) {
		_id = id;
		_date = date;
		_w = w;
	...
	public void spawnGroup(int align, int nbr, boolean log, int cellID) {
		...
		if (_mobGroups.size() - _fixMobGroups.size() >= _maxGroup)
			return;
		for (int a = 1; a <= nbr; a++) {
			MobGroup group = new MobGroup(_nextObjectID, align, _mobPossibles,
					this, cellID, this._maxSize);
			if (group.getMobs().isEmpty())
				continue;
			_mobGroups.put(_nextObjectID, group);
			if (log) {
				GameServer.addToLog("Groupe de monstres ajoutes sur la map: "
						+ _id + " alignement: " + align + " ID: "
						+ _nextObjectID);
	...
	public void spawnNewGroup(boolean timer, int cellID, String groupData,
			String condition) {
	...
	public Fight newKoli(ArrayList<Personnage> team1,
			ArrayList<Personnage> team2) {
		int id = 1;
		if (!_fights.isEmpty())
			id = ((Integer) (_fights.keySet().toArray()[_fights.size() - 1])) + 1;
		Fight f = new Fight(0, this, team1, team2);
		_fights.put(id, f);
		return f;
	}
	...
	public String getGMsPackets() {
		...
		for (Case cell : _cases.values())
			for (Personnage perso : cell.getPersos().values())
				packet.append("GM|+").append(perso.parseToGM())
						.append('\u0000');
		return packet.toString();
	}
	...
	public String getFightersGMsPackets() {
		...
		for (Entry<Integer, Case> cell : _cases.entrySet()) {
			for (Entry<Integer, Fighter> f : cell.getValue().getFighters()
					.entrySet()) {
	...
	public String getObjectsGDsPackets() {
		...
		for (Entry<Integer, Case> entry : _cases.entrySet()) {
			if (entry.getValue().getObject() != null) {
				...
				toreturn.append("GDF|").append(cellID).append(";")
						.append(object.getState()).append(";")
	...
	public int getRandomFreeCellID() {
		...
		for (Entry<Integer, Case> entry : _cases.entrySet()) {
			...
			for (Entry<Integer, NPC> npcEntry : _npcs.entrySet()) {
				if (npcEntry.getValue().get_cellID() == entry.getValue()
						.getID())
					ok = false;
			}
			if (!ok)
	...
	public void refreshSpawns() {
		...
		spawnGroup(Constants.ALIGNEMENT_NEUTRE, _maxGroup, true, -1);// Spawn
																		// des
																		// groupes
																		// d'alignement
																		// neutre
		spawnGroup(Constants.ALIGNEMENT_BONTARIEN, 1, true, -1);// Spawn du
																// groupe de
																// gardes
																// bontarien
																// s'il y a
		spawnGroup(Constants.ALIGNEMENT_BRAKMARIEN, 1, true, -1);// Spawn du
																	// groupe de
																	// gardes
																	// brakmarien
																	// s'il y a
	...
	public void onPlayerArriveOnCell(Personnage perso, int caseID,
		...
		if (!hasEndingFight) {
			for (MobGroup group : _mobGroups.values()) {
				if (Pathfinding.getDistanceBetween(this, caseID,
						group.getCellID()) <= group.getAggroDistance())// S'il y
																		// aggro
				{
					if ((group.getAlignement() == -1 || ((perso.get_align() == 1 || perso
							.get_align() == 2) && (perso.get_align() != group
							.getAlignement())))
							&& ConditionParser.validConditions(perso,
									group.getCondition())) {
	...
	public Carte getMapCopy() {
		...
		for (Entry<Integer, Case> entry : _cases.entrySet())
			cases.put(entry.getKey(), new Case(map, entry.getValue().getID(),
					entry.getValue().isWalkable(false), entry.getValue()
							.isLoS(),
					(entry.getValue().getObject() == null ? -1 : entry
							.getValue().getObject().getID())));
		map.setCases(cases);
		return map;
	}
	...
	public InteractiveObject getMountParkDoor() {
		for (Case c : _cases.values()) {
			if (c.getObject() == null)
				continue;
			// Si enclose
			if (c.getObject().getID() == 6763 || c.getObject().getID() == 6766
	...
	public void sendFloorItems(Personnage perso) {
		for (Case c : _cases.values()) {
			if (c.getDroppedItem() != null)
				SocketManager.GAME_SEND_GDO_PACKET(perso, '+', c.getID(), c
						.getDroppedItem().getTemplate().getID(), 0);
		}
	}
	...
	public int getStoreCount() {
		return (World.getSeller(get_id()) == null ? 0 : World.getSeller(
				get_id()).size());
	}
	...
	public void cleanOfflinePlayers() {
		for (Iterator<Case> iterator = _cases.values().iterator(); iterator
				.hasNext();) {
			Case c = (Case) iterator.next();
			for (Iterator<?> iterator1 = c.getPersos().values().iterator(); iterator1
					.hasNext();) {
				Personnage entry = (Personnage) iterator1.next();
				if (!entry.isOnline()) {
					SocketManager.GAME_SEND_ERASE_ON_MAP_TO_MAP(this,
							entry.get_GUID());
					c.removePlayer(entry.get_GUID());
				}
			}
	...
	public void cleanOfflinePlayersV2() {
		for (Iterator<Case> iterator = _cases.values().iterator(); iterator
				.hasNext();) {
			Case c = (Case) iterator.next();
			if (c != null && c.getPersos() != null) {
				for (Iterator<?> iterator1 = c.getPersos().values().iterator(); iterator1
					...
					Personnage entry = (Personnage) iterator1.next();
					if (entry != null
							&& World.getPersonnage(entry.get_GUID()) == null) {
						try {
							SocketManager.GAME_SEND_ERASE_ON_MAP_TO_MAP(this,
									entry.get_GUID());
							c.removePlayer(entry.get_GUID());
	...
	public int getRandomNearFreeCellID(int cellid)// obtenir une cell al�atoire
													// et proche
	{
		ArrayList<Integer> freecell = new ArrayList<Integer>();
		...
		for (Integer entry : cases) {
			...
			ok = true;
			for (Entry<Integer, NPC> npcEntry : _npcs.entrySet()) {
				if (npcEntry.getValue().get_cellID() == _cases.get(entry)
						.getID())
					ok = false;
			}
			if (!ok)
		...
		if (freecell.isEmpty()) {
			GameServer
					.addToLog("Aucune cellulle libre n'a ete trouve sur la map "
							+ _id + " groupe non d�plac�");
			return -1;
		}
		int rand = Formulas.getRandomValue(0, freecell.size() - 1);
		return freecell.get(rand);
	...
	public void onMap_MonstersDisplacement() {
		...
		for (Entry<Integer, MobGroup> entry : getMobGroups().entrySet()) {
			...
			String pathstr;
			try {
				pathstr = Pathfinding.getShortestStringPathBetween(this, entry
						.getValue().getCellID(), cell, 0);
			} catch (Exception e) {
				return;
			}
			if (pathstr == null)
			...
			for (Personnage z : getPersos()) {
				SocketManager.GAME_SEND_GA_PACKET(z.get_compte()
						.getGameThread().get_out(), "0", "1", entry.getValue()
						.getID() + "", pathstr);
			}
		}
		...
		if (MoveOrNot == 1) {
			...
			String pathstr;
			try {
				pathstr = Pathfinding.getShortestStringPathBetween(this,
						perco.get_cellID(), cell, 0);
			} catch (Exception e) {
				return;
			}
			if (pathstr == null)
			...
			for (Personnage z : getPersos()) {
				SocketManager.GAME_SEND_GA_PACKET(z.get_compte()
						.getGameThread().get_out(), "0", "1", perco.getGuid()
						+ "", pathstr);
			}
		}
	}
