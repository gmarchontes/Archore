...
public class Guild {
	...
	private Map<Integer, SortStats> Spells = new TreeMap<Integer, SortStats>(); // <ID,
																				// Level>
	private Map<Integer, Integer> stats = new TreeMap<Integer, Integer>(); // <Effet,
																			// Quantit�>
	// Stats en combat
	...
	public static class GuildMember {
		...
		public GuildMember(int gu, Guild g, String name, int lvl, int gfx,
				int r, long x, byte pXp, int ri, byte a, String lastCo) {
			_guid = gu;
			_guild = g;
			_name = name;
			_level = lvl;
		...
		public int getHoursFromLastCo() {
			...
			LocalDate lastCo = new LocalDate(Integer.parseInt(strDate[0]),
					Integer.parseInt(strDate[1]), Integer.parseInt(strDate[2]));
		...
		public void setAllRights(int rank, byte xp, int right) {
			...
			if (right != this._rights && right != 1) // V�rifie si les droits
														// sont pareille ou si
														// des droits de meneur;
														// pour ne pas faire la
														// conversion pour rien
		...
		public void parseIntToRight(int total) {
			...
			if (haveRight.size() > 0) // Si les droits contiennent quelque chose
										// -> Vidage (M�me si le TreeMap
										// supprimerais les entr�es doublon lors
										// de l'ajout)
				haveRight.clear();
			...
			Integer[] mapKey = haveRight.keySet().toArray(
					new Integer[haveRight.size()]); // R�cup�re les clef de map
													// dans un tableau d'Integer
	...
	public Guild(int id, String name, String emblem, int lvl, long xp,
			int capital, int nbrmax, String sorts, String stats) {
		_id = id;
		_name = name;
		_emblem = emblem;
		_xp = xp;
		...
		statsFight.put(Constants.STATS_ADD_SAGE,
				get_Stats(Constants.STATS_ADD_SAGE));
		...
		statsFight.put(Constants.STATS_ADD_RP_NEU,
				(int) Math.floor(get_lvl() / 2));
		statsFight.put(Constants.STATS_ADD_RP_FEU,
				(int) Math.floor(get_lvl() / 2));
		statsFight.put(Constants.STATS_ADD_RP_EAU,
				(int) Math.floor(get_lvl() / 2));
		statsFight.put(Constants.STATS_ADD_RP_AIR,
				(int) Math.floor(get_lvl() / 2));
		statsFight.put(Constants.STATS_ADD_RP_TER,
				(int) Math.floor(get_lvl() / 2));
		statsFight.put(Constants.STATS_ADD_AFLEE,
				(int) Math.floor(get_lvl() / 2));
		statsFight.put(Constants.STATS_ADD_MFLEE,
				(int) Math.floor(get_lvl() / 2));
	}

	public GuildMember addMember(int guid, String name, int lvl, int gfx,
			int r, byte pXp, long x, int ri, byte a, String lastCo) {
		GuildMember GM = new GuildMember(guid, this, name, lvl, gfx, r, x, pXp,
				ri, a, lastCo);
	...
	public GuildMember addNewMember(Personnage p) {
		GuildMember GM = new GuildMember(p.get_GUID(), this, p.get_name(),
				p.get_lvl(), p.get_gfxID(), 0, 0, (byte) 0, 0, p.get_align(), p
						.get_compte().getLastConnectionDate());
	...
	public void boostSpell(int ID) {
		...
		if (SS != null && SS.getLevel() == 5)
			return;
		Spells.put(ID, ((SS == null) ? World.getSort(ID).getStatsByLevel(1)
	...
	public void removeMember(Personnage perso) {
		House h = House.get_HouseByPerso(perso);// On prend �a maison
		if (h != null) {
	...
	public String compileSpell() {
		...
		for (Entry<Integer, SortStats> curSpell : Spells.entrySet()) {
			...
			toReturn.append(curSpell.getKey())
					.append(";")
					.append(((curSpell.getValue() == null) ? 0 : curSpell
							.getValue().getLevel()));
	...
	public String compileStats() {
		...
		for (Entry<Integer, Integer> curStats : stats.entrySet()) {
			...
			toReturn.append(curStats.getKey()).append(";")
					.append(curStats.getValue());
	...
	public String parsePercotoGuild() {
		...
		packet.append(100 * get_lvl()).append("|").append(get_lvl())
				.append("|");
		packet.append(get_Stats(158)).append("|").append(get_Stats(176))
				.append("|");
		packet.append(get_Stats(124)).append("|").append(get_nbrPerco())
				.append("|");
		packet.append(get_Capital()).append("|")
				.append((1000 + (10 * get_lvl()))).append("|")
				.append(compileSpell());
