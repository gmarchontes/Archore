...
public class Percepteur {
	...
	public Percepteur(int guid, short map, int cellID, byte orientation,
			int GuildID, short N1, short N2, String items, long kamas, long xp) {
		_guid = guid;
		_MapID = map;
		_cellID = cellID;
	...
	public static String parseGM(Carte map) {
		...
		for (Entry<Integer, Percepteur> perco : World.getPercos().entrySet()) {
			if (perco.getValue()._inFight > 0)
				continue;// On affiche pas le perco si il est en combat
			if (perco.getValue()._MapID == map.get_id()) {
				...
				sock.append(perco.getValue()._N1).append(",")
						.append(perco.getValue()._N2).append(";");
	...
	public static String parsetoGuild(int GuildID) {
		...
		for (Entry<Integer, Percepteur> perco : World.getPercos().entrySet()) {
			if (perco.getValue().get_guildID() == GuildID) {
				Carte map = World
						.getCarte((short) perco.getValue().get_mapID());
				if (isFirst)
					packet.append("+");
				if (!isFirst)
					packet.append("|");
				packet.append(perco.getValue().getGuid()).append(";")
						.append(perco.getValue().get_N1()).append(",")
						.append(perco.getValue().get_N2()).append(";");

				packet.append(Integer.toString(map.get_id(), 36)).append(",")
						.append(map.getX()).append(",").append(map.getY())
						.append(";");
				...
				if (perco.getValue().get_inFight() == 1) {
					if (map.getFight(perco.getValue().get_inFightID()) == null) {
						...
						packet.append(perco.getValue().get_turnTimer()).append(
								";");// TimerActuel si combat
					}
					packet.append("45000;");// TimerInit
					int numcase = (World.getCarte(perco.getValue().get_mapID())
							.get_maxTeam1() - 1);
					if (numcase > 7)
						numcase = 7;
					packet.append(numcase).append(";");// Nombre de place
														// maximum : En fonction
														// de la map moins celle
														// du perco
					packet.append("?,?,");// ?
				} else {
					packet.append("0;");
				...
				// ?,?,callername,startdate(Base
				// 10),lastHarvesterName,lastHarvestDate(Base
				// 10),nextHarvestDate(Base 10)
				isFirst = false;
			} else {
				continue;
			}
	...
	public static void parseAttaque(Personnage perso, int guildID) {
		for (Entry<Integer, Percepteur> perco : World.getPercos().entrySet()) {
			if (perco.getValue().get_inFight() > 0
					&& perco.getValue().get_guildID() == guildID) {
				SocketManager.GAME_SEND_gITp_PACKET(perso,
						parseAttaqueToGuild(perco.getValue()));
			}
		}
	}
	...
	public static void parseDefense(Personnage perso, int guildID) {
		for (Entry<Integer, Percepteur> perco : World.getPercos().entrySet()) {
			if (perco.getValue().get_inFight() > 0
					&& perco.getValue().get_guildID() == guildID) {
				SocketManager.GAME_SEND_gITP_PACKET(perso,
						parseDefenseToGuild(perco.getValue()));
			}
		}
	}
	...
	public static String parseAttaqueToGuild(Percepteur perco) {
		...
		Fight F = World.getCarte(perco.get_mapID()).getFight(
				perco.get_inFightID());
		for (Fighter f : F.getFighters(1))// Attaque
		{
			str.append("|");
			str.append(Integer.toString(f.getPersonnage().get_GUID(), 36))
					.append(";");
	...
	public void removeFromPercepteur(Personnage P, int guid, int qua) {
		...
		if (PersoObj == null)// Si le joueur n'avait aucun item similaire
		{
			...
			} else // S'il reste des objets
			{
				...
				String str = "O+" + PercoObj.getGuid() + "|"
						+ PercoObj.getQuantity() + "|"
						+ PercoObj.getTemplate().getID() + "|"
						+ PercoObj.parseStatsString();
				SocketManager.GAME_SEND_EsK_PACKET(P, str);
	...
	public String get_LogItems() {
		...
		for (Objet obj : _LogObjets.values()) {
			if (!isFirst)
				str.append(";");
			str.append(obj.getTemplate().getID()).append(",")
					.append(obj.getQuantity());
			isFirst = false;
		}
		return str.toString();
	}
	...
	public static void removePercepteur(int GuildID) {
		for (Entry<Integer, Percepteur> perco : World.getPercos().entrySet()) {
			if (perco.getValue().get_guildID() == GuildID) {
				...
				for (Personnage p : World.getCarte(
						(short) perco.getValue().get_mapID()).getPersos()) {
					SocketManager.GAME_SEND_ERASE_ON_MAP_TO_MAP(
							p.get_curCarte(), perco.getValue().getGuid());// Suppression
																			// visuelle
				}
				SQLManager.DELETE_PERCO(perco.getKey());// Supprime les
														// percepteurs
			} else {
				continue;
			}
	...
	public boolean addDefenseFight(Personnage P) {
		if (_DefensepersosID.size() >= World.getCarte(this.get_mapID())
				.get_maxTeam1()) {
			return false;
		} else {
	...
	public ArrayList<Drop> getDrops() {
		...
		for (Objet obj : _objets.values()) {
			toReturn.add(new Drop(obj.getTemplate().getID(), 0, 100, obj
					.getQuantity()));
		}
		return toReturn;
	}
