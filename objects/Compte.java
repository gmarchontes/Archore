...
public class Compte {
	...
	public long _endTimestampMute;
	public int _muteTime;
	public long _muteHeur = 0;
	public int _position = -1;// Position du joueur
	private int _regalo;
	private Map<Integer, ArrayList<HdvEntry>> _hdvsItems;// Contient les items
															// des HDV format :
															// <hdvID,<cheapestID>>
	private int _points;
	...
	public Compte(int aGUID, String aName, String aPass, String aPseudo,
			String aQuestion, String aReponse, int aGmLvl, int subscriber, int vip,
			boolean aBanned, String aLastIp, String aLastConnectionDate,
			String bank, int bankKamas, String friends, String enemy,
			int regalo, int points) {
		this._GUID = aGUID;
		this._name = aName;
		this._pass = aPass;
	...
	public void mute(boolean Mute, int Temps) {
		...
		String msg = "";
		if (_mute)
			msg = "Vous avez été muté.";
		else
			msg = "Vous avez été démuté.";
		SocketManager.GAME_SEND_MESSAGE(_curPerso, msg,
				Ancestra.CONFIG_MOTD_COLOR);
		if (Temps == 0)
			return;
		_muteTime = Temps * 1000;
		_endTimestampMute = System.currentTimeMillis() + Temps * 1000; // Fin du mute
	...
	public static boolean COMPTE_LOGIN(String name, String pass, String key) {
		if (World.getCompteByName(name) != null) {
			World.addAccount(SQLManager.REFRESH_ACCOUNT_INFOS(World
					.getCompteByName(name)));
		}
		if (World.getCompteByName(name) != null
				&& World.getCompteByName(name).isValidPass(pass, key)) {
			return true;
		} else {
			return false;
		}
	...
	public boolean createPerso(String name, int sexe, int classe, int color1,
			int color2, int color3) {

		Personnage perso = Personnage.CREATE_PERSONNAGE(name, sexe, classe,
				color1, color2, color3, this);
		if (perso == null) {
			return false;
		}
		_persos.put(perso.get_GUID(), perso);
	...
	public void updateInfos(int aGUID, String aName, String aPass,
			String aPseudo, String aQuestion, String aReponse, int aGmLvl,
			boolean aBanned) {
		this._GUID = aGUID;
		this._name = aName;
		this._pass = aPass;
	...
	public void resetAllChars(boolean save) {
		for (Personnage P : _persos.values()) {
			...
			if (P.get_curExchange() != null)
				P.get_curExchange().cancel();
			// Si en groupe
			if (P.getGroup() != null)
				P.getGroup().leave(P);
			...
			if (P.get_isTradingWith() > 0) {
				...
				if (p != null) {
					if (p.isOnline()) {
						PrintWriter out = p.get_compte().getGameThread()
								.get_out();
	...
	public void SendOnline() {
		for (int i : _friendGuids) {
			if (this.isFriendWith(i)) {
				...
				if (perso != null && perso.is_showFriendConnection()
						&& perso.isOnline())
					SocketManager
							.GAME_SEND_FRIEND_ONLINE(this._curPerso, perso);
			}
		}
	}
	...
	public void addFriend(int guid) {
		...
		if (!_friendGuids.contains(guid)) {
			...
			SocketManager.GAME_SEND_FA_PACKET(
					_curPerso,
					"K"
							+ World.getCompte(guid).get_pseudo()
							+ World.getCompte(guid).get_curPerso()
									.parseToFriendList(_GUID));
			SQLManager.UPDATE_ACCOUNT_DATA(this);
		} else
			SocketManager.GAME_SEND_FA_PACKET(_curPerso, "Ea");
	}

	public void removeFriend(int guid) {
		if (_friendGuids.remove((Object) guid))
			SQLManager.UPDATE_ACCOUNT_DATA(this);
	...
	public void addEnemy(String packet, int guid) {
		...
		if (!_EnemyGuids.contains(guid)) {
			...
			SocketManager.GAME_SEND_iAEA_PACKET(_curPerso);
	}

	public void removeEnemy(int guid) {
		if (_EnemyGuids.remove((Object) guid))
			SQLManager.UPDATE_ACCOUNT_DATA(this);
	...
	public boolean recoverItem(int ligneID, int amount) {
		...
		int hdvID = Math.abs(_curPerso.get_isTradingWith());// R�cup�re l'ID de
															// l'HDV

		HdvEntry entry = null;
		for (HdvEntry tempEntry : _hdvsItems.get(hdvID))// Boucle dans la liste
														// d'entry de l'HDV pour
														// trouver un entry avec
														// le meme cheapestID
														// que sp�cifi�
		{
			if (tempEntry.getLigneID() == ligneID)// Si la boucle trouve un
													// objet avec le meme
													// cheapestID, arrete la
													// boucle
			{
				entry = tempEntry;
				break;
		...
		if (entry == null)// Si entry == null cela veut dire que la boucle s'est
							// effectu� sans trouver d'item avec le meme
							// cheapestID
			return false;

		_hdvsItems.get(hdvID).remove(entry);// Retire l'item de la liste des
											// objets a vendre du compte

		Objet obj = entry.getObjet();

		boolean OBJ = _curPerso.addObjet(obj, true);// False = Meme item dans
													// l'inventaire donc
													// augmente la qua
		if (!OBJ) {
