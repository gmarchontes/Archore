...
public class RealmThread implements Runnable {
	...
	private void parsePacket(String packet) {
		switch (_packetNum) {
		case 1:// Version
			if (!packet.equalsIgnoreCase(Constants.CLIENT_VERSION)
					&& !Constants.IGNORE_VERSION) {
		...
		case 3:// HashPass
			...
			if (Compte.COMPTE_LOGIN(_accountName, _hashPass, _hashKey)) {
				...
				} else if (_compte.isOnline()
						&& _compte.getGameThread() == null) {
					SocketManager.REALM_SEND_ALREADY_CONNECTED(_out);
					SocketManager.REALM_SEND_ALREADY_CONNECTED(_compte
							.getRealmThread()._out);
					return;
				}
				if (_compte.isBanned()) {
				...
				if (Ancestra.CONFIG_PLAYER_LIMIT != -1
						&& Ancestra.CONFIG_PLAYER_LIMIT <= Ancestra.gameServer
								.getPlayerNumber()) {
					// Seulement si joueur
				...
				if (World.ipIsUsed2(ip) >= Ancestra.PLAYER_IP) {
					SocketManager.REALM_SEND_TOO_MANY_PLAYER_ERROR(_out);
					try {
						_s.close();
					} catch (IOException e) {
				...
				_compte.setCurIP(ip);
				RealmServer._totalAbo++;// On incr�mente le total
				_compte._position = RealmServer._totalAbo;// On lui donne une
															// position
				SocketManager.REALM_SEND_Ad_Ac_AH_AlK_AQ_PACKETS(_out, _compte
						.get_pseudo(), (_compte.get_gmLvl() > 0 ? (1) : (0)),
			...
			} else// Si le compte n'a pas �t� reconnu
			{
				...
				if (Compte.COMPTE_LOGIN(_accountName, _hashPass, _hashKey)) {
					...
					if (Ancestra.CONFIG_PLAYER_LIMIT != -1
						...
						if (_compte.get_gmLvl() == 0 && _compte.get_vip() == 0) {
							SocketManager
									.REALM_SEND_TOO_MANY_PLAYER_ERROR(_out);
							try {
								_s.close();
							} catch (IOException e) {
							}
					...
					SocketManager.REALM_SEND_Ad_Ac_AH_AlK_AQ_PACKETS(_out,
							_compte.get_pseudo(),
							(_compte.get_gmLvl() > 0 ? (1) : (0)), _compte
									.get_question());
				} else// Si le compte n'a pas �t� reconnu
				{
		...
		default:
			if (packet.substring(0, 2).equals("Af")) {
				...
				SocketManager.REALM_SEND_PERSO_LIST(_out,
						_compte.GET_PERSO_NUMBER());
			} else {
				if (!packet.equals("AX" + Ancestra.serverID))
					break;
				Ancestra.gameServer.addWaitingCompte(this._compte);
				String ip = this._compte.get_curIP();
				SocketManager.REALM_SEND_GAME_SERVER_IP(this._out,
						this._compte.get_GUID(), ip.equals("127.0.0.1"));
			}
		}
	}
}
